...
<TR><TD><FONT Face="Verdana, Arial" size="2"><B>Topic:</B></font></td><TD><FONT Face="Verdana, Arial" size="2">HOWTO: Installing OE on Mandrake 8.1</FONT></td></tr></table>
</td></tr>
...
<td>
<TEXTAREA NAME="ReplyMessage" ROWS=10 COLS=45 WRAP="VIRTUAL">[QUOTE]Originally posted by flipper:
[B]1.  Print out the instructions for installing OE on RH7.2, you'll be following those almost to the "T"

2. check for the following libraries:

[b]rpm -qa | grep 'pcap'[/b]
This should produce:
libpcap0-0.6.2-1mdk
libpcap0-devel-0.6.2-1mdk.

You can get libpcap on disk1 and pcap-devel is on disk2 of the Mandrake 8.1 install disks.

[b]rpm -qa | grep 'Mesa'[/b]
This should produce:
...
I believe you need all of these, so check disk 1 and disk 2 for the appropriate RPMs.

3. If you have an Nvidia card, then you can download the NVIDIA_kernel and NVIDIA_GLX files and install them.  Also download the README and make sure you modify your /etc/X11/XF86Config-4 file as specified in the README.

4. Go into X and run [b][i]gears --info[/i][/b] from an xterm.  If this works, then your OpenGL is working.

5. Now download qt-3.0.1 as specified in Slicer's instructions and compile it the way he says to.

6. Now grab odinseye and daoccrypt and install/compile them per Slicer's instructions.  Don't forget to set the QTDIR.

7. Run/test OE as specified in Slicer's instructions.

Enjoy!

==Flipper==

[This message has been edited by flipper (edited 12-17-2001).][/B][/QUOTE]
...
<TABLE BORDER=0 cellpadding=4 border=0 cellspacing=1 WIDTH="95%"><TR bgcolor="#000000"><TD COLSPAN=2><CENTER><FONT SIZE="2" COLOR="#00ff00" FACE="Verdana, Arial">T O P I C  &nbsp;&nbsp;&nbsp; R E V I E W</font></CENTER></TD></TR><TR bgcolor="#1f1f1f"><TD valign=top><FONT SIZE="2" FACE="Verdana, Arial">flipper</FONT></TD><TD><FONT SIZE="2" FACE="Verdana, Arial">1.  Print out the instructions for installing OE on RH7.2, you'll be following those almost to the "T"<P>2. check for the following libraries:<P><B>rpm -qa | grep 'pcap'</B><BR>This should produce:<BR>libpcap0-0.6.2-1mdk<BR>libpcap0-devel-0.6.2-1mdk.<P>You can get libpcap on disk1 and pcap-devel is on disk2 of the Mandrake 8.1 install disks.<P><B>rpm -qa | grep 'Mesa'</B><BR>This should produce:<BR>Mesa-common-3.4.2-2mdk<BR>Mesa-common-devel-3.4.2-2mdk<BR>Mesa-3.4.2-2mdk<BR>Mesa-demos-3.4.2-2mdk<P>I believe you need all of these, so check disk 1 and disk 2 for the appropriate RPMs.<P>3. If you have an Nvidia card, then you can download the NVIDIA_kernel and NVIDIA_GLX files and install them.  Also download the README and make sure you modify your /etc/X11/XF86Config-4 file as specified in the README.<P>4. Go into X and run <B><I>gears --info</I></B> from an xterm.  If this works, then your OpenGL is working.<P>5. Now download qt-3.0.1 as specified in Slicer's instructions and compile it the way he says to.<P>6. Now grab odinseye and daoccrypt and install/compile them per Slicer's instructions.  Don't forget to set the QTDIR.<P>7. Run/test OE as specified in Slicer's instructions.<P>Enjoy!<P>==Flipper==<p>[This message has been edited by flipper (edited 12-17-2001).]</FONT></TD></TR><TR bgcolor="#1f1f1f"><TD valign=top><FONT SIZE="2" FACE="Verdana, Arial">flipper</FONT></TD><TD><FONT SIZE="2" FACE="Verdana, Arial">The latest patch (with the missing images directory) compiles and runs just great in Mandrake 8.1.<P> <IMG SRC="http://www.ethernalquest.com/ubb/smile.gif"></FONT></TD></TR></TABLE>
</FONT>
