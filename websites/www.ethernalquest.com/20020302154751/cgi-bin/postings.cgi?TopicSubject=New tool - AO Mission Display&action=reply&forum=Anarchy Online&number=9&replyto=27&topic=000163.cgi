...
<TR><TD><FONT Face="Verdana, Arial" size="2"><B>Topic:</B></font></td><TD><FONT Face="Verdana, Arial" size="2">New tool - AO Mission Display</FONT></td></tr></table>
</td></tr>
...
<td>
<TEXTAREA NAME="ReplyMessage" ROWS=10 COLS=45 WRAP="VIRTUAL">[QUOTE]Originally posted by GRKN:
[B]Does this program work by sniffing packets? If so, then I think I know how to make it work on a dialup. You could just program a small proxy into it, and have Anarchy use that proxy. Then replace the WinPCap references with references to the proxy. To make Anarchy use the proxy, you find the address in memory that contains the IP of the server anarchy connects to, then add a memory editor to the program, so that when it starts, it edits that portion of the memory to 127.0.0.1. Then when anarchy tries to connect it will really be connecting to itself on a different port. Now make the program accept this connection then open a new connection to the real server. After that the program can easily relay messages to and from the server and client, while checking the packets for the mission data(or anything else you add to it).

Sadly enough, I only know VB. So, if this was programmed in Visual Basic, I will gladly help program this in. Otherwise, I'll just be able to help with the concept, ideas, hex editing, and testing.[/B][/QUOTE]
...
<A HREF="http://www.hackersquest.org/ubb/Forum9/HTML/000163.html" target=_blank> *** Click here to review this topic. ***</A>
</FONT>
