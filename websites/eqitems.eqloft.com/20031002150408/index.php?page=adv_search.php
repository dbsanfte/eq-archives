
<html>
<head>
<title>EQItems</title>
<meta http-equiv="Content-Type" content="text/html; charset=iso-8859-1">
<link href="style.css" rel="stylesheet" type="text/css">
</head>

<body bgcolor="#CCCCCC">
<center>
<table width="760" CLASS="table_bord" cellpadding="1" cellspacing="1">
<td>
  <table width="760" CLASS="main_entete">
    <td style="font-size:18px">EQItems</td>
  </table>
</td>
</table>
<br><br>
<table width="760">
  <tr>
    <td width="122" valign="top" align="center">
      <table width="122" CLASS="table_bord" cellpadding="1" cellspacing="1">
	<td>
	  <table width="122" cellspacing="0" cellpadding="0">
  	    <tr>
    	      <td CLASS="menu_entete">Accueil</td>
  	    </tr>
  	    <tr>
 	      <td CLASS="menu_corps"><img src="img/puce.png">&nbsp;<a CLASS="menu" href="index.php?page=news.php">News</a></td>
  	    </tr>
  	    <tr>
    	      <td CLASS="menu_corps"><img src="img/puce.png">&nbsp;<a CLASS="menu" href="index.php?page=contact.php">Contact</a></td>
  	    </tr>
  	  </table>
	</td>
      </table>
      <br>
      <table width="122" CLASS="table_bord" cellpadding="1" cellspacing="1">
	<td>
          <table width="122" cellspacing="0" cellpadding="0">
	    <tr>
    	      <td CLASS="menu_entete">Recherche</td>
  	    </tr>
  	    <tr>
    	      <td CLASS="menu_corps"><img src="img/puce.png">&nbsp;<a CLASS="menu" href="index.php?page=quick_search.php">Rapide</a></td>
  	    </tr>
  	    <tr>
    	      <td CLASS="menu_corps"><img src="img/puce.png">&nbsp;<a CLASS="menu" href="index.php?page=adv_search.php">Avancée</a></td>
  	    </tr>
  	    <tr>
    	      <td CLASS="menu_corps"><img src="img/puce.png">&nbsp;<a CLASS="menu" href="index.php?page=zone_search.php">Par Zone</a></td>
  	    </tr>
	  </table>
	</td>
      </table>
      <br>
      <table width="122" CLASS="table_bord" cellpadding="1" cellspacing="1">
	<td>
          <table width="122" cellspacing="0" cellpadding="0">
  	    <tr>
    	      <td CLASS="menu_entete">Liens</td>
  	    </tr>
  	    <tr>
    	      <td CLASS="menu_corps"><img src="img/puce.png">&nbsp;<a CLASS="menu" href="index.php?page=guides.php">Guides</a></td>
  	    </tr>
  	    <tr>
    	      <td CLASS="menu_corps"><img src="img/puce.png">&nbsp;<a CLASS="menu" href="index.php?page=liens.php">Divers</a></td>
  	    </tr>
	  </table>
	</td>
      </table>
      <br>
      <table width="122" CLASS="table_bord" cellpadding="1" cellspacing="1">
        <td>
	  <table width="122" cellspacing="0" cellpadding="0">
	    <td><a href="http://www.eqloft.com"><img src="img/hebergement.gif" border="0"></a></td>
	  </table>
	</td>
      </table>
    </td>
    <td width="5"><img src="img/spacer.gif"></td>
    <td width="465" valign="top">

    
<table width="465" CLASS="table_bord" cellpadding="1" cellspacing="1">
  <td>
    <table width="465" cellspacing="0" cellpadding="0">
      <tr>
        <td CLASS="main_entete">Recherche avancée</td>
      </tr>
      <tr>
        <td CLASS="main_corps">
          <form action="" method="post" name="choix">
          <table border="0" cellpadding="0" cellspacing="3" align="center" valign="center" CLASS="main_corps">
            <tr>
	      <td>
	        Classe : 
	      </td>
	      <td>
	        <select name="classe[]"><option value="all"
>Any<option value=BRD>Bard<option value=BST>Beastlord<option value=CLR>Cleric<option value=DRU>Druid<option value=ENC>Enchanter<option value=MAG>Magician<option value=MNK>Monk<option value=NEC>Necromancer<option value=PAL>Paladin<option value=RNG>Ranger<option value=ROG>Rogue<option value=SHD>Shadowknight<option value=SHM>Shaman<option value=WAR>Warrior<option value=WIZ>Wizard<option value=ALL>All<option value=NONE>NONE
		</select>
	      </td>
	    </tr>
	    <tr>  
              <td>
                Race :
              </td>
              <td>
                <select name="race[]"><option value="all"
                >Any<option value=BAR>Barbarian<option value=DEF>Dark Elf<option value=DWF>Dwarf<option value=ERU>Erudite<option value=GNM>Gnome<option value=HFL>Halfling<option value=HIE>High Elf<option value=HEF>Half Elf<option value=HUM>Human<option value=IKS>Iksar<option value=OGR>Ogre<option value=TRL>Troll<option value=VAH>Vah Shir<option value=ELF>Wood Elf<option value=ALL>All<option value=NONE>NONE
		</select>
              </td>
            </tr>
            <tr>
              <td>
                Weapon Skill :
              </td>
              <td>
                <select name="weapskill[]"><option value="any"
                >Any<option value=1>1H Blunt<option value=3>1H Slashing<option value=2>2H Blunt<option value=6>2H Piercing<option value=4>2H Slashing<option value=8>Archery<option value=13>Brass Instrument<option value=7>Hand to Hand<option value=12>Percussion Instrument<option value=5>Piercing<option value=10>Stringed Instrument<option value=9>Throwing<option value=11>Wind Instrument
		</select>
	      </td>
	    </tr>
	    <tr>  
              <td>
                Slot :
              </td>
              <td>
                <select name="slot[]"><option value="all"
                >Any<option value=1>Ear<option value=2>Neck<option value=3>Face<option value=4>Head<option value=5>Fingers<option value=6>Wrist<option value=7>Arms<option value=8>Hands<option value=9>Shoulders<option value=10>Back<option value=11>Chest<option value=12>Waist<option value=13>Legs<option value=14>Feet<option value=15>Primary<option value=16>Secondary<option value=17>Range<option value=18>Ammo<option value=19>Inv
		</select>
              </td>
            </tr>
          <!--
          </table>
          <table border="0" cellpadding="0" cellspacing="3" align="center" valign="center" CLASS="main_corps">
          -->
            <tr>
              <td colspan="2">
                <hr>
              </td>
            </tr>   
            <tr>
              <td>
                Effet :
              </td>
              <td>
                <select name="sort[]"><option value="any"
                >Any
		    <option value="1490"
		  >Aanya's Quickening
		    <option value="683"
		  >Aanya`s Animation
		    <option value="1920"
		  >Abduction of Strength
		    <option value="97"
		  >Abolish Disease
		    <option value="1574"
		  >Abolish Enchantment
		    <option value="96"
		  >Abolish Poison
		    <option value="1409"
		  >Abscond
		    <option value="51"
		  >Abundant Drink
		    <option value="52"
		  >Abundant Food
		    <option value="2140"
		  >Abysmal Banishment
		    <option value="1041"
		  >Accuracy
		    <option value="1275"
		  >Acid Jet
		    <option value="2399"
		  >Acidic Current
		    <option value="2187"
		  >AcryliaKB
		    <option value="2104"
		  >Act of Valor
		    <option value="1696"
		  >Acting Armor I
		    <option value="1697"
		  >Acting Armor II
		    <option value="1698"
		  >Acting Armor III
		    <option value="1684"
		  >Acting Cold Resist I
		    <option value="1689"
		  >Acting Cold Resist II
		    <option value="1694"
		  >Acting Cold Shield
		    <option value="1686"
		  >Acting Disease Resist I
		    <option value="1691"
		  >Acting Disease Resist II
		    <option value="1695"
		  >Acting Disease Shield
		    <option value="1683"
		  >Acting Fire Resist I
		    <option value="1688"
		  >Acting Fire Resist II
		    <option value="1692"
		  >Acting Fire Shield
		    <option value="1699"
		  >Acting Health I
		    <option value="1700"
		  >Acting Health II
		    <option value="1701"
		  >Acting Health III
		    <option value="1702"
		  >Acting Health IV
		    <option value="1682"
		  >Acting Magic Resist I
		    <option value="1687"
		  >Acting Magic Resist II
		    <option value="1685"
		  >Acting Poison Resist I
		    <option value="1690"
		  >Acting Poison Resist II
		    <option value="1693"
		  >Acting Poison Shield
		    <option value="1703"
		  >Acting Spirit I
		    <option value="1704"
		  >Acting Spirit II
		    <option value="765"
		  >ActingGuard
		    <option value="1015"
		  >ActingGuard
		    <option value="763"
		  >ActingResist
		    <option value="1013"
		  >ActingResist
		    <option value="764"
		  >ActingShield
		    <option value="1014"
		  >ActingShield
		    <option value="1357"
		  >Acumen
		    <option value="2481"
		  >Acumen
		    <option value="2215"
		  >Acumen of Dar Khura
		    <option value="642"
		  >Adorning Grace
		    <option value="1042"
		  >Adroitness
		    <option value="2442"
		  >Adv Farsight II
		    <option value="2162"
		  >AdvisorNova
		    <option value="1322"
		  >Aegis
		    <option value="1800"
		  >Aegis of Bathezid
		    <option value="1451"
		  >Aegis of Ro
		    <option value="1229"
		  >Aegolism
		    <option value="510"
		  >Affliction
		    <option value="2482"
		  >Affliction Efficiency I
		    <option value="2483"
		  >Affliction Efficiency II
		    <option value="2484"
		  >Affliction Efficiency III
		    <option value="2485"
		  >Affliction Haste I
		    <option value="2486"
		  >Affliction Haste II
		    <option value="2487"
		  >Affliction Haste III
		    <option value="153"
		  >Agility
		    <option value="713"
		  >Agilmente`s Aria of Eagles
		    <option value="1113"
		  >Agilmente`s Aria of Eagles
		    <option value="1967"
		  >Aid of Khurenz
		    <option value="2118"
		  >Air Elemental Form
		    <option value="2119"
		  >Air Elemental Form
		    <option value="2120"
		  >Air Elemental Form
		    <option value="944"
		  >AirElementalAttack
		    <option value="905"
		  >AirElementalInvisibility
		    <option value="997"
		  >AirElementalStrike
		    <option value="169"
		  >Alacrity
		    <option value="722"
		  >Alenia`s Disenchanting Melody
		    <option value="1791"
		  >All Your Base Are Belong To Us
		    <option value="297"
		  >Alliance
		    <option value="183"
		  >Allure
		    <option value="637"
		  >Allure of Death
		    <option value="2488"
		  >Allure of the Pool
		    <option value="2489"
		  >Allure of the Pool(2)
		    <option value="141"
		  >Allure of the Wild
		    <option value="149"
		  >Alluring Aura
		    <option value="1599"
		  >Alluring Whispers
		    <option value="1109"
		  >Aloe Sweat
		    <option value="661"
		  >Alter Plane: Hate
		    <option value="669"
		  >Alter Plane: Sky
		    <option value="1169"
		  >Al`Kabor's Expansive Mind
		    <option value="1948"
		  >Amplification
		    <option value="70"
		  >Anarchy
		    <option value="1268"
		  >Ancient Breath
		    <option value="2385"
		  >Ancient: Burnout V
		    <option value="2387"
		  >Ancient: Chaotic Visions
		    <option value="2382"
		  >Ancient: Destruction of Ice
		    <option value="2386"
		  >Ancient: Eternal Rapture
		    <option value="2378"
		  >Ancient: Feral Avatar
		    <option value="2388"
		  >Ancient: Gift of Aegolism
		    <option value="2383"
		  >Ancient: Greater Concussion
		    <option value="2375"
		  >Ancient: High Priest's Bulwark
		    <option value="2392"
		  >Ancient: Legacy of Blades
		    <option value="2381"
		  >Ancient: Lifebane
		    <option value="2380"
		  >Ancient: Master of Death
		    <option value="2379"
		  >Ancient: Scourge of Nife
		    <option value="2384"
		  >Ancient: Shock of Sun
		    <option value="2393"
		  >Ancient: Starfire of Ro
		    <option value="1530"
		  >Angstlich's Assonance
		    <option value="701"
		  >Angstlich`s Appalling Screech
		    <option value="1114"
		  >Angstlich`s Appalling Screech
		    <option value="439"
		  >Animate Dead
		    <option value="1308"
		  >Annul Magic
		    <option value="2159"
		  >Annul Self
		    <option value="2490"
		  >Annulling Spirit
		    <option value="1656"
		  >Ant Legs
		    <option value="696"
		  >Anthem de Arms
		    <option value="1307"
		  >Antidote
		    <option value="2154"
		  >Applied Singularity
		    <option value="1004"
		  >AprilFoolsSpell
		    <option value="938"
		  >Aqua Lung
		    <option value="1198"
		  >Arch Lich
		    <option value="66"
		  >Arch Shielding
		    <option value="18"
		  >Armor of Faith
		    <option value="1227"
		  >Armor of Protection
		    <option value="1850"
		  >Armor of the Faithful
		    <option value="2260"
		  >Arx Foris
		    <option value="1485"
		  >Asphyxiate
		    <option value="383"
		  >Assiduous Vision
		    <option value="1290"
		  >Asystole
		    <option value="1413"
		  >Atol's Spectral Shackles
		    <option value="479"
		  >Atone
		    <option value="2284"
		  >Attack of the Cral Ligi
		    <option value="1511"
		  >Augment
		    <option value="656"
		  >Augment Death
		    <option value="9"
		  >Augmentation
		    <option value="1196"
		  >Augmentation of Death
		    <option value="1044"
		  >Aura of Antibody
		    <option value="1112"
		  >Aura of Battle
		    <option value="1281"
		  >Aura of Battle
		    <option value="1063"
		  >Aura of Black Petals
		    <option value="1060"
		  >Aura of Blue Petals
		    <option value="1046"
		  >Aura of Cold
		    <option value="1062"
		  >Aura of Green Petals
		    <option value="1047"
		  >Aura of Heat
		    <option value="863"
		  >Aura of Marr
		    <option value="1048"
		  >Aura of Purity
		    <option value="1059"
		  >Aura of Red Petals
		    <option value="2316"
		  >Aura of Scale
		    <option value="1823"
		  >Aura of the Nurga
		    <option value="2255"
		  >Aura of Vinitras
		    <option value="1061"
		  >Aura of White Petals
		    <option value="835"
		  >AuraofElementalMastery
		    <option value="27"
		  >Avalanche
		    <option value="1380"
		  >Avatar
		    <option value="790"
		  >Avatar Guard
		    <option value="1030"
		  >Avatar Guard
		    <option value="2395"
		  >Avatar of Fear
		    <option value="796"
		  >Avatar Power
		    <option value="791"
		  >Avatar Resist
		    <option value="1031"
		  >Avatar Resist
		    <option value="788"
		  >Avatar Shield
		    <option value="1028"
		  >Avatar Shield
		    <option value="789"
		  >Avatar Sight
		    <option value="1029"
		  >Avatar Sight
		    <option value="1093"
		  >Avatar Snare
		    <option value="882"
		  >AvatarKnockback
		    <option value="2242"
		  >Balance of Zebuxoruk
		    <option value="969"
		  >Bamph
		    <option value="1465"
		  >Bandoleer of Luclin
		    <option value="1372"
		  >Bane of Nife
		    <option value="2400"
		  >Bane of Thule
		    <option value="1067"
		  >Bane of Tunare
		    <option value="115"
		  >Banish Summoned
		    <option value="117"
		  >Banish Undead
		    <option value="1313"
		  >Banishment
		    <option value="1312"
		  >Banishment of Shadows
		    <option value="363"
		  >Banshee Aura
		    <option value="2237"
		  >Banshee Wail
		    <option value="515"
		  >Barbcoat
		    <option value="909"
		  >BarbedBones
		    <option value="2491"
		  >Barrage of Debris
		    <option value="675"
		  >Barrier of Combustion
		    <option value="1713"
		  >Barrier of Force
		    <option value="1783"
		  >Battery Vision
		    <option value="1951"
		  >Battlecry of the Vah Shir
		    <option value="1495"
		  >Bedlam
		    <option value="244"
		  >Befriend Animal
		    <option value="181"
		  >Beguile
		    <option value="140"
		  >Beguile Animals
		    <option value="747"
		  >Beguile Plants
		    <option value="196"
		  >Beguile Undead
		    <option value="1840"
		  >Beguiling Luminescence
		    <option value="1912"
		  >Beguiling Visage
		    <option value="932"
		  >Beholder Dispel
		    <option value="1260"
		  >Bellowing Winds
		    <option value="172"
		  >Benevolence
		    <option value="1618"
		  >Berserker Madness I
		    <option value="1640"
		  >Berserker Madness II
		    <option value="1641"
		  >Berserker Madness III
		    <option value="1661"
		  >Berserker Madness IV
		    <option value="175"
		  >Berserker Spirit
		    <option value="20"
		  >Berserker Strength
		    <option value="2033"
		  >Beta AE Gain Mana
		    <option value="2034"
		  >Beta Aegolism
		    <option value="2028"
		  >Beta Clarity II
		    <option value="2032"
		  >Beta Complete Healing
		    <option value="2031"
		  >Beta Focus of Spirit
		    <option value="2029"
		  >Beta POTG
		    <option value="2030"
		  >Beta Regrowth
		    <option value="2026"
		  >Beta Riotous Health
		    <option value="2027"
		  >Beta Visions of Grandeur
		    <option value="2186"
		  >BetaAcrylia
		    <option value="2190"
		  >BetaAcrylia2
		    <option value="1820"
		  >BetaAcrylia3
		    <option value="2492"
		  >BetaATK100
		    <option value="2493"
		  >BetaATK120
		    <option value="2494"
		  >BetaATK140
		    <option value="2495"
		  >BetaATK160
		    <option value="2496"
		  >BetaATK20
		    <option value="2497"
		  >BetaATK40
		    <option value="2498"
		  >BetaATK60
		    <option value="2499"
		  >BetaATK80
		    <option value="2500"
		  >Bewildering Dementia
		    <option value="34"
		  >Bind Affinity
		    <option value="1831"
		  >Bind Affinity
		    <option value="499"
		  >Bind Sight
		    <option value="2501"
		  >Biomelt IX
		    <option value="983"
		  >Bixie Poison
		    <option value="2502"
		  >Black Claw
		    <option value="2419"
		  >Black Winds
		    <option value="2163"
		  >Blade Dance
		    <option value="2164"
		  >Blade DanceSK
		    <option value="1340"
		  >Bladecoat
		    <option value="192"
		  >Blanket of Forgetfulness
		    <option value="2334"
		  >Blast of Frost
		    <option value="1211"
		  >Blast of Poison
		    <option value="119"
		  >Blaze
		    <option value="1995"
		  >Blazing Heat
		    <option value="1854"
		  >Blessed Armor of the Risen
		    <option value="1855"
		  >Blessing of Aegolism
		    <option value="1728"
		  >Blessing of Nature
		    <option value="2315"
		  >Blessing of Ssraeshza
		    <option value="1758"
		  >Blessing of the Blackstar
		    <option value="1746"
		  >Blessing of the Grove
		    <option value="2503"
		  >Blessing of the Harvest
		    <option value="2504"
		  >Blessing of the Storm
		    <option value="1289"
		  >Blessing of the Theurgist
		    <option value="2333"
		  >Blessing of the Vah Shir
		    <option value="2505"
		  >Blessing of Vah Kerrath
		    <option value="843"
		  >Blinding Fear
		    <option value="133"
		  >Blinding Luminance
		    <option value="1614"
		  >Blinding Poison I
		    <option value="1621"
		  >Blinding Poison II
		    <option value="1622"
		  >Blinding Poison III
		    <option value="2506"
		  >Blinding Smog
		    <option value="854"
		  >Blinding Step
		    <option value="1384"
		  >Blizzard
		    <option value="509"
		  >Blizzard Blast
		    <option value="2004"
		  >Blood Bath
		    <option value="913"
		  >Blood Claw
		    <option value="1524"
		  >Bobbing Corpse
		    <option value="450"
		  >Boil Blood
		    <option value="948"
		  >Boiling Blood
		    <option value="67"
		  >Bolt of Flame
		    <option value="1603"
		  >Bolt of Karana
		    <option value="1488"
		  >Boltran's Agacerie
		    <option value="1487"
		  >Boltran`s Agacerie
		    <option value="682"
		  >Boltran`s Animation
		    <option value="455"
		  >Bond of Death
		    <option value="821"
		  >Bond of Sathir
		    <option value="634"
		  >Bonds of Force
		    <option value="1549"
		  >Bonds of Tunare
		    <option value="1829"
		  >Bone Grasp
		    <option value="1743"
		  >Bone Melt
		    <option value="1118"
		  >Bone Shatter
		    <option value="350"
		  >Bone Walk
		    <option value="1171"
		  >Boneshear
		    <option value="1450"
		  >Boon of Immolation
		    <option value="2306"
		  >Boon of Recovery
		    <option value="1476"
		  >Boon of the Clear Mind
		    <option value="1256"
		  >Boon of the Garou
		    <option value="2133"
		  >Boulder Smash
		    <option value="516"
		  >Bramblecoat
		    <option value="243"
		  >Bravery
		    <option value="2507"
		  >Breath of Corruption
		    <option value="1386"
		  >Breath of Karana
		    <option value="2117"
		  >Breath of Rag'Mozdezh
		    <option value="1382"
		  >Breath of Ro
		    <option value="2508"
		  >Breath of the Black
		    <option value="477"
		  >Breath of the Dead
		    <option value="2509"
		  >Breath of the Fallen
		    <option value="1760"
		  >Breath of the Sea
		    <option value="2232"
		  >Breath of the Shissar
		    <option value="1933"
		  >Breath of Tunare
		    <option value="692"
		  >Breeze
		    <option value="1935"
		  >Brell's Mountainous Barrier
		    <option value="32"
		  >Brilliance
		    <option value="2510"
		  >Brilliance of Ro
		    <option value="1463"
		  >Bristlebane's Bundle
		    <option value="1620"
		  >Brittle Haste I
		    <option value="1631"
		  >Brittle Haste II
		    <option value="1632"
		  >Brittle Haste III
		    <option value="1662"
		  >Brittle Haste IV
		    <option value="699"
		  >Brusco`s Boastful Bellow
		    <option value="1529"
		  >Brusco`s Bombastic Bellow
		    <option value="2511"
		  >Bubonian Rabies
		    <option value="1319"
		  >Bulwark of Faith
		    <option value="93"
		  >Burn
		    <option value="1610"
		  >Burning Acid
		    <option value="2005"
		  >Burning Pain
		    <option value="841"
		  >BurningTouch
		    <option value="840"
		  >BurningTouch2
		    <option value="326"
		  >Burnout
		    <option value="105"
		  >Burnout II
		    <option value="106"
		  >Burnout III
		    <option value="1254"
		  >Burnout IV
		    <option value="901"
		  >Burns Spell
		    <option value="993"
		  >Burrowing Scarab
		    <option value="91"
		  >Burst of Fire
		    <option value="92"
		  >Burst of Flame
		    <option value="2377"
		  >Burst of Flame
		    <option value="279"
		  >Burst of Strength
		    <option value="767"
		  >ByeZBye
		    <option value="1017"
		  >ByeZBye
		    <option value="2330"
		  >Cabilis Sending
		    <option value="494"
		  >Cackling Bones
		    <option value="1740"
		  >Cacophony
		    <option value="1449"
		  >Cadeau of Flame
		    <option value="197"
		  >Cajole Undead
		    <option value="182"
		  >Cajoling Whispers
		    <option value="2363"
		  >Calamity
		    <option value="990"
		  >Caleah's Aura
		    <option value="1522"
		  >Calefaction
		    <option value="866"
		  >Calilimony
		    <option value="2512"
		  >Call of Alarm
		    <option value="638"
		  >Call of Bones
		    <option value="2241"
		  >Call of Darkness
		    <option value="1244"
		  >Call of Earth
		    <option value="838"
		  >Call of Earth2
		    <option value="1245"
		  >Call of Fire
		    <option value="686"
		  >Call of Flame
		    <option value="1335"
		  >Call of Karana
		    <option value="2239"
		  >Call of Shadows
		    <option value="1243"
		  >Call of Sky
		    <option value="779"
		  >Call of the Earth
		    <option value="1553"
		  >Call of the Hero
		    <option value="1246"
		  >Call of the Predator
		    <option value="778"
		  >Call of the Storm
		    <option value="2346"
		  >Call of the Zero
		    <option value="1249"
		  >CallOfFireEffect
		    <option value="1247"
		  >CallOfSkyEffect
		    <option value="46"
		  >Calm
		    <option value="512"
		  >Calm Animal
		    <option value="1909"
		  >Calming Visage
		    <option value="246"
		  >Camouflage
		    <option value="2094"
		  >Camouflage
		    <option value="888"
		  >Can o` Whoop Ass
		    <option value="47"
		  >Cancel Magic
		    <option value="2078"
		  >Cannibalization
		    <option value="264"
		  >Cannibalize
		    <option value="748"
		  >Cannibalize II
		    <option value="1354"
		  >Cannibalize III
		    <option value="1117"
		  >Cannibalize IV
		    <option value="1541"
		  >Cantata of Replenishment
		    <option value="1230"
		  >Cantata of Soothing
		    <option value="1707"
		  >Captain Nalots Quickening
		    <option value="418"
		  >Careless Lightning
		    <option value="2415"
		  >Carnage
		    <option value="261"
		  >Cascade of Hail
		    <option value="452"
		  >Cascading Darkness
		    <option value="1543"
		  >Cassindra's Insipid Ditty
		    <option value="1072"
		  >Cassindra`s Chant of Clarity
		    <option value="718"
		  >Cassindra`s Chorus of Clarity
		    <option value="740"
		  >Cassindra`s Elegy
		    <option value="460"
		  >Cast Force
		    <option value="406"
		  >Cast Sight
		    <option value="2143"
		  >Caustic Mist
		    <option value="337"
		  >Cavorting Bones
		    <option value="544"
		  >Cazic Gate
		    <option value="563"
		  >Cazic Portal
		    <option value="786"
		  >Cazic Shield
		    <option value="959"
		  >Cazic Touch
		    <option value="2402"
		  >Cazic's Gift
		    <option value="1926"
		  >Cease
		    <option value="170"
		  >Celerity
		    <option value="1094"
		  >Celestial Aura
		    <option value="1068"
		  >Celestial Cleansing
		    <option value="1304"
		  >Celestial Elixir
		    <option value="1226"
		  >Celestial Healing
		    <option value="2069"
		  >Celestial Regeneration
		    <option value="1847"
		  >Celestial Remedy
		    <option value="1722"
		  >Celestial Tranquility
		    <option value="218"
		  >Center
		    <option value="1397"
		  >Cessation of Cor
		    <option value="832"
		  >Ceticious Cloud
		    <option value="2513"
		  >Chain Lightning
		    <option value="695"
		  >Chant of Battle
		    <option value="823"
		  >Chaos Breath
		    <option value="349"
		  >Chaos Flux
		    <option value="295"
		  >Chaotic Feedback
		    <option value="1442"
		  >Char
		    <option value="155"
		  >Charisma
		    <option value="299"
		  >Charm
		    <option value="259"
		  >Charm Animals
		    <option value="303"
		  >Chase the Moon
		    <option value="1396"
		  >Chill Bones
		    <option value="1150"
		  >Chill of Unlife
		    <option value="537"
		  >Chill Sight
		    <option value="1194"
		  >Chilling Embrace
		    <option value="1075"
		  >Chloroblast
		    <option value="144"
		  >Chloroplast
		    <option value="520"
		  >Choke
		    <option value="875"
		  >Choking
		    <option value="2016"
		  >Choking Spores
		    <option value="912"
		  >Choking Wind
		    <option value="698"
		  >Chords of Dissonance
		    <option value="1954"
		  >Chorus of Replenishment
		    <option value="2319"
		  >Chromatic Disruption
		    <option value="703"
		  >Cinda`s Charismatic Carillon
		    <option value="68"
		  >Cinder Bolt
		    <option value="1081"
		  >Cinder Jolt
		    <option value="551"
		  >Circle of Butcher
		    <option value="1222"
		  >Circle of Cobalt Scar
		    <option value="549"
		  >Circle of Commons
		    <option value="554"
		  >Circle of Feerrott
		    <option value="462"
		  >Circle of Force
		    <option value="1220"
		  >Circle of Great Divide
		    <option value="1216"
		  >Circle of Iceclad
		    <option value="548"
		  >Circle of Karana
		    <option value="552"
		  >Circle of Lavastorm
		    <option value="556"
		  >Circle of Misty
		    <option value="553"
		  >Circle of Ro
		    <option value="1864"
		  >Circle of Seasons
		    <option value="555"
		  >Circle of Steamfont
		    <option value="1334"
		  >Circle of Summer
		    <option value="1802"
		  >Circle of Surefall Glade
		    <option value="1299"
		  >Circle of the Combines
		    <option value="550"
		  >Circle of Toxxulia
		    <option value="1180"
		  >Circle of Wakening Lands
		    <option value="1333"
		  >Circle of Winter
		    <option value="538"
		  >Clarify Mana
		    <option value="173"
		  >Clarity
		    <option value="1475"
		  >Clarity II
		    <option value="2008"
		  >Clash of Will
		    <option value="2328"
		  >Claw of Bestial Fury
		    <option value="1987"
		  >Claw of Khati Sha
		    <option value="2326"
		  >Claw of the Beast
		    <option value="2325"
		  >Claw of the Hunter
		    <option value="2327"
		  >Claw of the Savage Lord
		    <option value="7"
		  >Cleanse
		    <option value="2071"
		  >Cleansing Light
		    <option value="343"
		  >Clinging Darkness
		    <option value="4"
		  >Cloak
		    <option value="1925"
		  >Cloak of the Akheva
		    <option value="987"
		  >Clockwork Poison
		    <option value="646"
		  >Cloud
		    <option value="2514"
		  >Cloud of Deadly Decay
		    <option value="1267"
		  >Cloud of Disempowerment
		    <option value="1265"
		  >Cloud of Fear
		    <option value="2515"
		  >Cloud of Poisonous Decay
		    <option value="1248"
		  >Cloud of Silence
		    <option value="1810"
		  >Cobalt Scar Gate
		    <option value="1207"
		  >Cobalt Scar Portal
		    <option value="797"
		  >Cockatrice Gaze
		    <option value="1781"
		  >Cog Boost
		    <option value="1039"
		  >Cohesion
		    <option value="2516"
		  >Coils of Temptation
		    <option value="338"
		  >Coldlight
		    <option value="1469"
		  >Collaboration
		    <option value="289"
		  >Color Flux
		    <option value="176"
		  >Color Shift
		    <option value="177"
		  >Color Skew
		    <option value="1478"
		  >Color Slant
		    <option value="327"
		  >Column of Fire
		    <option value="379"
		  >Column of Frost
		    <option value="461"
		  >Column of Lightning
		    <option value="1110"
		  >Combine Gate
		    <option value="1298"
		  >Combine Portal
		    <option value="216"
		  >Combust
		    <option value="2437"
		  >Combustion Mastery II
		    <option value="542"
		  >Common Gate
		    <option value="564"
		  >Common Portal
		    <option value="163"
		  >Companion Spirit
		    <option value="1077"
		  >Complete Heal
		    <option value="12"
		  >Complete Healing
		    <option value="1234"
		  >Composition of Ervaj
		    <option value="1559"
		  >Conceal Rapier
		    <option value="1561"
		  >Conceal Rapier
		    <option value="1563"
		  >Conceal Rapier
		    <option value="2177"
		  >Concentration
		    <option value="746"
		  >Concussion
		    <option value="2060"
		  >Condemnation of Nife
		    <option value="654"
		  >Conflagration
		    <option value="1797"
		  >Conglaciation of Bone
		    <option value="629"
		  >Conjuration: Air
		    <option value="626"
		  >Conjuration: Earth
		    <option value="628"
		  >Conjuration: Fire
		    <option value="627"
		  >Conjuration: Water
		    <option value="1555"
		  >Conjure Corpse
		    <option value="755"
		  >Contact Poison I
		    <option value="1635"
		  >Contact Poison II
		    <option value="1636"
		  >Contact Poison III
		    <option value="1637"
		  >Contact Poison IV
		    <option value="1140"
		  >Contraction
		    <option value="1515"
		  >Convergence
		    <option value="361"
		  >Convoke Shadow
		    <option value="54"
		  >Cornucopia
		    <option value="1195"
		  >Corpal Empathy
		    <option value="2406"
		  >Corpse Breath
		    <option value="2517"
		  >Corrupted Pus
		    <option value="95"
		  >Counteract Disease
		    <option value="94"
		  >Counteract Poison
		    <option value="201"
		  >Courage
		    <option value="1297"
		  >Covetous Subversion
		    <option value="2282"
		  >Cral Ligi Attack
		    <option value="2283"
		  >Cral Ligi Attack
		    <option value="98"
		  >Creeping Crud
		    <option value="635"
		  >Creeping Vision
		    <option value="1374"
		  >Cripple
		    <option value="1893"
		  >Crippling Claudication
		    <option value="2011"
		  >Crippling Snare
		    <option value="736"
		  >Crission`s Pixie Strike
		    <option value="2518"
		  >Cromastorm
		    <option value="2362"
		  >Crushing Fist of the Wind
		    <option value="438"
		  >Crystallize Mana
		    <option value="211"
		  >Cure Blindness
		    <option value="212"
		  >Cure Disease
		    <option value="202"
		  >Cure Poison
		    <option value="2263"
		  >Curse of Eternal Suffering
		    <option value="2291"
		  >Curse of Lord Wedazi
		    <option value="2254"
		  >Curse of Rhag`Zadune
		    <option value="2335"
		  >Curse of Sshraezha
		    <option value="1565"
		  >Curse of the Garou
		    <option value="2290"
		  >Curse of the Revenant
		    <option value="407"
		  >Curse of the Simple Mind
		    <option value="1714"
		  >Curse of the Spirits
		    <option value="2519"
		  >Curse of Turgur
		    <option value="2520"
		  >Curse of Walking Sleep
		    <option value="2253"
		  >Curse of Xerkizh
		    <option value="2435"
		  >Cursed Acceleration Mastery II
		    <option value="971"
		  >Customer Service Resurrect
		    <option value="2521"
		  >Cyclone
		    <option value="103"
		  >Dagger of Symbols
		    <option value="1719"
		  >Dance of the Blade
		    <option value="236"
		  >Dance of the Fireflies
		    <option value="88"
		  >Daring
		    <option value="356"
		  >Dark Empathy
		    <option value="636"
		  >Dark Pact
		    <option value="981"
		  >Dark Poison
		    <option value="2522"
		  >Darkening Death
		    <option value="915"
		  >Darkness
		    <option value="1759"
		  >Dawncall
		    <option value="960"
		  >Daze
		    <option value="961"
		  >Dazed
		    <option value="189"
		  >Dazzle
		    <option value="2075"
		  >Dead Aim
		    <option value="456"
		  >Dead Man Floating
		    <option value="1174"
		  >Dead Men Floating
		    <option value="351"
		  >Deadeye
		    <option value="970"
		  >Deadly Lifetap
		    <option value="982"
		  >Deadly Poison
		    <option value="1279"
		  >Deadly Velium Poison
		    <option value="1329"
		  >Death Pact
		    <option value="1242"
		  >Death Peace
		    <option value="1841"
		  >Death Roll
		    <option value="2523"
		  >Death Strike
		    <option value="2524"
		  >Deathfog IV
		    <option value="1828"
		  >Deathly Chants
		    <option value="2525"
		  >Deathly Facade
		    <option value="2323"
		  >Deathly Fever
		    <option value="2321"
		  >Deathly Fire
		    <option value="2320"
		  >Deathly Ice
		    <option value="2322"
		  >Deathly Spores
		    <option value="2221"
		  >Deathly Temptation
		    <option value="2526"
		  >Debilitating Death
		    <option value="1903"
		  >Decession
		    <option value="2324"
		  >Deep Spores
		    <option value="1395"
		  >Deflux
		    <option value="694"
		  >Defoliate
		    <option value="1412"
		  >Defoliation
		    <option value="151"
		  >Deftness
		    <option value="1891"
		  >Degeneration
		    <option value="1376"
		  >Deliriously Nimble
		    <option value="2527"
		  >Deluge
		    <option value="1480"
		  >Dementia
		    <option value="1798"
		  >Dementing Visions
		    <option value="1393"
		  >Demi Lich
		    <option value="1546"
		  >Denon`s Bereavement
		    <option value="737"
		  >Denon`s Desperate Dirge
		    <option value="725"
		  >Denon`s Disruptive Discord
		    <option value="731"
		  >Denon`s Dissension
		    <option value="1056"
		  >Deranged Wonderment
		    <option value="1997"
		  >Descending Might
		    <option value="1845"
		  >Desert Sending
		    <option value="1927"
		  >Desist
		    <option value="1916"
		  >Despair
		    <option value="2006"
		  >Despairing Darkness
		    <option value="918"
		  >Desperate Hope
		    <option value="2294"
		  >Desperation
		    <option value="1730"
		  >Destroy
		    <option value="1736"
		  >Devour Soul
		    <option value="815"
		  >Devourer Tongue
		    <option value="1401"
		  >Devouring Darkness
		    <option value="156"
		  >Dexterity
		    <option value="265"
		  >Dexterous Aura
		    <option value="393"
		  >Diamondskin
		    <option value="1489"
		  >Dictate
		    <option value="616"
		  >Dimensional Hole
		    <option value="324"
		  >Dimensional Pocket
		    <option value="1283"
		  >Diminution
		    <option value="1613"
		  >Diminution
		    <option value="2310"
		  >Diminutive Stature
		    <option value="668"
		  >Discordant Mind
		    <option value="2528"
		  >Discorded Terror
		    <option value="985"
		  >Disease
		    <option value="339"
		  >Disease Cloud
		    <option value="2091"
		  >Disease Touch
		    <option value="824"
		  >Diseased Cloud
		    <option value="280"
		  >Disempower
		    <option value="1871"
		  >Disinfecting Aura
		    <option value="1506"
		  >Disintegrate
		    <option value="1595"
		  >Disjunction
		    <option value="114"
		  >Dismiss Summoned
		    <option value="116"
		  >Dismiss Undead
		    <option value="2296"
		  >Disruptive Force
		    <option value="690"
		  >Distill Mana
		    <option value="1548"
		  >Distraction
		    <option value="206"
		  >Divine Aura
		    <option value="129"
		  >Divine Barrier
		    <option value="2184"
		  >Divine Endurance
		    <option value="1525"
		  >Divine Favor
		    <option value="1073"
		  >Divine Glory
		    <option value="1328"
		  >Divine Intervention
		    <option value="1301"
		  >Divine Light
		    <option value="688"
		  >Divine Might
		    <option value="811"
		  >Divine Might Effect
		    <option value="1235"
		  >Divine Purpose
		    <option value="2067"
		  >Divine Resurrection
		    <option value="1238"
		  >Divine Strength
		    <option value="2447"
		  >Divine Stun
		    <option value="1929"
		  >Divine Vigor
		    <option value="1602"
		  >Divine Wrath
		    <option value="760"
		  >Dizzy I
		    <option value="1650"
		  >Dizzy II
		    <option value="1651"
		  >Dizzy III
		    <option value="1664"
		  >Dizzy IV
		    <option value="519"
		  >Dizzying Wind
		    <option value="1280"
		  >Doljons Rage
		    <option value="195"
		  >Dominate Undead
		    <option value="2090"
		  >Dominating Gaze
		    <option value="2529"
		  >Dominion of Force
		    <option value="2374"
		  >Donlo's Dance Party
		    <option value="2530"
		  >Donlo's Dementia
		    <option value="451"
		  >Dooming Darkness
		    <option value="829"
		  >Dragon Charm
		    <option value="2096"
		  >Dragon Force
		    <option value="777"
		  >Dragon Roar
		    <option value="958"
		  >Dragon Roar
		    <option value="2040"
		  >Drain Blood
		    <option value="2024"
		  >Drain Blood2
		    <option value="2025"
		  >Drain Blood3
		    <option value="1993"
		  >Drain Mana
		    <option value="446"
		  >Drain Soul
		    <option value="523"
		  >Drain Spirit
		    <option value="2368"
		  >Drake Breath
		    <option value="2369"
		  >Drake Breath
		    <option value="2370"
		  >Drake Breath
		    <option value="2371"
		  >Drake Breath
		    <option value="847"
		  >DrakeBreath
		    <option value="1737"
		  >DrakeBreathBig
		    <option value="1419"
		  >Draught of Fire
		    <option value="1423"
		  >Draught of Ice
		    <option value="1425"
		  >Draught of Jiva
		    <option value="1314"
		  >Dread of Night
		    <option value="924"
		  >Dread Touch
		    <option value="660"
		  >Drifting Death
		    <option value="1815"
		  >Droga Gate
		    <option value="258"
		  >Drones of Doom
		    <option value="269"
		  >Drowsy
		    <option value="884"
		  >DryBoneFireBurst
		    <option value="862"
		  >Dulsehound
		    <option value="617"
		  >Dyn`s Dizzying Draught
		    <option value="1461"
		  >Dyzil's Deafening Decoy
		    <option value="1944"
		  >Eagle Eye
		    <option value="2121"
		  >Earth Elemental Form
		    <option value="2122"
		  >Earth Elemental Form
		    <option value="2123"
		  >Earth Elemental Form
		    <option value="2287"
		  >Earth Elemental Strike
		    <option value="2288"
		  >Earth Elemental Strike
		    <option value="1710"
		  >Earthcall
		    <option value="946"
		  >EarthElementalAttack
		    <option value="999"
		  >EarthElementalStrike
		    <option value="405"
		  >Earthquake
		    <option value="640"
		  >Ebbing Strength
		    <option value="1108"
		  >Echinacea Infusion
		    <option value="962"
		  >Efreeti Fire
		    <option value="1348"
		  >Egress
		    <option value="1264"
		  >Electric Blast
		    <option value="2531"
		  >Electrical Short
		    <option value="108"
		  >Elemental Armor
		    <option value="1952"
		  >Elemental Chorus
		    <option value="1185"
		  >Elemental Maelstrom
		    <option value="705"
		  >Elemental Rhythms
		    <option value="107"
		  >Elemental Shield
		    <option value="403"
		  >Elemental: Air
		    <option value="400"
		  >Elemental: Earth
		    <option value="402"
		  >Elemental: Fire
		    <option value="401"
		  >Elemental: Water
		    <option value="399"
		  >Elementaling: Air
		    <option value="396"
		  >Elementaling: Earth
		    <option value="398"
		  >Elementaling: Fire
		    <option value="397"
		  >Elementaling: Water
		    <option value="316"
		  >Elementalkin: Air
		    <option value="57"
		  >Elementalkin: Earth
		    <option value="315"
		  >Elementalkin: Fire
		    <option value="314"
		  >Elementalkin: Water
		    <option value="837"
		  >ElementalMasteryBlast
		    <option value="836"
		  >ElementalMasteryStrike
		    <option value="1899"
		  >Elnerick Entombment of Ice
		    <option value="2212"
		  >Elnerick's Electrical Rending
		    <option value="2302"
		  >Ember Strike
		    <option value="1761"
		  >Embrace of the Kelpmaiden
		    <option value="1405"
		  >Emissary of Thule
		    <option value="1672"
		  >Enchant Adamantite
		    <option value="1675"
		  >Enchant Brellium
		    <option value="1144"
		  >Enchant Clay
		    <option value="663"
		  >Enchant Electrum
		    <option value="664"
		  >Enchant Gold
		    <option value="1671"
		  >Enchant Mithril
		    <option value="665"
		  >Enchant Platinum
		    <option value="662"
		  >Enchant Silver
		    <option value="1674"
		  >Enchant Steel
		    <option value="1579"
		  >Enchant Velium
		    <option value="2532"
		  >Endurance
		    <option value="224"
		  >Endure Cold
		    <option value="225"
		  >Endure Disease
		    <option value="223"
		  >Endure Fire
		    <option value="227"
		  >Endure Magic
		    <option value="226"
		  >Endure Poison
		    <option value="85"
		  >Enduring Breath
		    <option value="1833"
		  >Energy Blight
		    <option value="2533"
		  >Energy Burst
		    <option value="1742"
		  >Energy Sap
		    <option value="467"
		  >Energy Storm
		    <option value="290"
		  >Enfeeblement
		    <option value="1842"
		  >Enfeebling Shock
		    <option value="1326"
		  >Enforced Reverence
		    <option value="1501"
		  >Engorging Roots
		    <option value="354"
		  >Engulfing Darkness
		    <option value="76"
		  >Engulfing Roots
		    <option value="2534"
		  >Enhancement Haste I
		    <option value="2535"
		  >Enhancement Haste II
		    <option value="2536"
		  >Enhancement Haste III
		    <option value="2304"
		  >Enlightening Barrier
		    <option value="1470"
		  >Enlightenment
		    <option value="2020"
		  >Enraging Blow
		    <option value="1411"
		  >Enslave Death
		    <option value="511"
		  >Ensnare
		    <option value="75"
		  >Ensnaring Roots
		    <option value="130"
		  >Enstill
		    <option value="2039"
		  >Entanglement
		    <option value="186"
		  >Enthrall
		    <option value="1203"
		  >Enticement of Flame
		    <option value="966"
		  >Entomb in Ice
		    <option value="187"
		  >Entrance
		    <option value="2243"
		  >Entrapment
		    <option value="2537"
		  >Entrapment of Dreams
		    <option value="1390"
		  >Entrapping Roots
		    <option value="2538"
		  >Enveloping Entropy
		    <option value="1835"
		  >Enveloping Fog
		    <option value="489"
		  >Enveloping Roots
		    <option value="435"
		  >Envenomed Bolt
		    <option value="433"
		  >Envenomed Breath
		    <option value="852"
		  >Envenomed Heal
		    <option value="1851"
		  >Epitaph of Life
		    <option value="2389"
		  >Erud Crossing Portal
		    <option value="2265"
		  >Ervaj's Lost Composition
		    <option value="1794"
		  >Essence Drain
		    <option value="1793"
		  >Essence Tap
		    <option value="1888"
		  >Eternities Torment
		    <option value="2269"
		  >Eugene's Memory Blur
		    <option value="1410"
		  >Evacuate
		    <option value="601"
		  >Evacuate: Fay
		    <option value="603"
		  >Evacuate: Nek
		    <option value="600"
		  >Evacuate: North
		    <option value="602"
		  >Evacuate: Ro
		    <option value="604"
		  >Evacuate: West
		    <option value="1143"
		  >Evenglow
		    <option value="55"
		  >Everfount
		    <option value="2210"
		  >Everlasting Breath
		    <option value="1311"
		  >Exile Summoned
		    <option value="1310"
		  >Exile Undead
		    <option value="2100"
		  >Exodus
		    <option value="1071"
		  >Expedience
		    <option value="659"
		  >Expel Summoned
		    <option value="657"
		  >Expel Undead
		    <option value="658"
		  >Expulse Summoned
		    <option value="232"
		  >Expulse Undead
		    <option value="2539"
		  >Extended Affliction I
		    <option value="2540"
		  >Extended Affliction II
		    <option value="2541"
		  >Extended Affliction III
		    <option value="2542"
		  >Extended Enhancement I
		    <option value="2543"
		  >Extended Enhancement II
		    <option value="2544"
		  >Extended Enhancement III
		    <option value="2545"
		  >Extended Range I
		    <option value="2546"
		  >Extended Range II
		    <option value="2547"
		  >Extended Range III
		    <option value="940"
		  >Extended Regeneration
		    <option value="416"
		  >Extinguish Fatigue
		    <option value="296"
		  >Eye of Confusion
		    <option value="1502"
		  >Eye of Tallon
		    <option value="322"
		  >Eye of Zomm
		    <option value="916"
		  >EyeLevitate
		    <option value="650"
		  >Eyes of the Cat
		    <option value="374"
		  >Fade
		    <option value="1941"
		  >Falcon Eye
		    <option value="1902"
		  >Familiar
		    <option value="1054"
		  >Fangol's Breath
		    <option value="1472"
		  >Fascination
		    <option value="939"
		  >Fatigue Drain
		    <option value="541"
		  >Fay Gate
		    <option value="561"
		  >Fay Portal
		    <option value="2136"
		  >FD Test
		    <option value="228"
		  >Fear
		    <option value="474"
		  >Fear2
		    <option value="1572"
		  >Feast of Blood
		    <option value="178"
		  >Feckless Might
		    <option value="758"
		  >Feeble Mind I
		    <option value="1645"
		  >Feeble Mind II
		    <option value="1646"
		  >Feeble Mind III
		    <option value="1658"
		  >Feeble Mind IV
		    <option value="977"
		  >Feeble Poison
		    <option value="2228"
		  >Feeblemind
		    <option value="190"
		  >Feedback
		    <option value="2396"
		  >Feerrott Exile
		    <option value="268"
		  >Feet like Cat
		    <option value="365"
		  >Feign Death
		    <option value="1762"
		  >Fellspine
		    <option value="138"
		  >Feral Spirit
		    <option value="1098"
		  >Festering Sores
		    <option value="1415"
		  >Fetter
		    <option value="1107"
		  >Field of Bone Port
		    <option value="2017"
		  >Fiery Curse
		    <option value="1152"
		  >Fiery Might
		    <option value="2548"
		  >Fiery Retribution
		    <option value="378"
		  >Fingers of Fire
		    <option value="432"
		  >Fire
		    <option value="2285"
		  >Fire Blast
		    <option value="476"
		  >Fire Bolt
		    <option value="2124"
		  >Fire Elemental Form
		    <option value="2125"
		  >Fire Elemental Form
		    <option value="2126"
		  >Fire Elemental Form
		    <option value="312"
		  >Fire Flux
		    <option value="457"
		  >Fire Spiral of Al'Kabor
		    <option value="986"
		  >FireBeetleSpit
		    <option value="1999"
		  >Fireblast
		    <option value="2207"
		  >Fireclaw
		    <option value="943"
		  >FireElementalAttack
		    <option value="870"
		  >FireElementalAttack2
		    <option value="904"
		  >FireElementalAura
		    <option value="253"
		  >Firefist
		    <option value="817"
		  >FireHornet
		    <option value="84"
		  >Firestorm
		    <option value="56"
		  >Firestrike
		    <option value="1901"
		  >Firetree's Familiar Augment
		    <option value="2211"
		  >Firetree's Familiar Enhancemnt
		    <option value="29"
		  >Firework1
		    <option value="118"
		  >Firework2
		    <option value="205"
		  >Firework3
		    <option value="213"
		  >Firework4
		    <option value="928"
		  >Firey Death
		    <option value="994"
		  >Fishnova
		    <option value="781"
		  >Fist of Air
		    <option value="782"
		  >Fist of Earth
		    <option value="780"
		  >Fist of Fire
		    <option value="1388"
		  >Fist of Karana
		    <option value="834"
		  >Fist of Mastery
		    <option value="784"
		  >Fist of Sentience
		    <option value="783"
		  >Fist of Water
		    <option value="1218"
		  >Fixation of Ro
		    <option value="121"
		  >Flame Arc
		    <option value="321"
		  >Flame Bolt
		    <option value="188"
		  >Flame Flux
		    <option value="1276"
		  >Flame Jet
		    <option value="238"
		  >Flame Lick
		    <option value="1236"
		  >Flame of Light
		    <option value="1751"
		  >Flame of the Efreeti
		    <option value="652"
		  >Flame Shock
		    <option value="2549"
		  >Flame Wind
		    <option value="1597"
		  >Flames of Ro
		    <option value="1567"
		  >Flamesong
		    <option value="808"
		  >FlameSongOfRo
		    <option value="2550"
		  >Flamewall
		    <option value="2551"
		  >Flaming Comet
		    <option value="1504"
		  >Flaming Sword of Xuzl
		    <option value="309"
		  >Flare
		    <option value="200"
		  >Flash of Light
		    <option value="270"
		  >Fleeting Fury
		    <option value="2308"
		  >Flesh Rend
		    <option value="1619"
		  >Flesh Rot I
		    <option value="1629"
		  >Flesh Rot II
		    <option value="1630"
		  >Flesh Rot III
		    <option value="2424"
		  >Fling
		    <option value="1083"
		  >Flowing Thought I
		    <option value="1084"
		  >Flowing Thought II
		    <option value="1085"
		  >Flowing Thought III
		    <option value="1086"
		  >Flowing Thought IV
		    <option value="1091"
		  >Flowing Thought IX
		    <option value="1087"
		  >Flowing Thought V
		    <option value="1088"
		  >Flowing Thought VI
		    <option value="1089"
		  >Flowing Thought VII
		    <option value="1090"
		  >Flowing Thought VIII
		    <option value="1092"
		  >Flowing Thought X
		    <option value="1051"
		  >Flurry
		    <option value="2131"
		  >Flurry of Pebbles
		    <option value="1886"
		  >Focus Death
		    <option value="1214"
		  >Focus of Spirit
		    <option value="2074"
		  >Focused Mind
		    <option value="1861"
		  >Foliage Shield
		    <option value="123"
		  >Force
		    <option value="1940"
		  >Force of Nature
		    <option value="21"
		  >Force Shock
		    <option value="459"
		  >Force Spiral of Al'Kabor
		    <option value="22"
		  >Force Strike
		    <option value="1494"
		  >Forlorn Deeds
		    <option value="1868"
		  >Form of the Bear
		    <option value="1213"
		  >Form of the Great Bear
		    <option value="426"
		  >Form of the Great Wolf
		    <option value="1344"
		  >Form of the Howler
		    <option value="1345"
		  >Form of the Hunter
		    <option value="1321"
		  >Fortitude
		    <option value="2552"
		  >Foul Pus
		    <option value="1142"
		  >Free
		    <option value="2065"
		  >Free
		    <option value="2066"
		  >Free
		    <option value="833"
		  >Freezing Breath
		    <option value="2553"
		  >Freezing Scourge
		    <option value="2083"
		  >Frenzied Burnout
		    <option value="165"
		  >Frenzied Spirit
		    <option value="503"
		  >Frenzied Strength
		    <option value="307"
		  >Frenzy
		    <option value="2021"
		  >Frenzying Blow
		    <option value="2157"
		  >FrenzyNova
		    <option value="2554"
		  >Frigid Dominion
		    <option value="1583"
		  >Froglok Poison
		    <option value="1387"
		  >Frost
		    <option value="53"
		  >Frost Bolt
		    <option value="849"
		  >Frost Breath
		    <option value="1724"
		  >Frost Breath
		    <option value="1049"
		  >Frost Port
		    <option value="274"
		  >Frost Rift
		    <option value="1609"
		  >Frost Shards
		    <option value="463"
		  >Frost Shock
		    <option value="381"
		  >Frost Spiral of Al'Kabor
		    <option value="655"
		  >Frost Storm
		    <option value="1749"
		  >Frost Storm
		    <option value="507"
		  >Frost Strike
		    <option value="955"
		  >FrostAOE
		    <option value="1727"
		  >Frostbite
		    <option value="2009"
		  >Frostcall
		    <option value="930"
		  >FrostElementalAura
		    <option value="1135"
		  >Frostreaver's Blessing
		    <option value="929"
		  >Frosty Death
		    <option value="1141"
		  >Frosty Death2
		    <option value="2555"
		  >Frozen Torrent
		    <option value="702"
		  >Fufil`s Curtailing Chant
		    <option value="2214"
		  >Funeral Pyre of Kelador
		    <option value="1989"
		  >Fungal Infusion
		    <option value="2185"
		  >Fungal Net
		    <option value="1588"
		  >Fungal Regrowth
		    <option value="1790"
		  >Fungal Regrowth
		    <option value="1988"
		  >Fungal Restoration
		    <option value="2262"
		  >Fungal Spore Cloud
		    <option value="2023"
		  >Fungal Vengeance
		    <option value="914"
		  >Fungus Spores
		    <option value="152"
		  >Furious Strength
		    <option value="558"
		  >Furor
		    <option value="2556"
		  >Furor of the Wild
		    <option value="325"
		  >Fury
		    <option value="1221"
		  >Fury of Air
		    <option value="2557"
		  >Fury of the Chosen
		    <option value="2331"
		  >Fury of the Vah Shir
		    <option value="2558"
		  >Fwexar's Rage
		    <option value="976"
		  >Gain Mana
		    <option value="437"
		  >Gale of Poison
		    <option value="2559"
		  >Gallenite's Bark of Fury
		    <option value="2456"
		  >Gallenite's Destruction
		    <option value="2560"
		  >Gallenite's Distraction
		    <option value="2561"
		  >Gallenite's Inverted Mishap
		    <option value="2457"
		  >Gallenite's Major Destruction
		    <option value="2455"
		  >Gallenite's Minor Destruction
		    <option value="1176"
		  >Gangrenous Touch of Zum`uul
		    <option value="2213"
		  >Garrison's Superior Sundering
		    <option value="1897"
		  >Garrisons Mighty Mana Shock
		    <option value="1149"
		  >Garzicor's Vengeance
		    <option value="2562"
		  >Gas Leak
		    <option value="194"
		  >Gasping Embrace
		    <option value="35"
		  >Gate
		    <option value="2082"
		  >Gather Mana
		    <option value="521"
		  >Gather Shadows
		    <option value="2563"
		  >Gathering of Souls
		    <option value="1792"
		  >Gathering of the Mind
		    <option value="527"
		  >Gaze
		    <option value="879"
		  >GelatRoot
		    <option value="919"
		  >Ghoul Drain
		    <option value="878"
		  >GhoulRoot
		    <option value="2372"
		  >Gift of A'err
		    <option value="1192"
		  >Gift of Brilliance
		    <option value="2564"
		  >Gift of Gebron
		    <option value="2565"
		  >Gift of Hate
		    <option value="1191"
		  >Gift of Insight
		    <option value="1190"
		  >Gift of Magic
		    <option value="1477"
		  >Gift of Pure Thought
		    <option value="1462"
		  >Gift of Xev
		    <option value="1339"
		  >Girdle of Karana
		    <option value="1914"
		  >Glamorous Visage
		    <option value="154"
		  >Glamour
		    <option value="1473"
		  >Glamour of Kintaz
		    <option value="1337"
		  >Glamour of Tunare
		    <option value="50"
		  >Glimpse
		    <option value="2139"
		  >Glowing Glyphs
		    <option value="1078"
		  >GM Summon Bone Chips
		    <option value="1080"
		  >GM Summon Rations
		    <option value="1079"
		  >GM Summon Water Flasks
		    <option value="785"
		  >GMGate
		    <option value="2160"
		  >Golem Coat
		    <option value="2156"
		  >Golem Smash
		    <option value="2158"
		  >Golem Smash2
		    <option value="816"
		  >Goop Poison
		    <option value="2132"
		  >Granite Crush
		    <option value="248"
		  >Grasping Roots
		    <option value="2152"
		  >Gravel Rain
		    <option value="867"
		  >Graveyard Dust
		    <option value="72"
		  >Gravity Flux
		    <option value="1784"
		  >Grease Injection
		    <option value="1808"
		  >Great Divide Gate
		    <option value="1205"
		  >Great Divide Portal
		    <option value="2178"
		  >Greater Concentration
		    <option value="633"
		  >Greater Conjuration: Air
		    <option value="630"
		  >Greater Conjuration: Earth
		    <option value="632"
		  >Greater Conjuration: Fire
		    <option value="631"
		  >Greater Conjuration: Water
		    <option value="2566"
		  >Greater Corrupted Pus
		    <option value="1905"
		  >Greater Familiar
		    <option value="2567"
		  >Greater Foul Pus
		    <option value="2348"
		  >Greater Grimling Leech
		    <option value="14"
		  >Greater Healing
		    <option value="2353"
		  >Greater Infusion
		    <option value="2356"
		  >Greater Mana Surge
		    <option value="2182"
		  >Greater Mystical Infusion
		    <option value="2359"
		  >Greater Rejuvination
		    <option value="2436"
		  >Greater Relentless Magic
		    <option value="2417"
		  >Greater Shadow Poison
		    <option value="65"
		  >Greater Shielding
		    <option value="965"
		  >Greater Spiroc Thunder
		    <option value="574"
		  >Greater Summoning: Air
		    <option value="571"
		  >Greater Summoning: Earth
		    <option value="573"
		  >Greater Summoning: Fire
		    <option value="572"
		  >Greater Summoning: Water
		    <option value="2366"
		  >Greater Thought Sieve
		    <option value="1460"
		  >Greater Vocaration: Air
		    <option value="1457"
		  >Greater Vocaration: Earth
		    <option value="1459"
		  >Greater Vocaration: Fire
		    <option value="1458"
		  >Greater Vocaration: Water
		    <option value="425"
		  >Greater Wolf Form
		    <option value="1148"
		  >Greenmist
		    <option value="345"
		  >Grim Aura
		    <option value="2165"
		  >Grimling Comet
		    <option value="2166"
		  >Grimling Death Chant
		    <option value="2349"
		  >Grimling Heal
		    <option value="2097"
		  >Grimling LT 30
		    <option value="2168"
		  >Grimling Mesmerize
		    <option value="2167"
		  >Grimling Possession
		    <option value="2350"
		  >Grimling Rot
		    <option value="1571"
		  >Grizznot SMASH
		    <option value="2279"
		  >Grol Baku Strike
		    <option value="2280"
		  >Grol Baku Strike
		    <option value="2292"
		  >Group Primal Essence
		    <option value="71"
		  >Group Resist Magic
		    <option value="1733"
		  >Grow
		    <option value="1867"
		  >Grow
		    <option value="2183"
		  >Grow
		    <option value="250"
		  >Growth
		    <option value="2076"
		  >Growth
		    <option value="1058"
		  >Grubs
		    <option value="2568"
		  >Gruszia's Grasp
		    <option value="17"
		  >Guard
		    <option value="388"
		  >Guardian
		    <option value="704"
		  >Guardian Rhythms
		    <option value="164"
		  >Guardian Spirit
		    <option value="798"
		  >Guide Alliance
		    <option value="1026"
		  >Guide Alliance
		    <option value="805"
		  >Guide Bolt
		    <option value="1023"
		  >Guide Bolt
		    <option value="799"
		  >Guide Cancel Magic
		    <option value="1018"
		  >Guide Cancel Magic
		    <option value="803"
		  >Guide Health
		    <option value="1021"
		  >Guide Health
		    <option value="804"
		  >Guide Invulnerability
		    <option value="1022"
		  >Guide Invulnerability
		    <option value="801"
		  >Guide Journey
		    <option value="1019"
		  >Guide Journey
		    <option value="806"
		  >Guide Memory Blur
		    <option value="1024"
		  >Guide Memory Blur
		    <option value="802"
		  >Guide Vision
		    <option value="1020"
		  >Guide Vision
		    <option value="766"
		  >GuideActing
		    <option value="1016"
		  >GuideActing
		    <option value="2569"
		  >Gyrosonic Disruption
		    <option value="2570"
		  >Hail of Ice
		    <option value="2571"
		  >Hail Storm
		    <option value="233"
		  >Halo of Light
		    <option value="670"
		  >Hammer of Requital
		    <option value="36"
		  >Hammer of Striking
		    <option value="222"
		  >Hammer of Wrath
		    <option value="2430"
		  >Hand of the Gods
		    <option value="1824"
		  >Hands of the Nurga
		    <option value="87"
		  >Harm Touch
		    <option value="2150"
		  >Harm Touch
		    <option value="906"
		  >Harm Touch NPC
		    <option value="2103"
		  >Harmful Touch
		    <option value="2266"
		  >Harmonize
		    <option value="2267"
		  >Harmonize
		    <option value="2268"
		  >Harmonize
		    <option value="249"
		  >Harmony
		    <option value="198"
		  >Harmshield
		    <option value="1870"
		  >Harnessing of Spirit
		    <option value="963"
		  >Harpy Voice
		    <option value="900"
		  >HarpyCharm
		    <option value="1526"
		  >Harvest
		    <option value="1178"
		  >Harvest Leaves
		    <option value="975"
		  >Haste
		    <option value="492"
		  >Haunting Corpse
		    <option value="1908"
		  >Haunting Visage
		    <option value="1937"
		  >Hawk Eye
		    <option value="2180"
		  >Hawk's Eye Tonic
		    <option value="292"
		  >Haze
		    <option value="11"
		  >Healing
		    <option value="1251"
		  >Healing Complete
		    <option value="830"
		  >Healing Touch
		    <option value="1934"
		  >Healing Wave of Prexus
		    <option value="2446"
		  >Healing Yay!
		    <option value="160"
		  >Health
		    <option value="366"
		  >Heart Flutter
		    <option value="2572"
		  >Heart of the Void
		    <option value="359"
		  >Heat Blood
		    <option value="2573"
		  >Heat Lightning
		    <option value="674"
		  >Heat Sight
		    <option value="1960"
		  >Herikol's Soothing
		    <option value="1318"
		  >Heroic Bond
		    <option value="1320"
		  >Heroic Bond
		    <option value="1315"
		  >Heroism
		    <option value="1137"
		  >Hide in a can
		    <option value="2278"
		  >Hinderance of the Vas Ren
		    <option value="10"
		  >Holy Armor
		    <option value="122"
		  >Holy Might
		    <option value="1739"
		  >Holy Shock
		    <option value="2111"
		  >Holy Steed
		    <option value="2397"
		  >Horrific Force
		    <option value="2405"
		  >Horrifying Apparition
		    <option value="1913"
		  >Horrifying Visage
		    <option value="2229"
		  >Horror
		    <option value="859"
		  >HpToMana
		    <option value="1096"
		  >Hsagra's Wrath
		    <option value="1134"
		  >Hug
		    <option value="368"
		  >Hungry Earth
		    <option value="6"
		  >Hymn of Restoration
		    <option value="28"
		  >Ice
		    <option value="1731"
		  >Ice breath
		    <option value="727"
		  >Ice Comet
		    <option value="1729"
		  >Ice Rend
		    <option value="653"
		  >Ice Shock
		    <option value="1208"
		  >Ice Spear of Solist
		    <option value="1368"
		  >Ice Strike
		    <option value="885"
		  >IceBoneFrostBurst
		    <option value="1199"
		  >Iceclad Gate
		    <option value="1200"
		  >Iceclad Portal
		    <option value="376"
		  >Icestrike
		    <option value="2574"
		  >Icicle Aura
		    <option value="1836"
		  >Icy Claws
		    <option value="304"
		  >Identify
		    <option value="90"
		  >Ignite
		    <option value="5"
		  >Ignite Blood
		    <option value="557"
		  >Ignite Bones
		    <option value="1756"
		  >Ikatiar's Revenge
		    <option value="2230"
		  >Illumination
		    <option value="595"
		  >Illusion: Air Elemental
		    <option value="584"
		  >Illusion: Barbarian
		    <option value="588"
		  >Illusion: Dark Elf
		    <option value="594"
		  >Illusion: Drybone
		    <option value="589"
		  >Illusion: Dwarf
		    <option value="582"
		  >Illusion: Earth Elemental
		    <option value="585"
		  >Illusion: Erudite
		    <option value="1513"
		  >Illusion: Female
		    <option value="596"
		  >Illusion: Fire Elemental
		    <option value="1512"
		  >Illusion: Gender
		    <option value="593"
		  >Illusion: Gnome
		    <option value="581"
		  >Illusion: Half-Elf
		    <option value="592"
		  >Illusion: Halfling
		    <option value="587"
		  >Illusion: High Elf
		    <option value="580"
		  >Illusion: Human
		    <option value="242"
		  >Illusion: Iksar
		    <option value="1910"
		  >Illusion: Imp
		    <option value="1514"
		  >Illusion: Male
		    <option value="591"
		  >Illusion: Ogre
		    <option value="579"
		  >Illusion: Skeleton
		    <option value="598"
		  >Illusion: Spirit Wolf
		    <option value="599"
		  >Illusion: Tree
		    <option value="590"
		  >Illusion: Troll
		    <option value="2155"
		  >Illusion: Vah Shir
		    <option value="597"
		  >Illusion: Water Elemental
		    <option value="583"
		  >Illusion: Werewolf
		    <option value="586"
		  >Illusion: Wood Elf
		    <option value="1667"
		  >Imbue Amber
		    <option value="1676"
		  >Imbue Black Pearl
		    <option value="1679"
		  >Imbue Black Sapphire
		    <option value="1677"
		  >Imbue Diamond
		    <option value="1670"
		  >Imbue Emerald
		    <option value="1681"
		  >Imbue Fire Opal
		    <option value="1666"
		  >Imbue Ivory
		    <option value="1673"
		  >Imbue Jade
		    <option value="1580"
		  >Imbue Opal
		    <option value="1680"
		  >Imbue Peridot
		    <option value="1582"
		  >Imbue Plains Pebble
		    <option value="1678"
		  >Imbue Rose Quartz
		    <option value="1669"
		  >Imbue Ruby
		    <option value="1668"
		  >Imbue Sapphire
		    <option value="1581"
		  >Imbue Topaz
		    <option value="131"
		  >Immobilize
		    <option value="77"
		  >Immolate
		    <option value="831"
		  >Immolating Breath
		    <option value="1278"
		  >Immolating Breath II
		    <option value="1136"
		  >Immunity
		    <option value="357"
		  >Impart Strength
		    <option value="1819"
		  >Imperceptibility
		    <option value="1104"
		  >Impotent Rage
		    <option value="2575"
		  >Improved Damage I
		    <option value="2576"
		  >Improved Damage II
		    <option value="2577"
		  >Improved Damage III
		    <option value="2087"
		  >Improved Familiar
		    <option value="2578"
		  >Improved Healing I
		    <option value="2579"
		  >Improved Healing II
		    <option value="2580"
		  >Improved Healing III
		    <option value="1193"
		  >Improved Invis to Undead
		    <option value="1188"
		  >Improved Invisibility
		    <option value="1217"
		  >Improved Superior Camouflage
		    <option value="2115"
		  >In Irons
		    <option value="162"
		  >Incapacitate
		    <option value="1796"
		  >Incinerate Bones
		    <option value="2581"
		  >Incinerating Spirit
		    <option value="364"
		  >Infectious Cloud
		    <option value="992"
		  >Infectious Spores
		    <option value="1432"
		  >Inferno of Al'Kabor
		    <option value="478"
		  >Inferno Shield
		    <option value="464"
		  >Inferno Shock
		    <option value="1516"
		  >Infusion
		    <option value="2352"
		  >Infusion
		    <option value="2434"
		  >Initiate's Protection Accel
		    <option value="2432"
		  >Initiate's Wounding Accel
		    <option value="759"
		  >Injected Poison I
		    <option value="1647"
		  >Injected Poison II
		    <option value="1648"
		  >Injected Poison III
		    <option value="1649"
		  >Injected Poison IV
		    <option value="1665"
		  >Injected Poison V
		    <option value="872"
		  >Ink jet
		    <option value="266"
		  >Inner Fire
		    <option value="2582"
		  >Inno Bag Summon1
		    <option value="2583"
		  >Inno Bag Summon2
		    <option value="2584"
		  >Inno Bag Summon3
		    <option value="2585"
		  >Inno Bag Summon4
		    <option value="995"
		  >InnoruuksPresence
		    <option value="996"
		  >InnoruuksWrath
		    <option value="2586"
		  >InnoTest1
		    <option value="2587"
		  >InnoTest2
		    <option value="1355"
		  >Insidious Decay
		    <option value="524"
		  >Insidious Fever
		    <option value="525"
		  >Insidious Malady
		    <option value="1890"
		  >Insidious Retrogression
		    <option value="174"
		  >Insight
		    <option value="179"
		  >Insipid Weakness
		    <option value="125"
		  >Inspire Fear
		    <option value="471"
		  >Inspire Fear2
		    <option value="1928"
		  >Instrument of Nife
		    <option value="1906"
		  >Intellectual Advancement
		    <option value="1907"
		  >Intellectual Superiority
		    <option value="448"
		  >Intensify Death
		    <option value="1418"
		  >Invert Gravity
		    <option value="221"
		  >Invigor
		    <option value="1607"
		  >Invigorate
		    <option value="2295"
		  >Invigoration of Rytan
		    <option value="41"
		  >Invisibility
		    <option value="1780"
		  >Invisibility Cloak
		    <option value="1202"
		  >Invisibility to Undead
		    <option value="871"
		  >Invisibility versus Animal
		    <option value="254"
		  >Invisibility versus Animals
		    <option value="234"
		  >Invisibility versus Undead
		    <option value="2068"
		  >Invisibility versus Undead
		    <option value="442"
		  >Invoke Death
		    <option value="126"
		  >Invoke Fear
		    <option value="472"
		  >Invoke Fear2
		    <option value="251"
		  >Invoke Lightning
		    <option value="493"
		  >Invoke Shadow
		    <option value="2175"
		  >Itraer Vius Touch
		    <option value="1055"
		  >Ive Got Candy
		    <option value="717"
		  >Jaxan`s Jig o` Vigor
		    <option value="2588"
		  >Jaydrox Rocks
		    <option value="2450"
		  >Jedah's Conservation
		    <option value="2451"
		  >Jedah's Greater Conservation
		    <option value="2449"
		  >Jedah's Lesser Conservation
		    <option value="1523"
		  >Jolt
		    <option value="1942"
		  >Jolting Blades
		    <option value="2205"
		  >JoltingBladesEffect
		    <option value="2462"
		  >Jolum's Abatement
		    <option value="2463"
		  >Jolum's Major Abatement
		    <option value="2461"
		  >Jolum's Minor Abatement
		    <option value="1544"
		  >Jonthan's Inspiration
		    <option value="744"
		  >Jonthan's Provocation
		    <option value="729"
		  >Jonthan's Whistling Warsong
		    <option value="2044"
		  >Journey: Combine
		    <option value="2041"
		  >Journey: Fay
		    <option value="2264"
		  >Journey: Luclin
		    <option value="2043"
		  >Journey: North
		    <option value="2042"
		  >Journey: Tox
		    <option value="1844"
		  >Journey: Velious
		    <option value="856"
		  >JourneymanBoots
		    <option value="1853"
		  >Judgement
		    <option value="1575"
		  >Judgement of Ice
		    <option value="676"
		  >Juli`s Animation
		    <option value="1435"
		  >Jyll's Static Pulse
		    <option value="1437"
		  >Jyll's Wave of Heat
		    <option value="1436"
		  >Jyll's Zephyr of Ice
		    <option value="2412"
		  >Kaggy's Curse
		    <option value="1825"
		  >Kambooz's Touch
		    <option value="1949"
		  >Katta's Song of Sword Dancing
		    <option value="1531"
		  >Kazumi's Note of Preservation
		    <option value="719"
		  >Kelin`s Lucid Lullaby
		    <option value="723"
		  >Kelin`s Lugubrious Lament
		    <option value="1958"
		  >Keshuval's Rejuvination
		    <option value="2589"
		  >Khalshazar's Deceit
		    <option value="1875"
		  >Khura's Focusing
		    <option value="677"
		  >Kilan`s Animation
		    <option value="1116"
		  >Kilva's Skin of Flame
		    <option value="685"
		  >Kintaz`s Animation
		    <option value="2137"
		  >Kneel Test
		    <option value="1172"
		  >Knight's Blessing
		    <option value="881"
		  >Knockback
		    <option value="1915"
		  >Koadic's Endless Intellect
		    <option value="2299"
		  >Koadic's Expansive Mind
		    <option value="2307"
		  >Koadic's Heightened Focus
		    <option value="1795"
		  >Kylie's Venom
		    <option value="301"
		  >Languid Pace
		    <option value="1497"
		  >Largarn's Lamentation
		    <option value="1533"
		  >Largo`s Absonant Binding
		    <option value="700"
		  >Largo`s Melodic Binding
		    <option value="69"
		  >Lava Bolt
		    <option value="848"
		  >Lava Breath
		    <option value="1723"
		  >Lava Breath
		    <option value="846"
		  >Lava Breath - Test
		    <option value="468"
		  >Lava Storm
		    <option value="1166"
		  >Lawrence's Uberity
		    <option value="86"
		  >Lay on Hands
		    <option value="1291"
		  >Leach
		    <option value="386"
		  >Leatherskin
		    <option value="490"
		  >Leering Corpse
		    <option value="1509"
		  >Legacy of Spike
		    <option value="1343"
		  >Legacy of Thorn
		    <option value="625"
		  >Lesser Conjuration: Air
		    <option value="622"
		  >Lesser Conjuration: Earth
		    <option value="624"
		  >Lesser Conjuration: Fire
		    <option value="623"
		  >Lesser Conjuration: Water
		    <option value="2441"
		  >Lesser Evacuate
		    <option value="1900"
		  >Lesser Familiar
		    <option value="2347"
		  >Lesser Grimling Leech
		    <option value="2351"
		  >Lesser Infusion
		    <option value="2354"
		  >Lesser Mana Surge
		    <option value="2357"
		  >Lesser Rejuvination
		    <option value="2416"
		  >Lesser Shadow Poison
		    <option value="245"
		  >Lesser Shielding
		    <option value="2440"
		  >Lesser Succor
		    <option value="2448"
		  >Lesser Summon Corpse
		    <option value="498"
		  >Lesser Summoning: Air
		    <option value="495"
		  >Lesser Summoning: Earth
		    <option value="497"
		  >Lesser Summoning: Fire
		    <option value="496"
		  >Lesser Summoning: Water
		    <option value="1100"
		  >Lethargy
		    <option value="1408"
		  >Levant
		    <option value="260"
		  >Levitate
		    <option value="2590"
		  >Levitate Test
		    <option value="947"
		  >Levitation
		    <option value="2223"
		  >Levitation
		    <option value="639"
		  >Lich
		    <option value="2591"
		  >Life Bind
		    <option value="2095"
		  >Life Curse
		    <option value="908"
		  >Life Drain
		    <option value="687"
		  >Life Leech
		    <option value="2084"
		  >Lifeburn
		    <option value="444"
		  >Lifedraw
		    <option value="2592"
		  >Lifeshock
		    <option value="501"
		  >Lifespike
		    <option value="340"
		  >Lifetap
		    <option value="1747"
		  >LifetapEffectNormal
		    <option value="1748"
		  >LifetapEffectSK
		    <option value="16"
		  >Light Healing
		    <option value="2174"
		  >Lightcrawler Flash
		    <option value="419"
		  >Lightning Blast
		    <option value="37"
		  >Lightning Bolt
		    <option value="827"
		  >Lightning Breath
		    <option value="1175"
		  >Lightning Call
		    <option value="465"
		  >Lightning Shock
		    <option value="973"
		  >Lightning Spire
		    <option value="466"
		  >Lightning Storm
		    <option value="417"
		  >Lightning Strike
		    <option value="2003"
		  >Lightning Surge
		    <option value="2593"
		  >Liquid Hydrokill
		    <option value="761"
		  >Liquid Silver I
		    <option value="1652"
		  >Liquid Silver II
		    <option value="1653"
		  >Liquid Silver III
		    <option value="161"
		  >Listless Power
		    <option value="341"
		  >Locate Corpse
		    <option value="2313"
		  >Lotus Spines
		    <option value="762"
		  >Lower Resists I
		    <option value="1654"
		  >Lower Resists II
		    <option value="1655"
		  >Lower Resists III
		    <option value="1659"
		  >Lower Resists IV
		    <option value="812"
		  >LowerElement
		    <option value="207"
		  >Lull
		    <option value="239"
		  >Lull Animal
		    <option value="2309"
		  >Lunar Whispers
		    <option value="1420"
		  >Lure of Flame
		    <option value="1424"
		  >Lure of Frost
		    <option value="1551"
		  >Lure of Ice
		    <option value="1422"
		  >Lure of Lightning
		    <option value="2420"
		  >Lure of Shadows
		    <option value="2594"
		  >Lure of the Storm
		    <option value="902"
		  >Lycanthropy
		    <option value="732"
		  >Lyssa`s Cataloging Libretto
		    <option value="715"
		  >Lyssa`s Locating Lyric
		    <option value="716"
		  >Lyssa`s Solidarity of Vision
		    <option value="730"
		  >Lyssa`s Veracious Concord
		    <option value="2148"
		  >Madness
		    <option value="1177"
		  >Maelstrom of Electricity
		    <option value="792"
		  >Magi Bolt
		    <option value="1032"
		  >Magi Bolt
		    <option value="795"
		  >Magi Circle
		    <option value="1035"
		  >Magi Circle
		    <option value="794"
		  >Magi Curse
		    <option value="1034"
		  >Magi Curse
		    <option value="793"
		  >Magi Strike
		    <option value="1033"
		  >Magi Strike
		    <option value="1946"
		  >Magical Monologue
		    <option value="577"
		  >Magnify
		    <option value="64"
		  >Major Shielding
		    <option value="1554"
		  >Mala
		    <option value="1106"
		  >Malevolent Grasp
		    <option value="441"
		  >Malignant Dead
		    <option value="109"
		  >Malise
		    <option value="110"
		  >Malisement
		    <option value="1360"
		  >Malo
		    <option value="111"
		  >Malosi
		    <option value="1359"
		  >Malosini
		    <option value="2595"
		  >Mana Beam
		    <option value="2438"
		  >Mana Conservation Mastery II
		    <option value="1612"
		  >Mana Conversion
		    <option value="920"
		  >Mana Drain
		    <option value="1147"
		  >Mana Flare
		    <option value="2596"
		  >Mana Preservation I
		    <option value="2597"
		  >Mana Preservation II
		    <option value="2598"
		  >Mana Preservation III
		    <option value="2072"
		  >Mana Regeneration
		    <option value="2599"
		  >Mana Shield
		    <option value="2240"
		  >Mana Shroud
		    <option value="73"
		  >Mana Sieve
		    <option value="957"
		  >Mana Sink
		    <option value="2355"
		  >Mana Surge
		    <option value="2600"
		  >Manabolt
		    <option value="2080"
		  >Manaburn
		    <option value="917"
		  >ManaConvert
		    <option value="1510"
		  >Manasink
		    <option value="1391"
		  >Manaskin
		    <option value="1447"
		  >Manastorm
		    <option value="2601"
		  >Manastorm
		    <option value="860"
		  >ManaToHp
		    <option value="1375"
		  >Maniacal Strength
		    <option value="1718"
		  >Manifest Elements
		    <option value="1259"
		  >Manticore Poison
		    <option value="2390"
		  >Marauder's Mire Portal
		    <option value="2336"
		  >Marauder's Wrath
		    <option value="2422"
		  >Mark of Darkness
		    <option value="1330"
		  >Mark of Karn
		    <option value="1852"
		  >Mark of Retribution
		    <option value="2602"
		  >Mark of the Plague Lords
		    <option value="1943"
		  >Mark of the Predator
		    <option value="610"
		  >Markar's Clash
		    <option value="1417"
		  >Markar's Discord
		    <option value="1521"
		  >Markar's Relocation
		    <option value="2300"
		  >Marvelous Visions
		    <option value="2222"
		  >Marzin's Mark
		    <option value="1347"
		  >Mask of the Hunter
		    <option value="2216"
		  >Mask of the Stalker
		    <option value="2142"
		  >Mass Insanity
		    <option value="1735"
		  >Mastodon Stomp
		    <option value="697"
		  >McVaxius` Berserker Crescendo
		    <option value="1542"
		  >McVaxius` Rousing Rondo
		    <option value="2603"
		  >Mechinetic Dislocation I
		    <option value="2604"
		  >Mechinetic Dislocation II
		    <option value="2605"
		  >Mechinetic Dislocation III
		    <option value="2431"
		  >Medic's Acceleration
		    <option value="734"
		  >Melanie`s Mellifluous Motion
		    <option value="1590"
		  >Melodious Befuddlement
		    <option value="1231"
		  >Melody of Ervaj
		    <option value="300"
		  >Memory Blur
		    <option value="1496"
		  >Memory Flux
		    <option value="352"
		  >Mend Bones
		    <option value="1921"
		  >Mental Corruption
		    <option value="2048"
		  >MentalCorruptionEffect
		    <option value="306"
		  >Mesmerization
		    <option value="291"
		  >Mesmerize
		    <option value="828"
		  >Mesmerizing Breath
		    <option value="2606"
		  >Metamorphosis65
		    <option value="2607"
		  >Meteor Storm
		    <option value="1732"
		  >Mind Cloud
		    <option value="775"
		  >Mind Grip
		    <option value="191"
		  >Mind Wipe
		    <option value="1894"
		  >Mind Wrack
		    <option value="2130"
		  >Miner Plague
		    <option value="1826"
		  >Mines of Nurga
		    <option value="1403"
		  >Minion of Shadows
		    <option value="1898"
		  >Minor  Familiar
		    <option value="621"
		  >Minor Conjuration: Air
		    <option value="618"
		  >Minor Conjuration: Earth
		    <option value="620"
		  >Minor Conjuration: Fire
		    <option value="619"
		  >Minor Conjuration: Water
		    <option value="199"
		  >Minor Healing
		    <option value="286"
		  >Minor Illusion
		    <option value="287"
		  >Minor Shielding
		    <option value="395"
		  >Minor Summoning: Air
		    <option value="334"
		  >Minor Summoning: Earth
		    <option value="394"
		  >Minor Summoning: Fire
		    <option value="335"
		  >Minor Summoning: Water
		    <option value="294"
		  >Mircyl's Animation
		    <option value="1138"
		  >Mischief Castle Gate
		    <option value="1139"
		  >Mischief Entry Gate
		    <option value="645"
		  >Mist
		    <option value="1053"
		  >Mistmoore Charm
		    <option value="753"
		  >Mistwalker
		    <option value="1285"
		  >Modulating Rod
		    <option value="1284"
		  >Modulation
		    <option value="1725"
		  >Molten Breath
		    <option value="2061"
		  >Molten Fist
		    <option value="868"
		  >Monkey stun
		    <option value="2608"
		  >Monsoons
		    <option value="1182"
		  >Monster Summoning I
		    <option value="1184"
		  >Monster Summoning II
		    <option value="1186"
		  >Monster Summoning III
		    <option value="2206"
		  >Moonfire
		    <option value="2609"
		  >Mooto's Raging Bellow
		    <option value="910"
		  >Mort Drowsy
		    <option value="1378"
		  >Mortal Deftness
		    <option value="818"
		  >Mosquito Bite
		    <option value="934"
		  >Mud
		    <option value="756"
		  >Muscle Lock I
		    <option value="1638"
		  >Muscle Lock II
		    <option value="1639"
		  >Muscle Lock III
		    <option value="1657"
		  >Muscle Lock IV
		    <option value="1103"
		  >Muscle Spasm
		    <option value="1467"
		  >Muzzle of Mardu
		    <option value="2179"
		  >Mystical Aptitude
		    <option value="2181"
		  >Mystical Infusion
		    <option value="2047"
		  >Mystical Transvergance
		    <option value="2459"
		  >Naki's Charm of Pernicity
		    <option value="2460"
		  >Naki's Greater Pernicity
		    <option value="2458"
		  >Naki's Lesser Pernicity
		    <option value="1556"
		  >Naltron's Mark
		    <option value="1257"
		  >Nature Walkers Behest
		    <option value="1832"
		  >Nature's Melody
		    <option value="1939"
		  >Nature's Precision
		    <option value="1865"
		  >Nature's Recovery
		    <option value="1076"
		  >Nature's Touch
		    <option value="1594"
		  >Nature's Wrath
		    <option value="1787"
		  >Natures Holy Wrath
		    <option value="1341"
		  >Natureskin
		    <option value="2318"
		  >Needlite Fever
		    <option value="543"
		  >Nek Gate
		    <option value="562"
		  >Nek Portal
		    <option value="1741"
		  >Net
		    <option value="1557"
		  >Neutralize Magic
		    <option value="2274"
		  >Nexus Gate
		    <option value="2273"
		  >Nexus Portal
		    <option value="1170"
		  >Nildaar's Storm of Locusts
		    <option value="1534"
		  >Nillipus` March of the Wee
		    <option value="159"
		  >Nimble
		    <option value="1545"
		  >Niv`s Harmonic
		    <option value="743"
		  >Niv`s Melody of Preservation
		    <option value="540"
		  >North Gate
		    <option value="560"
		  >North Portal
		    <option value="1128"
		  >NPCAegolism
		    <option value="1045"
		  >Null Aura
		    <option value="48"
		  >Nullify Magic
		    <option value="2364"
		  >Nulling Void
		    <option value="346"
		  >Numb the Dead
		    <option value="373"
		  >Numbing Cold
		    <option value="1896"
		  >O'keils Embers
		    <option value="1201"
		  >O'Keils Flickering Flame
		    <option value="377"
		  >O'Keils Radiation
		    <option value="647"
		  >Obscure
		    <option value="942"
		  >Obsidian Shatter
		    <option value="1233"
		  >Occlusion of Sound
		    <option value="2610"
		  >Oil Spray
		    <option value="1970"
		  >Omakin's Alacrity
		    <option value="1591"
		  >One Hundred Blows
		    <option value="1769"
		  >Open Black Box
		    <option value="1767"
		  >Open Blue Box
		    <option value="1773"
		  >Open Gold Box
		    <option value="1768"
		  >Open Green Box
		    <option value="1775"
		  >Open Grey Box
		    <option value="1779"
		  >Open Mithril Box
		    <option value="1778"
		  >Open Opaque Box
		    <option value="1774"
		  >Open Pearlescent Box
		    <option value="1770"
		  >Open Purple Box
		    <option value="1766"
		  >Open Red Box
		    <option value="1777"
		  >Open Shimmering Box
		    <option value="1772"
		  >Open Silver Box
		    <option value="1776"
		  >Open Striped Box
		    <option value="1133"
		  >Open Testing slot
		    <option value="1771"
		  >Open White Box
		    <option value="2611"
		  >Orxkras Bane
		    <option value="2339"
		  >Overfiend's Fury
		    <option value="1006"
		  >Overthere
		    <option value="1483"
		  >Overwhelming Splendor
		    <option value="2612"
		  >Overwhelming Stench
		    <option value="44"
		  >Pacify
		    <option value="137"
		  >Pack Chloroplast
		    <option value="136"
		  >Pack Regeneration
		    <option value="168"
		  >Pack Spirit
		    <option value="689"
		  >Pact of Shadow
		    <option value="2367"
		  >Pain and Suffering
		    <option value="933"
		  >Panic
		    <option value="240"
		  >Panic Animal
		    <option value="58"
		  >Panic the Dead
		    <option value="2022"
		  >Paralyzing Curse
		    <option value="132"
		  >Paralyzing Earth
		    <option value="1615"
		  >Paralyzing Poison I
		    <option value="1623"
		  >Paralyzing Poison II
		    <option value="1624"
		  >Paralyzing Poison III
		    <option value="1155"
		  >Pathing Fear
		    <option value="2226"
		  >Pebble Rain
		    <option value="284"
		  >Pendril's Animation
		    <option value="1994"
		  >Penetrating Roots
		    <option value="1604"
		  >Persuade
		    <option value="2439"
		  >Petrification Mastery
		    <option value="1837"
		  >Petrifying Fear
		    <option value="2208"
		  >Phantasmal Armor
		    <option value="1448"
		  >Phantom Armor
		    <option value="80"
		  >Phantom Chain
		    <option value="332"
		  >Phantom Leather
		    <option value="81"
		  >Phantom Plate
		    <option value="953"
		  >Phobocancel
		    <option value="24"
		  >Pillage Enchantment
		    <option value="385"
		  >Pillar of Fire
		    <option value="1426"
		  >Pillar of Flame
		    <option value="1428"
		  >Pillar of Frost
		    <option value="1427"
		  >Pillar of Lightning
		    <option value="31"
		  >Plague
		    <option value="1872"
		  >Plague of Insects
		    <option value="886"
		  >PlagueRatDisease
		    <option value="897"
		  >PlagueRatDiseaseLesser
		    <option value="1414"
		  >Plainsight
		    <option value="26"
		  >Pogonip
		    <option value="979"
		  >Poison
		    <option value="1616"
		  >Poison Animal I
		    <option value="1625"
		  >Poison Animal II
		    <option value="1626"
		  >Poison Animal III
		    <option value="347"
		  >Poison Bolt
		    <option value="844"
		  >Poison Breath
		    <option value="436"
		  >Poison Storm
		    <option value="1617"
		  >Poison Summoned I
		    <option value="1627"
		  >Poison Summoned II
		    <option value="1628"
		  >Poison Summoned III
		    <option value="2092"
		  >Poison Touch
		    <option value="1834"
		  >Poisoned Web
		    <option value="1154"
		  >Poisonous Chill
		    <option value="2413"
		  >Polymorphic Mayhem
		    <option value="1095"
		  >Porlos' Fury
		    <option value="2059"
		  >Portal Effect
		    <option value="2613"
		  >Portal to Butcher
		    <option value="2614"
		  >Portal to Butcher
		    <option value="2615"
		  >Portal to Erud's Crossing
		    <option value="2616"
		  >Portal to Erudin
		    <option value="2617"
		  >Portal to Firiona Vie
		    <option value="2618"
		  >Portal to Freeport
		    <option value="2619"
		  >Portal to Iceclad
		    <option value="2620"
		  >Portal to Oasis
		    <option value="2621"
		  >Portal to Ocean
		    <option value="2622"
		  >Portal to Overthere
		    <option value="2623"
		  >Portal to Qeynos
		    <option value="2624"
		  >Portal to Ro
		    <option value="2625"
		  >Portal to Timorous
		    <option value="2626"
		  >Portal to Timorous
		    <option value="865"
		  >Potion of Assailing
		    <option value="1466"
		  >Pouch of Quellious
		    <option value="1043"
		  >Power
		    <option value="1528"
		  >Power of the Forests
		    <option value="1373"
		  >Pox of Bertoxxulous
		    <option value="1764"
		  >Prayer
		    <option value="2627"
		  >Prayer to Bertoxxulous
		    <option value="2628"
		  >Prayer to Brell
		    <option value="2629"
		  >Prayer to Bristlebane
		    <option value="2630"
		  >Prayer to Cazic-Thule
		    <option value="2631"
		  >Prayer to Darkness
		    <option value="2632"
		  >Prayer to Innoruuk
		    <option value="2633"
		  >Prayer to Karana
		    <option value="2634"
		  >Prayer to Marr
		    <option value="2635"
		  >Prayer to Odus
		    <option value="2636"
		  >Prayer to Rodcet
		    <option value="2637"
		  >Prayer to the Deep Muses
		    <option value="2638"
		  >Prayer to Tunare
		    <option value="1785"
		  >Precision Mana Infusion
		    <option value="2342"
		  >Presence of Ssraeshza
		    <option value="1162"
		  >Primal Avatar
		    <option value="1601"
		  >Primal Essence
		    <option value="1173"
		  >Prime Healers Blessing
		    <option value="851"
		  >Prismatic Flux
		    <option value="1786"
		  >Produce Wrench
		    <option value="384"
		  >Project Lightning
		    <option value="644"
		  >Protect
		    <option value="1814"
		  >Protection
		    <option value="1859"
		  >Protection of Diamond
		    <option value="1860"
		  >Protection of Nature
		    <option value="1857"
		  >Protection of Rock
		    <option value="1858"
		  >Protection of Steel
		    <option value="2445"
		  >Protection of the Cabbage
		    <option value="1224"
		  >Protection of the Glades
		    <option value="2332"
		  >Protection of the Vah Shir
		    <option value="1856"
		  >Protection of Wood
		    <option value="708"
		  >Psalm of Cooling
		    <option value="709"
		  >Psalm of Mystic Shielding
		    <option value="711"
		  >Psalm of Purity
		    <option value="710"
		  >Psalm of Vitality
		    <option value="707"
		  >Psalm of Warmth
		    <option value="2639"
		  >Punishing Blow
		    <option value="925"
		  >Purge
		    <option value="691"
		  >Purify Mana
		    <option value="1953"
		  >Purifying Chorus
		    <option value="706"
		  >Purifying Rhythms
		    <option value="2064"
		  >Purifying Tonic
		    <option value="1738"
		  >Putrefy Flesh
		    <option value="923"
		  >Putrid Breath
		    <option value="1399"
		  >Pyrocruor
		    <option value="2141"
		  >Pyrokinetic Hallucinations.
		    <option value="1932"
		  >Quelious' Words of Tranquility
		    <option value="38"
		  >Quickness
		    <option value="2035"
		  >Quicksand
		    <option value="1464"
		  >Quiver of Marr
		    <option value="1394"
		  >Quivering Veil of Xarn
		    <option value="2079"
		  >Rabid Bear
		    <option value="984"
		  >Rabies
		    <option value="641"
		  >Radiant Visage
		    <option value="473"
		  >Radius of Fear2
		    <option value="336"
		  >Rage
		    <option value="2640"
		  >Rage of Ssraeshza
		    <option value="1716"
		  >Rage of Tallon
		    <option value="2641"
		  >Rage of the Rainkeeper
		    <option value="1151"
		  >Rage of the Sky
		    <option value="1717"
		  >Rage of Vallon
		    <option value="1715"
		  >Rage of Zek
		    <option value="1552"
		  >Rage of Zomm
		    <option value="2642"
		  >Raging Blizzard
		    <option value="150"
		  >Raging Strength
		    <option value="2116"
		  >Rag`Zhezum's Deathly Embrace
		    <option value="329"
		  >Rain of Blades
		    <option value="1269"
		  >Rain of Cold
		    <option value="82"
		  >Rain of Fire
		    <option value="120"
		  >Rain of Lava
		    <option value="1270"
		  >Rain of Molten Lava
		    <option value="408"
		  >Rain of Spikes
		    <option value="2099"
		  >Rain of Spores
		    <option value="409"
		  >Rain of Swords
		    <option value="2404"
		  >Rain of Terror
		    <option value="1818"
		  >Rain of the Arch Mage
		    <option value="643"
		  >Rampage
		    <option value="2643"
		  >Random Teleport 37
		    <option value="1296"
		  >Rapacious Subversion
		    <option value="1474"
		  >Rapture
		    <option value="2644"
		  >Reagent Conservation I
		    <option value="2645"
		  >Reagent Conservation II
		    <option value="2646"
		  >Reagent Conservation III
		    <option value="776"
		  >Reality Warp
		    <option value="2425"
		  >Reanimation
		    <option value="2647"
		  >Reanimation Efficiency I
		    <option value="2648"
		  >Reanimation Efficiency II
		    <option value="2649"
		  >Reanimation Efficiency III
		    <option value="2650"
		  >Reanimation Haste I
		    <option value="2651"
		  >Reanimation Haste II
		    <option value="2652"
		  >Reanimation Haste III
		    <option value="2108"
		  >Rebuke Summoned
		    <option value="2109"
		  >Rebuke Summoned
		    <option value="2110"
		  >Rebuke Summoned
		    <option value="2015"
		  >Rebuke the Dead
		    <option value="1479"
		  >Recant Magic
		    <option value="1816"
		  >Recant Magic
		    <option value="853"
		  >Reckless Health
		    <option value="214"
		  >Reckless Strength
		    <option value="1325"
		  >Reckoning
		    <option value="330"
		  >Reclaim Energy
		    <option value="2426"
		  >Reconstitution
		    <option value="143"
		  >Regeneration
		    <option value="1350"
		  >Regrowth
		    <option value="1873"
		  >Regrowth of Dar Khura
		    <option value="1351"
		  >Regrowth of the Grove
		    <option value="1037"
		  >Rejuvenation
		    <option value="974"
		  >Rejuvinate
		    <option value="2358"
		  >Rejuvination
		    <option value="1057"
		  >Rekeklication
		    <option value="1300"
		  >Remedy
		    <option value="2275"
		  >Remove Curse
		    <option value="2209"
		  >Remove Greater Curse
		    <option value="749"
		  >Rend
		    <option value="443"
		  >Renew Bones
		    <option value="1286"
		  >Renew Elements
		    <option value="1287"
		  >Renew Summoning
		    <option value="2428"
		  >Renewal
		    <option value="193"
		  >Reoccurring Amnesia
		    <option value="2427"
		  >Reparation
		    <option value="1102"
		  >Repulse
		    <option value="1332"
		  >Repulse Animal
		    <option value="60"
		  >Resist Cold
		    <option value="62"
		  >Resist Disease
		    <option value="59"
		  >Resist Fire
		    <option value="63"
		  >Resist Magic
		    <option value="61"
		  >Resist Poison
		    <option value="941"
		  >Resistance to Magic
		    <option value="2073"
		  >Resistance Tonic
		    <option value="380"
		  >Resistant Skin
		    <option value="313"
		  >Resolution
		    <option value="447"
		  >Rest the Dead
		    <option value="491"
		  >Restless Bones
		    <option value="2429"
		  >Restoration
		    <option value="2081"
		  >Restore Companion
		    <option value="927"
		  >Restore Sight
		    <option value="391"
		  >Resurrection
		    <option value="751"
		  >Resurrection Effect
		    <option value="750"
		  >Resurrection Effects
		    <option value="387"
		  >Resuscitate
		    <option value="667"
		  >Retribution
		    <option value="1433"
		  >Retribution of Al'Kabor
		    <option value="390"
		  >Revive
		    <option value="1306"
		  >Reviviscence
		    <option value="2050"
		  >Rich's Test spell
		    <option value="1938"
		  >Riftwind's Protection
		    <option value="530"
		  >Ring of Butcher
		    <option value="1813"
		  >Ring of Cobalt Scar
		    <option value="529"
		  >Ring of Commons
		    <option value="1608"
		  >Ring of Faydark
		    <option value="534"
		  >Ring of Feerrott
		    <option value="1811"
		  >Ring of Great Divide
		    <option value="1215"
		  >Ring of Iceclad
		    <option value="528"
		  >Ring of Karana
		    <option value="532"
		  >Ring of Lavastorm
		    <option value="536"
		  >Ring of Misty
		    <option value="533"
		  >Ring of Ro
		    <option value="535"
		  >Ring of Steamfont
		    <option value="1803"
		  >Ring of Surefall Glade
		    <option value="1111"
		  >Ring of the Combines
		    <option value="531"
		  >Ring of Toxxulia
		    <option value="1812"
		  >Ring of Wakening Lands
		    <option value="1120"
		  >Ring of Winter
		    <option value="1255"
		  >Ring of Winter
		    <option value="1377"
		  >Riotous Health
		    <option value="348"
		  >Rising Dexterity
		    <option value="2038"
		  >Ritualistic Bloodlust
		    <option value="2653"
		  >River's Rancor
		    <option value="545"
		  >Ro Gate
		    <option value="565"
		  >Ro Portal
		    <option value="1863"
		  >Ro's Smoldering Disjunction
		    <option value="2036"
		  >Rock Mites
		    <option value="2173"
		  >Rockhopper Spit
		    <option value="1753"
		  >Rocksmash
		    <option value="1883"
		  >Rod of Mystical Transvergance
		    <option value="1277"
		  >Rodricks Gift
		    <option value="229"
		  >Root
		    <option value="1992"
		  >Rotting Bile
		    <option value="874"
		  >Rotting Flesh
		    <option value="2464"
		  >Rowyl's Gleeful Destruction
		    <option value="1219"
		  >Ro`s Fiery Sundering
		    <option value="926"
		  >Rubicite Aura
		    <option value="2340"
		  >Rumbling Earth
		    <option value="480"
		  >Rune I
		    <option value="481"
		  >Rune II
		    <option value="482"
		  >Rune III
		    <option value="483"
		  >Rune IV
		    <option value="1082"
		  >Rune Shield
		    <option value="1471"
		  >Rune V
		    <option value="2070"
		  >Sacred Barrier
		    <option value="1849"
		  >Sacred Word
		    <option value="1550"
		  >Sacrifice
		    <option value="680"
		  >Sagar`s Animation
		    <option value="1846"
		  >Sanctuary
		    <option value="2654"
		  >Sand Storm
		    <option value="305"
		  >Sanity Warp
		    <option value="819"
		  >Sathir's Gaze
		    <option value="822"
		  >Sathir's Mesmerization
		    <option value="1838"
		  >Savage Roar
		    <option value="139"
		  >Savage Spirit
		    <option value="2270"
		  >Savagery
		    <option value="423"
		  >Scale of Wolf
		    <option value="273"
		  >Scale Skin
		    <option value="1097"
		  >Scarab Storm
		    <option value="877"
		  >ScarecrowFear
		    <option value="954"
		  >Scareling Step
		    <option value="1443"
		  >Scars of Sigil
		    <option value="1295"
		  >Scent of Darkness
		    <option value="1293"
		  >Scent of Dusk
		    <option value="1294"
		  >Scent of Shadow
		    <option value="1498"
		  >Scent of Terris
		    <option value="1441"
		  >Scintillation
		    <option value="1036"
		  >Scorching Skin
		    <option value="1385"
		  >Scoriae
		    <option value="30"
		  >Scourge
		    <option value="1288"
		  >Scouring Wind
		    <option value="1263"
		  >Scream of Chaos
		    <option value="1919"
		  >Scream of Death
		    <option value="1917"
		  >Scream of Hate
		    <option value="1918"
		  >Scream of Pain
		    <option value="911"
		  >Screaming Mace
		    <option value="547"
		  >Screaming Terror
		    <option value="2049"
		  >ScreamOfDeathEffect
		    <option value="1168"
		  >Screech
		    <option value="2114"
		  >Screech Immunity
		    <option value="1005"
		  >Sear
		    <option value="1066"
		  >Searing Flames
		    <option value="1596"
		  >Sebilite Pox
		    <option value="1500"
		  >Sedulous Subversion
		    <option value="79"
		  >See Invisible
		    <option value="1446"
		  >Seeking Flame of Seukor
		    <option value="1709"
		  >Seething Fury
		    <option value="712"
		  >Selo`s Accelerando
		    <option value="1950"
		  >Selo`s Accelerating Chorus
		    <option value="1540"
		  >Selo`s Assonant Strane
		    <option value="741"
		  >Selo`s Chords of Cessation
		    <option value="733"
		  >Selo`s Consonant Chain
		    <option value="1115"
		  >Selo`s Song of Travel
		    <option value="1532"
		  >Selo`s Song of Travel
		    <option value="1789"
		  >Selo`s Song of Travel
		    <option value="237"
		  >Sense Animals
		    <option value="231"
		  >Sense Summoned
		    <option value="220"
		  >Sense the Dead
		    <option value="298"
		  >Sentinel
		    <option value="2134"
		  >Sermon of Ssraeshza
		    <option value="1848"
		  >Sermon of the Righteous
		    <option value="275"
		  >Serpent Sight
		    <option value="2301"
		  >Serpent's Bite
		    <option value="2233"
		  >Serpentine Curse
		    <option value="1404"
		  >Servant of Bones
		    <option value="2089"
		  >Servant of Nature
		    <option value="2271"
		  >Sha's Advantage
		    <option value="1973"
		  >Sha's Ferocity
		    <option value="1979"
		  >Sha's Lethargy
		    <option value="1969"
		  >Sha's Restoration
		    <option value="2391"
		  >Sha's Vengeance
		    <option value="1887"
		  >Shackle of Bone
		    <option value="1889"
		  >Shackle of Spirit
		    <option value="2189"
		  >Shackles
		    <option value="648"
		  >Shade
		    <option value="649"
		  >Shadow
		    <option value="1292"
		  >Shadow Compact
		    <option value="2298"
		  >Shadow Creep
		    <option value="2147"
		  >Shadow Fog
		    <option value="89"
		  >Shadow Sight
		    <option value="353"
		  >Shadow Step
		    <option value="369"
		  >Shadow Vortex
		    <option value="2407"
		  >Shadow Warding 1
		    <option value="2408"
		  >Shadow Warding 2
		    <option value="2409"
		  >Shadow Warding 3
		    <option value="2410"
		  >Shadow Warding 4
		    <option value="2411"
		  >Shadow Warding 5
		    <option value="1499"
		  >Shadowbond
		    <option value="678"
		  >Shalee`s Animation
		    <option value="285"
		  >Shallow Breath
		    <option value="768"
		  >ShapeChange25
		    <option value="1007"
		  >ShapeChange25
		    <option value="769"
		  >ShapeChange30
		    <option value="1008"
		  >ShapeChange30
		    <option value="770"
		  >ShapeChange35
		    <option value="1009"
		  >ShapeChange35
		    <option value="771"
		  >ShapeChange40
		    <option value="1010"
		  >ShapeChange40
		    <option value="772"
		  >ShapeChange45
		    <option value="1011"
		  >ShapeChange45
		    <option value="773"
		  >ShapeChange50
		    <option value="1012"
		  >ShapeChange50
		    <option value="1705"
		  >ShapeChange55
		    <option value="1706"
		  >ShapeChange60
		    <option value="2345"
		  >ShapeChange65
		    <option value="1576"
		  >Shards of Sorrow
		    <option value="839"
		  >Shardwurm Breath
		    <option value="427"
		  >Share Wolf Form
		    <option value="1956"
		  >Sharik's Replenishing
		    <option value="714"
		  >Shauri`s Sonorous Clouding
		    <option value="2303"
		  >Shield of Auras
		    <option value="272"
		  >Shield of Barbs
		    <option value="1342"
		  >Shield of Blades
		    <option value="128"
		  >Shield of Brambles
		    <option value="2423"
		  >Shield of Darkness
		    <option value="331"
		  >Shield of Fire
		    <option value="410"
		  >Shield of Flame
		    <option value="411"
		  >Shield of Lava
		    <option value="2655"
		  >Shield of Life
		    <option value="1232"
		  >Shield of Songs
		    <option value="431"
		  >Shield of Spikes
		    <option value="1745"
		  >Shield of the Eighth
		    <option value="1392"
		  >Shield of the Magi
		    <option value="1578"
		  >Shield of the Ring
		    <option value="1577"
		  >Shield of the Winter
		    <option value="255"
		  >Shield of Thistles
		    <option value="355"
		  >Shield of Thorns
		    <option value="19"
		  >Shield of Words
		    <option value="308"
		  >Shielding
		    <option value="235"
		  >Shieldskin
		    <option value="430"
		  >Shifting Shield
		    <option value="83"
		  >Shifting Sight
		    <option value="185"
		  >Shiftless Deeds
		    <option value="2231"
		  >Shissar Broodling Poison
		    <option value="323"
		  >Shock of Blades
		    <option value="1885"
		  >Shock of Fiery Blades
		    <option value="375"
		  >Shock of Fire
		    <option value="333"
		  >Shock of Flame
		    <option value="371"
		  >Shock of Frost
		    <option value="651"
		  >Shock of Ice
		    <option value="382"
		  >Shock of Lightning
		    <option value="203"
		  >Shock of Poison
		    <option value="2418"
		  >Shock of Shadows
		    <option value="112"
		  >Shock of Spikes
		    <option value="1445"
		  >Shock of Steel
		    <option value="113"
		  >Shock of Swords
		    <option value="1209"
		  >Shock of the Tainted
		    <option value="458"
		  >Shock Spiral of Al'Kabor
		    <option value="2144"
		  >Shower of Blood
		    <option value="2170"
		  >Shrieker Slow
		    <option value="2171"
		  >Shrieker Stun
		    <option value="2169"
		  >Shrieker's Curse
		    <option value="1585"
		  >Shrieking Howl
		    <option value="344"
		  >Shrink
		    <option value="1241"
		  >Shroud of Death
		    <option value="1239"
		  >Shroud of Hate
		    <option value="1240"
		  >Shroud of Pain
		    <option value="1821"
		  >Shroud of Stealth
		    <option value="1366"
		  >Shroud of the Spirits
		    <option value="1161"
		  >Shroud of Undeath
		    <option value="1253"
		  >ShroudOfDeathEffect
		    <option value="74"
		  >Sicken
		    <option value="576"
		  >Sight
		    <option value="360"
		  >Sight Graft
		    <option value="2421"
		  >Silence of the Shadows
		    <option value="1262"
		  >Silver Breath
		    <option value="1153"
		  >Silver Skin
		    <option value="1052"
		  >Siphon
		    <option value="445"
		  >Siphon Life
		    <option value="342"
		  >Siphon Strength
		    <option value="1444"
		  >Sirocco
		    <option value="679"
		  >Sisna`s Animation
		    <option value="2013"
		  >Skeletal Curse
		    <option value="421"
		  >Skin like Diamond
		    <option value="422"
		  >Skin like Nature
		    <option value="262"
		  >Skin like Rock
		    <option value="420"
		  >Skin like Steel
		    <option value="25"
		  >Skin like Wood
		    <option value="2376"
		  >Skin like Wood
		    <option value="1407"
		  >Skin of the Shadow
		    <option value="883"
		  >SkunkSpray
		    <option value="1273"
		  >Slime Mist
		    <option value="931"
		  >Slow
		    <option value="15"
		  >Smite
		    <option value="894"
		  >Smolder
		    <option value="2305"
		  >Smoldering Bullwark
		    <option value="2037"
		  >Smothering Sands
		    <option value="2259"
		  >Snake Bite
		    <option value="887"
		  >SnakeEleFireBurst
		    <option value="241"
		  >Snare
		    <option value="2052"
		  >SOBEffect
		    <option value="2053"
		  >SOIEffect
		    <option value="2218"
		  >SOIFffect
		    <option value="2656"
		  >Solar Storm
		    <option value="2051"
		  >SOLEffect
		    <option value="745"
		  >Solon's Bewitching Bravura
		    <option value="1547"
		  >Solon's Charismatic Concord
		    <option value="720"
		  >Solon's Song of the Sirens
		    <option value="858"
		  >Soluan`s Vigor
		    <option value="1536"
		  >Song of Dawn
		    <option value="1537"
		  >Song of Highsun
		    <option value="1538"
		  >Song of Midnight
		    <option value="1947"
		  >Song of Sustenance
		    <option value="1755"
		  >Song of the Deep Seas
		    <option value="1535"
		  >Song of Twilight
		    <option value="899"
		  >Sonic
		    <option value="2102"
		  >Sonic Feedback
		    <option value="892"
		  >SonicScream
		    <option value="500"
		  >Soothe
		    <option value="2054"
		  >SOScEffect
		    <option value="2220"
		  >SOSEffect
		    <option value="2057"
		  >SOStEffect
		    <option value="1593"
		  >Soul Bond
		    <option value="1712"
		  >Soul Consumption
		    <option value="2235"
		  >Soul Curse
		    <option value="972"
		  >Soul Devour
		    <option value="752"
		  >Soul Leech
		    <option value="2238"
		  >Soul Tap
		    <option value="1598"
		  >Soul Well
		    <option value="124"
		  >Sound of Force
		    <option value="2055"
		  >SOVEffect
		    <option value="2056"
		  >SOWEffect
		    <option value="101"
		  >Spear of Warding
		    <option value="1027"
		  >Special Sight
		    <option value="1099"
		  >SpectraStun
		    <option value="895"
		  >SpectreLifetap
		    <option value="896"
		  >SpectreStun
		    <option value="1126"
		  >Speechless Fright
		    <option value="2224"
		  >Speed of the Brood
		    <option value="1721"
		  >Speed of the Shissar
		    <option value="2657"
		  >Spell Haste I
		    <option value="2658"
		  >Spell Haste II
		    <option value="2659"
		  >Spell Haste III
		    <option value="1904"
		  >Spellshield
		    <option value="372"
		  >Sphere of Light
		    <option value="517"
		  >Spikecoat
		    <option value="1000"
		  >Spin the Bottle
		    <option value="956"
		  >Spine Chill
		    <option value="2153"
		  >Spire Strike
		    <option value="367"
		  >Spirit Armor
		    <option value="2236"
		  >Spirit Curse
		    <option value="2012"
		  >Spirit Leech
		    <option value="278"
		  >Spirit of Bear
		    <option value="1869"
		  >Spirit of Bih`Li
		    <option value="147"
		  >Spirit of Cat
		    <option value="219"
		  >Spirit of Cheetah
		    <option value="1862"
		  >Spirit of Eagle
		    <option value="2217"
		  >Spirit of Flame
		    <option value="1961"
		  >Spirit of Herikol
		    <option value="1982"
		  >Spirit of Inferno
		    <option value="1966"
		  >Spirit of Kashek
		    <option value="1959"
		  >Spirit of Keshuval
		    <option value="1978"
		  >Spirit of Khaliz
		    <option value="1976"
		  >Spirit of Khati Sha
		    <option value="1972"
		  >Spirit of Khurenz
		    <option value="1980"
		  >Spirit of Lightning
		    <option value="145"
		  >Spirit of Monkey
		    <option value="1346"
		  >Spirit of Oak
		    <option value="1968"
		  >Spirit of Omakin
		    <option value="148"
		  >Spirit of Ox
		    <option value="1336"
		  >Spirit of Scale
		    <option value="1957"
		  >Spirit of Sharik
		    <option value="283"
		  >Spirit of Snake
		    <option value="2219"
		  >Spirit of Snow
		    <option value="949"
		  >Spirit of the Azure Flame
		    <option value="1981"
		  >Spirit of the Blizzard
		    <option value="950"
		  >Spirit of the Cobalt Flame
		    <option value="1356"
		  >Spirit of the Howler
		    <option value="952"
		  >Spirit of the Sapphire Flame
		    <option value="1983"
		  >Spirit of the Scorpion
		    <option value="1986"
		  >Spirit of the Storm
		    <option value="951"
		  >Spirit of the Turqouise Flame
		    <option value="1984"
		  >Spirit of Vermin
		    <option value="1985"
		  >Spirit of Wind
		    <option value="277"
		  >Spirit of Wolf
		    <option value="1558"
		  >Spirit of Wolf
		    <option value="1963"
		  >Spirit of Yekan
		    <option value="1971"
		  >Spirit of Zehkes
		    <option value="271"
		  >Spirit Pouch
		    <option value="1212"
		  >Spirit Quickening
		    <option value="78"
		  >Spirit Sight
		    <option value="146"
		  >Spirit Strength
		    <option value="281"
		  >Spirit Strike
		    <option value="522"
		  >Spirit Tap
		    <option value="2338"
		  >Spiritual Discipline
		    <option value="1974"
		  >Spiritual Purity
		    <option value="1975"
		  >Spiritual Strength
		    <option value="964"
		  >Spiroc Thunder
		    <option value="1065"
		  >Splintering Shards
		    <option value="1402"
		  >Splurt
		    <option value="208"
		  >Spook the Dead
		    <option value="2098"
		  >Spreading Crud
		    <option value="1830"
		  >Ssraeshza's Command
		    <option value="1040"
		  >Stability
		    <option value="613"
		  >Staff of Runes
		    <option value="614"
		  >Staff of Symbols
		    <option value="611"
		  >Staff of Tracing
		    <option value="612"
		  >Staff of Warding
		    <option value="937"
		  >Stalking Probe
		    <option value="855"
		  >Stalwart Regeneration
		    <option value="157"
		  >Stamina
		    <option value="666"
		  >Starfire
		    <option value="256"
		  >Starshine
		    <option value="1788"
		  >Static
		    <option value="2660"
		  >Static Pulse
		    <option value="2661"
		  >Static Storm
		    <option value="921"
		  >Steal Strength
		    <option value="2662"
		  >Steam Blast
		    <option value="1782"
		  >Steam Overload
		    <option value="392"
		  >Steelskin
		    <option value="2297"
		  >Sting the Imagination
		    <option value="263"
		  >Stinging Swarm
		    <option value="903"
		  >Stone Breath
		    <option value="2663"
		  >Stone Gale
		    <option value="2664"
		  >Stone Thunder
		    <option value="2227"
		  >Stone's Embrace
		    <option value="893"
		  >StoneSpiderStun
		    <option value="2665"
		  >Storm Comet
		    <option value="2666"
		  >Storm Flame
		    <option value="2014"
		  >Storm of Ice
		    <option value="2007"
		  >Storm of Lightning
		    <option value="429"
		  >Storm Strength
		    <option value="1799"
		  >Storm Strength
		    <option value="1584"
		  >Storm Strike
		    <option value="2145"
		  >Storm Tremor
		    <option value="2256"
		  >Storm Tremor2
		    <option value="826"
		  >Stream of Acid
		    <option value="1165"
		  >Stream of Acid
		    <option value="158"
		  >Strength
		    <option value="267"
		  >Strength of Earth
		    <option value="1179"
		  >Strength of Nature
		    <option value="428"
		  >Strength of Stone
		    <option value="1611"
		  >Strength of the Kunzar
		    <option value="39"
		  >Strengthen
		    <option value="1074"
		  >Strengthen Death
		    <option value="13"
		  >Strike
		    <option value="1573"
		  >Strike of the Chosen
		    <option value="2281"
		  >Strike of the Grol Baku
		    <option value="1734"
		  >Strike of Thunder
		    <option value="1839"
		  >Striking Vertigo
		    <option value="23"
		  >Strip Enchantment
		    <option value="991"
		  >Strong Disease
		    <option value="980"
		  >Strong Poison
		    <option value="2077"
		  >Strong Root
		    <option value="989"
		  >Strong Wasp Poison
		    <option value="215"
		  >Stun
		    <option value="825"
		  >Stun Breath
		    <option value="1228"
		  >Stun Command
		    <option value="1998"
		  >Stun Shock
		    <option value="1589"
		  >Stunning Blow
		    <option value="935"
		  >Stunning Roar
		    <option value="1750"
		  >Stunning Strike
		    <option value="1349"
		  >Succor
		    <option value="606"
		  >Succor: Butcher
		    <option value="605"
		  >Succor: East
		    <option value="608"
		  >Succor: Lavastorm
		    <option value="609"
		  >Succor: North
		    <option value="607"
		  >Succor: Ro
		    <option value="1892"
		  >Succussion of Shadows
		    <option value="449"
		  >Suffocate
		    <option value="293"
		  >Suffocating Sphere
		    <option value="2311"
		  >Summon Ale
		    <option value="100"
		  >Summon Arrows
		    <option value="317"
		  >Summon Bandages
		    <option value="898"
		  >Summon Bill
		    <option value="2465"
		  >Summon Brass Choker
		    <option value="2479"
		  >Summon Brilliant Bauble
		    <option value="102"
		  >Summon Coldstone
		    <option value="1070"
		  >Summon Companion
		    <option value="2"
		  >Summon Corpse
		    <option value="2093"
		  >Summon Corpse
		    <option value="310"
		  >Summon Dagger
		    <option value="440"
		  >Summon Dead
		    <option value="2204"
		  >Summon Death Charger
		    <option value="210"
		  >Summon Drink
		    <option value="1880"
		  >Summon Elemental Blanket
		    <option value="1876"
		  >Summon Elemental Defender
		    <option value="318"
		  >Summon Fang
		    <option value="2344"
		  >Summon Fangs
		    <option value="49"
		  >Summon Food
		    <option value="967"
		  >Summon Fruit
		    <option value="857"
		  >Summon Gate Potion
		    <option value="2467"
		  >Summon Golden Choker
		    <option value="845"
		  >Summon Golin
		    <option value="2258"
		  >Summon Hat Rim
		    <option value="319"
		  >Summon Heatstone
		    <option value="1167"
		  >Summon Holy Ale of Brell
		    <option value="2203"
		  >Summon Holy Steed
		    <option value="2062"
		  >Summon Horse
		    <option value="2191"
		  >Summon Horse
		    <option value="2192"
		  >Summon Horse
		    <option value="2193"
		  >Summon Horse
		    <option value="2194"
		  >Summon Horse
		    <option value="2195"
		  >Summon Horse
		    <option value="2196"
		  >Summon Horse
		    <option value="2197"
		  >Summon Horse
		    <option value="2198"
		  >Summon Horse
		    <option value="2199"
		  >Summon Horse
		    <option value="2200"
		  >Summon Horse
		    <option value="2201"
		  >Summon Horse
		    <option value="2202"
		  >Summon Horse
		    <option value="2245"
		  >Summon Horse
		    <option value="2246"
		  >Summon Horse
		    <option value="2247"
		  >Summon Horse
		    <option value="2248"
		  >Summon Horse
		    <option value="2249"
		  >Summon Horse
		    <option value="2250"
		  >Summon Horse
		    <option value="2251"
		  >Summon Horse
		    <option value="2252"
		  >Summon Horse
		    <option value="2471"
		  >Summon Jade Bracelet
		    <option value="1600"
		  >Summon Lava Diamond
		    <option value="2469"
		  >Summon Leather Mantle
		    <option value="2468"
		  >Summon Linen Mantle
		    <option value="2472"
		  >Summon Opal Bracelet
		    <option value="1726"
		  >Summon Orb
		    <option value="2244"
		  >Summon Peridot
		    <option value="2343"
		  >Summon Peridots
		    <option value="1878"
		  >Summon Phantom Chain
		    <option value="1877"
		  >Summon Phantom Leather
		    <option value="1879"
		  >Summon Phantom Plate
		    <option value="104"
		  >Summon Ring of Flight
		    <option value="2473"
		  >Summon Ruby Bracelet
		    <option value="2444"
		  >Summon Shaded Sphere
		    <option value="1183"
		  >Summon Shard of the Core
		    <option value="2478"
		  >Summon Shiny Bauble
		    <option value="2470"
		  >Summon Silken Mantle
		    <option value="2466"
		  >Summon Silver Choker
		    <option value="2476"
		  >Summon Studded Ring
		    <option value="2477"
		  >Summon Tarnished Bauble
		    <option value="99"
		  >Summon Throwing Dagger
		    <option value="1763"
		  >Summon Throwing Hammer
		    <option value="2474"
		  >Summon Tiny Ring
		    <option value="2475"
		  >Summon Twisted Ring
		    <option value="1977"
		  >Summon Warder
		    <option value="3"
		  >Summon Waterstone
		    <option value="968"
		  >Summon Wine
		    <option value="320"
		  >Summon Wisp
		    <option value="1592"
		  >Summon: Guardian of the Vale
		    <option value="2667"
		  >Summoning Efficiency I
		    <option value="2668"
		  >Summoning Efficiency II
		    <option value="2669"
		  >Summoning Efficiency III
		    <option value="2670"
		  >Summoning Haste I
		    <option value="2671"
		  >Summoning Haste II
		    <option value="2672"
		  >Summoning Haste III
		    <option value="570"
		  >Summoning: Air
		    <option value="567"
		  >Summoning: Earth
		    <option value="569"
		  >Summoning: Fire
		    <option value="568"
		  >Summoning: Water
		    <option value="800"
		  >SumMonsterAttack
		    <option value="2673"
		  >SummonTest1
		    <option value="2289"
		  >Sun Revenant's Curse
		    <option value="142"
		  >Sunbeam
		    <option value="807"
		  >Sunset Home
		    <option value="1025"
		  >Sunset Home
		    <option value="1508"
		  >Sunskin
		    <option value="1440"
		  >Sunstrike
		    <option value="33"
		  >Superior Camouflage
		    <option value="8"
		  >Superior Healing
		    <option value="728"
		  >Supernova
		    <option value="454"
		  >Surge of Enfeeblement
		    <option value="1050"
		  >Swamp Port
		    <option value="1206"
		  >Swarm of Retribution
		    <option value="1250"
		  >Swarming Pain
		    <option value="171"
		  >Swift like the Wind
		    <option value="1711"
		  >Swift Spirit
		    <option value="615"
		  >Sword of Runes
		    <option value="1317"
		  >Symbol of Marzin
		    <option value="487"
		  >Symbol of Naltron
		    <option value="486"
		  >Symbol of Pinzarn
		    <option value="485"
		  >Symbol of Ryltan
		    <option value="484"
		  >Symbol of Transal
		    <option value="488"
		  >Sympathetic Aura
		    <option value="1587"
		  >Symphonic Harmony
		    <option value="757"
		  >System Shock I
		    <option value="1642"
		  >System Shock II
		    <option value="1643"
		  >System Shock III
		    <option value="1644"
		  >System Shock IV
		    <option value="1663"
		  >System Shock V
		    <option value="721"
		  >Syvelian`s Anti-Magic Aria
		    <option value="505"
		  >Tagar's Insects
		    <option value="2113"
		  >Tainted Bite
		    <option value="276"
		  >Tainted Breath
		    <option value="167"
		  >Talisman of Altuna
		    <option value="1874"
		  >Talisman of Epuration
		    <option value="1352"
		  >Talisman of Jasinth
		    <option value="1367"
		  >Talisman of Kragg
		    <option value="1353"
		  >Talisman of Shadoo
		    <option value="1866"
		  >Talisman of the Beast
		    <option value="1362"
		  >Talisman of the Brute
		    <option value="1361"
		  >Talisman of the Cat
		    <option value="1365"
		  >Talisman of the Raptor
		    <option value="1363"
		  >Talisman of the Rhino
		    <option value="1364"
		  >Talisman of the Serpent
		    <option value="166"
		  >Talisman of Tnarg
		    <option value="1936"
		  >Tangling Weeds
		    <option value="2257"
		  >Tangling Worms
		    <option value="1569"
		  >Tap Keg
		    <option value="288"
		  >Taper Enchantment
		    <option value="724"
		  >Tarew`s Aquatic Ayre
		    <option value="671"
		  >Tashan
		    <option value="672"
		  >Tashani
		    <option value="673"
		  >Tashania
		    <option value="1484"
		  >Tashanian
		    <option value="2019"
		  >Taunting Blow
		    <option value="2453"
		  >Tavee's Charm of Diuturnity
		    <option value="2454"
		  >Tavee's Greater Diuturnity
		    <option value="2452"
		  >Tavee's Lesser Diuturnity
		    <option value="1431"
		  >Tears of Druzzil
		    <option value="1429"
		  >Tears of Prexus
		    <option value="1430"
		  >Tears of Solusek
		    <option value="850"
		  >Telekinesis
		    <option value="936"
		  >Telescope
		    <option value="2674"
		  >Tempest Guard
		    <option value="2675"
		  >Tempest Wind
		    <option value="2337"
		  >Tempest Wing's Embrace
		    <option value="2000"
		  >Tendon slice
		    <option value="2676"
		  >Tendrils of Apathy
		    <option value="2161"
		  >Tendrils of Madness
		    <option value="1817"
		  >Tentacle Sting
		    <option value="184"
		  >Tepid Deeds
		    <option value="2002"
		  >Terrifying Darkness
		    <option value="513"
		  >Terrorize Animal
		    <option value="880"
		  >TerrorLifetap
		    <option value="2480"
		  >Test
		    <option value="1129"
		  >Test GACD
		    <option value="1130"
		  >Test GATD
		    <option value="1127"
		  >Test GLT
		    <option value="1132"
		  >Test GMD
		    <option value="1131"
		  >Test GSTRD
		    <option value="787"
		  >Test Shield
		    <option value="2677"
		  >Tetanic Blades
		    <option value="2443"
		  >The Anti-Gulper
		    <option value="1586"
		  >The Breaking
		    <option value="1258"
		  >The Dain's Justice
		    <option value="2312"
		  >The Emperor's Will
		    <option value="2063"
		  >The Nexus
		    <option value="1327"
		  >The Unspoken Word
		    <option value="2678"
		  >Theft of Life
		    <option value="1468"
		  >Theft of Thought
		    <option value="389"
		  >Thicken Mana
		    <option value="514"
		  >Thistlecoat
		    <option value="518"
		  >Thorncoat
		    <option value="1757"
		  >Thorny Shield
		    <option value="1990"
		  >Thought Drain
		    <option value="2261"
		  >Thought Horror Charm
		    <option value="2365"
		  >Thought Sieve
		    <option value="2146"
		  >Thought Vortex
		    <option value="1406"
		  >Thrall of Bones
		    <option value="2401"
		  >Thule's Scourge
		    <option value="1274"
		  >Thunder Blast
		    <option value="2679"
		  >Thunder Call
		    <option value="1931"
		  >Thunder of Karana
		    <option value="2680"
		  >Thunder Storm
		    <option value="469"
		  >Thunder Strike
		    <option value="1801"
		  >Thunder Strike
		    <option value="1438"
		  >Thunderbold
		    <option value="470"
		  >Thunderclap
		    <option value="2681"
		  >Thunderclap
		    <option value="1105"
		  >Thurgadin Gate
		    <option value="1371"
		  >Tigir's Insects
		    <option value="1146"
		  >Time Out
		    <option value="502"
		  >Tishan's Clash
		    <option value="1416"
		  >Tishan's Discord
		    <option value="1520"
		  >Tishan's Relocation
		    <option value="506"
		  >Togor's Insects
		    <option value="1197"
		  >Torbas Acid Blast
		    <option value="1568"
		  >Torment
		    <option value="1482"
		  >Torment of Argli
		    <option value="1720"
		  >Torment of Shadows
		    <option value="2682"
		  >Tornado
		    <option value="1358"
		  >Torpor
		    <option value="1924"
		  >Torrent of Fatigue
		    <option value="1922"
		  >Torrent of Hate
		    <option value="2683"
		  >Torrent of Ice
		    <option value="1923"
		  >Torrent of Pain
		    <option value="1369"
		  >Torrent of Poison
		    <option value="2684"
		  >Tortured Memory
		    <option value="1843"
		  >Torturing Winds
		    <option value="2685"
		  >Touch of Life
		    <option value="1400"
		  >Touch of Night
		    <option value="2234"
		  >Touch of the Mistress
		    <option value="2293"
		  >Touch of the Plaguebringer
		    <option value="2686"
		  >Touch of the Void
		    <option value="2188"
		  >Touch of Vinitras
		    <option value="2135"
		  >Touch of Zebuxorux
		    <option value="539"
		  >Tox Gate
		    <option value="559"
		  >Tox Portal
		    <option value="693"
		  >Track Corpse
		    <option value="2394"
		  >Tragedy at Cazic Thule
		    <option value="842"
		  >Trakanon's Touch
		    <option value="1204"
		  >Translocate
		    <option value="1160"
		  >Translocate: Cazic
		    <option value="1807"
		  >Translocate: Cobalt Scar
		    <option value="1124"
		  >Translocate: Combine
		    <option value="1157"
		  >Translocate: Common
		    <option value="1121"
		  >Translocate: Fay
		    <option value="1805"
		  >Translocate: Great Divide
		    <option value="1119"
		  >Translocate: Group
		    <option value="1804"
		  >Translocate: Iceclad
		    <option value="1156"
		  >Translocate: Nek
		    <option value="2272"
		  >Translocate: Nexus
		    <option value="1123"
		  >Translocate: North
		    <option value="1158"
		  >Translocate: Ro
		    <option value="1122"
		  >Translocate: Tox
		    <option value="1806"
		  >Translocate: Wakening Lands
		    <option value="1159"
		  >Translocate: West
		    <option value="1145"
		  >Translocational Anchor
		    <option value="1164"
		  >Translocational Disruption
		    <option value="1163"
		  >Translocational Suppression
		    <option value="2101"
		  >Transmute Flesh to Bone
		    <option value="1605"
		  >Transmute Gauntlets
		    <option value="1606"
		  >Transmute Hammer
		    <option value="1881"
		  >Transon's Elemental Infusion
		    <option value="2225"
		  >Transon's Elemental Renewal
		    <option value="1884"
		  >Transons Phantasmal Protection
		    <option value="1125"
		  >TravelerBoots
		    <option value="257"
		  >Treeform
		    <option value="404"
		  >Tremor
		    <option value="1309"
		  >Trepidation
		    <option value="1911"
		  >Trickster's Augmentation
		    <option value="2045"
		  >Trickster's Torment
		    <option value="2046"
		  >Trickster's TormentSK
		    <option value="861"
		  >Troll`s Essence
		    <option value="1517"
		  >Trucidation
		    <option value="204"
		  >True North
		    <option value="1570"
		  >Truesight
		    <option value="1266"
		  >Tsunami
		    <option value="1210"
		  >Tumultuous Strength
		    <option value="1338"
		  >Tunare's Request
		    <option value="1370"
		  >Turgur's Insects
		    <option value="2105"
		  >Turn Undead
		    <option value="2106"
		  >Turn Undead
		    <option value="2107"
		  >Turn Undead
		    <option value="1225"
		  >Turning of the Unnatural
		    <option value="282"
		  >Turtle Skin
		    <option value="738"
		  >Tuyen`s Chant of Flame
		    <option value="739"
		  >Tuyen`s Chant of Frost
		    <option value="681"
		  >Uleen`s Animation
		    <option value="1001"
		  >UltraDeathSpell
		    <option value="1002"
		  >UltraDeathSpell2
		    <option value="1003"
		  >UltraDeathSpell3
		    <option value="45"
		  >Ultravision
		    <option value="1493"
		  >Umbra
		    <option value="2341"
		  >Umbral Rot
		    <option value="1560"
		  >Unconceal Rapier
		    <option value="1562"
		  >Unconceal Rapier
		    <option value="1564"
		  >Unconceal Rapier
		    <option value="2088"
		  >Undead Pact
		    <option value="864"
		  >Undead`s Recourse
		    <option value="1379"
		  >Unfailing Reverence
		    <option value="2112"
		  >Unholy Steed
		    <option value="2018"
		  >Unity of Fire
		    <option value="1765"
		  >Unsheath Dagger
		    <option value="869"
		  >Unsummon Self
		    <option value="1503"
		  >Unswerving Hammer of Faith
		    <option value="1324"
		  >Upheaval
		    <option value="2151"
		  >Upheaval
		    <option value="1069"
		  >Valiant Companion
		    <option value="311"
		  >Valor
		    <option value="1930"
		  >Valor of Marr
		    <option value="809"
		  >VampEmbraceNecro
		    <option value="810"
		  >VampEmbraceShadow
		    <option value="907"
		  >Vampire Aura
		    <option value="889"
		  >Vampire Charm
		    <option value="890"
		  >Vampire Touch
		    <option value="891"
		  >Vampire Wolf Form
		    <option value="453"
		  >Vampiric Curse
		    <option value="1101"
		  >Vampiric Drain
		    <option value="358"
		  >Vampiric Embrace
		    <option value="475"
		  >VampiricEmbraceEffect
		    <option value="2058"
		  >Vampyre Invis
		    <option value="2172"
		  >Vampyre Regeneration
		    <option value="2276"
		  >Vas Ren Slow
		    <option value="2277"
		  >Vas Ren Slow
		    <option value="1882"
		  >Veil of Elements
		    <option value="1064"
		  >Velium Chill of Al`Kabor
		    <option value="1744"
		  >Velium Poison
		    <option value="1566"
		  >Velium Shards
		    <option value="1452"
		  >Velocity
		    <option value="2687"
		  >Vengeance
		    <option value="2688"
		  >Vengeance
		    <option value="2689"
		  >Vengeance
		    <option value="2690"
		  >Vengeance
		    <option value="2691"
		  >Vengeance
		    <option value="2692"
		  >Vengeance
		    <option value="1434"
		  >Vengeance of Al'Kabor
		    <option value="1252"
		  >Vengeance of Root
		    <option value="1527"
		  >Vengeance of the Glades
		    <option value="434"
		  >Venom of the Snake
		    <option value="1991"
		  >Venomous Blast
		    <option value="1752"
		  >Verlekarnorm's Disaster
		    <option value="742"
		  >Verses of Victory
		    <option value="1398"
		  >Vexing Mordinia
		    <option value="1996"
		  >Vibrant Might
		    <option value="575"
		  >Vigilant Spirit
		    <option value="1038"
		  >Vigor
		    <option value="1965"
		  >Vigor of Zehkes
		    <option value="1539"
		  >Vilia`s Chorus of Celerity
		    <option value="735"
		  >Vilia`s Verses of Celerity
		    <option value="578"
		  >Vision
		    <option value="1492"
		  >Visions of Grandeur
		    <option value="1456"
		  >Vocarate: Air
		    <option value="1453"
		  >Vocarate: Earth
		    <option value="1455"
		  >Vocarate: Fire
		    <option value="1454"
		  >Vocarate: Water
		    <option value="370"
		  >Voice Graft
		    <option value="1381"
		  >Voice of the Berserker
		    <option value="1421"
		  >Voltaic Draught
		    <option value="873"
		  >Vortex
		    <option value="2398"
		  >Vortex of Horror
		    <option value="1507"
		  >Wake of Karana
		    <option value="1323"
		  >Wake of Tranquility
		    <option value="1809"
		  >Wakening Lands Gate
		    <option value="1181"
		  >Wakening Lands Portal
		    <option value="504"
		  >Walking Sleep
		    <option value="1189"
		  >Wandering Mind
		    <option value="247"
		  >Ward Summoned
		    <option value="217"
		  >Ward Undead
		    <option value="1945"
		  >Warder's Protection
		    <option value="1955"
		  >Warsong of the Vah Shir
		    <option value="1223"
		  >Wash of Waves
		    <option value="988"
		  >Wasp Poison
		    <option value="2286"
		  >Water Blast
		    <option value="2127"
		  >Water Elemental Form
		    <option value="2128"
		  >Water Elemental Form
		    <option value="2129"
		  >Water Elemental Form
		    <option value="945"
		  >WaterElementalAttack
		    <option value="998"
		  >WaterElementalStrike
		    <option value="1271"
		  >Wave of Cold
		    <option value="2138"
		  >Wave of Death
		    <option value="362"
		  >Wave of Enfeeblement
		    <option value="127"
		  >Wave of Fear
		    <option value="1261"
		  >Wave of Flame
		    <option value="1237"
		  >Wave of Healing
		    <option value="1272"
		  >Wave of Heat
		    <option value="2086"
		  >Wave of Revulsion
		    <option value="2317"
		  >Wave of Toxicity
		    <option value="2693"
		  >Wave of White Noise
		    <option value="1754"
		  >Waves of the Deep Sea
		    <option value="978"
		  >Weak Poison
		    <option value="40"
		  >Weaken
		    <option value="754"
		  >Weakening Poison I
		    <option value="1633"
		  >Weakening Poison II
		    <option value="1634"
		  >Weakening Poison III
		    <option value="1660"
		  >Weakening Poison IV
		    <option value="1827"
		  >Weakening Spray
		    <option value="180"
		  >Weakness
		    <option value="2694"
		  >Weather Rocket
		    <option value="546"
		  >West Gate
		    <option value="566"
		  >West Portal
		    <option value="302"
		  >Whirl till you hurl
		    <option value="876"
		  >Whirl untill you hurl
		    <option value="813"
		  >WhirlBolt
		    <option value="252"
		  >Whirling Wind
		    <option value="814"
		  >Whirlwind
		    <option value="1389"
		  >Wildfire
		    <option value="2695"
		  >Will of the Master
		    <option value="1282"
		  >Wimpy Healing
		    <option value="2696"
		  >Wind of Renewal
		    <option value="1481"
		  >Wind of Tashani
		    <option value="1518"
		  >Wind of the North
		    <option value="1519"
		  >Wind of the South
		    <option value="1486"
		  >Wind of Tishanian
		    <option value="1439"
		  >Winds of Gelid
		    <option value="1822"
		  >Winds of the Archmage
		    <option value="1383"
		  >Winged Death
		    <option value="508"
		  >Winter's Roar
		    <option value="2010"
		  >Wintercall
		    <option value="1331"
		  >Wizard Mastery
		    <option value="424"
		  >Wolf Form
		    <option value="1491"
		  >Wonderous Rapidity
		    <option value="415"
		  >Word Divine
		    <option value="2414"
		  >Word of Darkness
		    <option value="135"
		  >Word of Healing
		    <option value="134"
		  >Word of Health
		    <option value="230"
		  >Word of Pain
		    <option value="2697"
		  >Word of Passage
		    <option value="1305"
		  >Word of Redemption
		    <option value="1303"
		  >Word of Restoration
		    <option value="412"
		  >Word of Shadow
		    <option value="414"
		  >Word of Souls
		    <option value="413"
		  >Word of Spirit
		    <option value="1302"
		  >Word of Vigor
		    <option value="2085"
		  >Words of the Undead King
		    <option value="2149"
		  >Worm Infestation
		    <option value="2176"
		  >Wormcall
		    <option value="2314"
		  >Wormspore Stick
		    <option value="2433"
		  >Wounding Accel Mastery II
		    <option value="328"
		  >Wrath
		    <option value="726"
		  >Wrath of Al'Kabor
		    <option value="2403"
		  >Wrath of Fright
		    <option value="1708"
		  >Wrath of Nature
		    <option value="1187"
		  >Wrath of the Elements
		    <option value="2698"
		  >Wrath of the Wild
		    <option value="2001"
		  >Wrathful Strike
		    <option value="774"
		  >Wurm Blaze
		    <option value="2373"
		  >Wurm Knock
		    <option value="820"
		  >WurmBreath
		    <option value="209"
		  >Yaulp
		    <option value="42"
		  >Yaulp II
		    <option value="43"
		  >Yaulp III
		    <option value="1316"
		  >Yaulp IV
		    <option value="684"
		  >Yegoreff`s Animation
		    <option value="1964"
		  >Yekan's Quickening
		    <option value="1962"
		  >Yekan's Recovery
		    <option value="922"
		  >Ykesha
		    <option value="526"
		  >Yonder
		    <option value="2699"
		  >Yttric Ventus
		    <option value="2700"
		  >Zephyr of Brell
		    <option value="1895"
		  >Zevfeer's Theft of Vitae
		    <option value="2360"
		  >Zruk Breath
		    <option value="2361"
		  >Zruk Tap
		    <option value="1505"
		  >Zumaik`s Animation
		</select>
	      </td>
	    </tr>
	    <tr>
              <td>
                Skill Mod :
              </td>
              <td>
                <select name="skill[]"><option value="any"
                >Any<option value=78>1H Blunt<option value=80>1H Slashing<option value=79>2H Blunt<option value=83>2H Piercing<option value=81>2H Slashing<option value=1>Abjuration<option value=2>Alchemy<option value=3>Alcohol Tolerance<option value=4>Alteration<option value=5>Apply poison<option value=85>Archery<option value=77>Backstab<option value=6>Baking<option value=41>Bash<option value=7>Begging<option value=8>Bind Wound<option value=9>Blacksmithing<option value=10>Block<option value=90>Brass Instrument<option value=11>Brewing<option value=12>Channeling<option value=91>Conjuration<option value=36>Defense<option value=44>Disarm<option value=13>Disarm Traps<option value=14>Divination<option value=15>Dodge<option value=43>Double Attack<option value=50>Dragon Punch<option value=42>Dual Wield<option value=49>Eagle Strike<option value=16>Evocation<option value=48>Feign Death<option value=17>Fishing<option value=18>Fletching<option value=51>Flying Kick<option value=19>Foraging<option value=84>Hand to Hand<option value=20>Hide<option value=0>Instant<option value=21>Instill Doubt<option value=22>Jewelry<option value=37>Kick<option value=23>Meditate<option value=45>Mend<option value=38>Offense<option value=24>Parry<option value=89>Percussion Instrument<option value=25>Picklock<option value=82>Piercing<option value=26>Poison Making<option value=27>Pottery<option value=32>Research<option value=28>Riposte<option value=46>Round Kick<option value=29>Safe Fall<option value=30>Sense Heading<option value=99>Singing<option value=39>Slam<option value=31>Sneak<option value=92>Specialize Abjuration<option value=93>Specialize Alteration<option value=94>Specialize Channeling<option value=97>Specialize Conjuration<option value=95>Specialize Divination<option value=96>Specialize Evocation<option value=98>Spell Research<option value=87>Stringed Instrument<option value=33>Swimming<option value=34>Tailoring<option value=40>Taunt<option value=86>Throwing<option value=47>Tiger Claw<option value=35>Tinkering<option value=88>Wind Instrument
		</select>&nbsp;&nbsp; Sup à : <input type="text" name="val_skill" size="3" value="0">
              </td>
            </tr>
            <tr>
	      <td>
	        Bane : 
	      </td>
	      <td>
	        <select name="bane[]" onChange="javascript:document.choix.submit(this.form)"><option value="any"
	        >Any<option value=1>Magic<option value=2>Fire<option value=3>Cold<option value=4>Poison<option value=5>Disease<option value=6>Bane
		</select>
		
              </td>
            </tr>
          <!--
          </table>
          <table border="0" cellpadding="0" cellspacing="3" align="center" valign="center" CLASS="main_corps">
          -->
            <tr>
              <td colspan="2">
                <hr>
              </td>
            </tr>
            <tr>
              <td>
                Stat 1 :
              </td>
              <td>
                <select name="stat1[]"><option value="any"
                >Any<option value=ac>Ac<option value=dmg>Dmg<option value=dly>Dly<option value=str>Strength<option value=sta>Stamina<option value=agi>Agility<option value=dex>Dexterity<option value=wis>Wisdom<option value=intel>Intelligence<option value=cha>Charisma<option value=svm>Magic Resist<option value=svf>Fire Resist<option value=svc>Cold Resist<option value=svp>Poison Resist<option value=svd>Disease Resist<option value=hp>Hp<option value=mana>Mana<option value=wt>Poids<option value=wtreduc>Reduc Poids
                </select>&nbsp;&nbsp;<input type="radio" name="stat_radio1" value=">="
		 checked
		><b>></b> à : <input type="radio" name="stat_radio1" value="<="
		
		><b><</b> à : <input type="radio" name="stat_radio1" value="="
		
                ><b>=</b> à : <input type="text" name="stat_val1" size="3" value="">
	      </td>
	    </tr>

            <tr>
              <td>
                Stat 2 :
              </td>
              <td>
                <select name="stat2[]"><option value="any"
                >Any<option value=ac>Ac<option value=dmg>Dmg<option value=dly>Dly<option value=str>Strength<option value=sta>Stamina<option value=agi>Agility<option value=dex>Dexterity<option value=wis>Wisdom<option value=intel>Intelligence<option value=cha>Charisma<option value=svm>Magic Resist<option value=svf>Fire Resist<option value=svc>Cold Resist<option value=svp>Poison Resist<option value=svd>Disease Resist<option value=hp>Hp<option value=mana>Mana<option value=wt>Poids<option value=wtreduc>Reduc Poids
                </select>&nbsp;&nbsp;<input type="radio" name="stat_radio2" value=">="
		 checked
		><b>></b> à : <input type="radio" name="stat_radio2" value="<="
		
		><b><</b> à  : <input type="radio" name="stat_radio2" value="="
		
                ><b>=</b> à : <input type="text" name="stat_val2" size="3" value="">
	      </td>
	    </tr>

            <tr>
              <td>
                Stat 3 :
              </td>
              <td>
                <select name="stat3[]"><option value="any"
                >Any<option value=ac>Ac<option value=dmg>Dmg<option value=dly>Dly<option value=str>Strength<option value=sta>Stamina<option value=agi>Agility<option value=dex>Dexterity<option value=wis>Wisdom<option value=intel>Intelligence<option value=cha>Charisma<option value=svm>Magic Resist<option value=svf>Fire Resist<option value=svc>Cold Resist<option value=svp>Poison Resist<option value=svd>Disease Resist<option value=hp>Hp<option value=mana>Mana<option value=wt>Poids<option value=wtreduc>Reduc Poids
                </select>&nbsp;&nbsp;<input type="radio" name="stat_radio3" value=">="
		 checked
		><b>></b> à : <input type="radio" name="stat_radio3" value="<="
		
		><b><</b> à  : <input type="radio" name="stat_radio3" value="="
		
                ><b>=</b> à : <input type="text" name="stat_val3" size="3" value="">
	      </td>
	    </tr>

	    <tr>
              <td>
                Ordonner par :
              </td>
              <td>
                <select name="order">
		<option value="nom"
		
		>Nom
		<option value="ac"
		
		>Ac
		<option value="ratio"
		
		>Ratio
		<option value="dmg"
		
		>Dmg
		<option value="dly"
		
		>Dly
		<option value="str"
		
		>Strength
		<option value="sta"
		
		>Stamina
		<option value="agi"
		
		>Agility
		<option value="dex"
		
		>Dexterity
		<option value="wis"
		
		>Wisdom
		<option value="intel"
		
		>Intelligence
		<option value="cha"
		>Charisma
		<option value="svm"
		
		>Magic Resist
		<option value="svf"
		
		>Fire Resist
		<option value="svc"
		
		>Cold Resist
		<option value="svp"
		
		>Poison Resist
		<option value="svd"
		
		>Disease Resist
		<option value="hp"
		
		>Hp
		<option value="mana"
		
		>Mana
		<option value="wt"
		
		>Poids
		<option value="wtreduc"
		
		>Reduc Poids
                </select>
              </td>
            </tr>
            <tr>
              <td colspan="2">
                <hr>
              </td>
            </tr>   
            <tr>
              <td>
                Nom :
              </td>
              <td>
                <input type="text" name="nom" value="">
              </td>
           </tr>
           <tr>
	     <td ALIGN="center" COLSPAN="2">
	       <input type="submit" name="modifier" value="Recherche" onClick="javascript:document.choix.submit(this.form)">
	     </td>
           </tr>
	 </table>
       </form>
       </td>
     </tr>
   </table>
 </td>
</table>


    
    </td>
    <td width="5"><img src="img/spacer.gif"></td>
    <td width="122" valign="top" align="center">
      <table width="122" CLASS="table_bord" cellpadding="1" cellspacing="1">
	<td>
	  <table width="122" cellspacing="0" cellpadding="0">
	    <tr>
	      <td CLASS="menu_entete">Sorts</td>
	    </tr>
	    <tr>
	      <td CLASS="menu_corps"><img src="img/puce.png">&nbsp;<a CLASS="menu" href="index.php?page=spells.php&classe=brd">Barde</a></td>
	    </tr>
	    <tr>
	      <td CLASS="menu_corps"><img src="img/puce.png">&nbsp;<a CLASS="menu" href="index.php?page=spells.php&classe=bst">Beastlord</a></td>
	    </tr>
	    <tr>
	      <td CLASS="menu_corps"><img src="img/puce.png">&nbsp;<a CLASS="menu" href="index.php?page=spells.php&classe=clr">Clerc</a></td>
	    </tr>
	    <tr>
	      <td CLASS="menu_corps"><img src="img/puce.png">&nbsp;<a CLASS="menu" href="index.php?page=spells.php&classe=dru">Druide</a></td>
	    </tr>
            <tr>
	      <td CLASS="menu_corps"><img src="img/puce.png">&nbsp;<a CLASS="menu" href="index.php?page=spells.php&classe=enc">Enchanteur</a></td>
	    </tr>
	    <tr>
	      <td CLASS="menu_corps"><img src="img/puce.png">&nbsp;<a CLASS="menu" href="index.php?page=spells.php&classe=mag">Magicien</a></td>
	    </tr>
	    <tr>
	      <td CLASS="menu_corps"><img src="img/puce.png">&nbsp;<a CLASS="menu" href="index.php?page=spells.php&classe=nec">Nécromancien</a></td>
	    </tr>
	    <tr>
	      <td CLASS="menu_corps"><img src="img/puce.png">&nbsp;<a CLASS="menu" href="index.php?page=spells.php&classe=pal">Paladin</a></td>
	    </tr>
	    <tr>
	      <td CLASS="menu_corps"><img src="img/puce.png">&nbsp;<a CLASS="menu" href="index.php?page=spells.php&classe=rng">Ranger</a></td>
	    </tr>
	    <tr>
	      <td CLASS="menu_corps"><img src="img/puce.png">&nbsp;<a CLASS="menu" href="index.php?page=spells.php&classe=shd">Shadowknight</a></td>
	    </tr>
	    <tr>
	      <td CLASS="menu_corps"><img src="img/puce.png">&nbsp;<a CLASS="menu" href="index.php?page=spells.php&classe=shm">Shaman</a></td>
	    </tr>
	    <tr>
	      <td CLASS="menu_corps"><img src="img/puce.png">&nbsp;<a CLASS="menu" href="index.php?page=spells.php&classe=wiz">Wizard</a></td>
	    </tr>
	    <tr>
	      <td CLASS="menu_corps"><img src="img/puce.png">&nbsp;<a CLASS="menu" href="index.php?page=spells.php&classe=misc">Effets Divers</a></td>
	    </tr>
	    <tr>
	      <td CLASS="menu_corps" align="center">------</td>
	    </tr>
	    <tr>
	      <td CLASS="menu_corps" align="center"><a CLASS="menu" href="index.php?page=spell_search.php">Recherche</a></td>
	    </tr>
	  </table>
	</td>
      </table>
    </td>
  </tr>
</table>

</center>

</body>
</html>
