...
document.write("<style type='text/css'>.pointer{cursor: "+cursor+";}</style>");
...
function set_forms()
{
	...
	{
		if(typeof(document.forms[i].send_to)!='undefined')
		{
			var module=document.forms[i].send_to.value;
			document.forms[i].action='/components/'+module+'.jsp';
			document.forms[i].method='POST';
		}
	}
}

function build_select(nom,obj,size,disabled,multiple,on_change)
{
	document.write('<select class=input name='+nom+' size='+size);
	if(on_change!=null)
		document.write(" onchange='"+on_change+"'");
