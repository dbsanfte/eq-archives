<HTML>
<HEAD>
<TITLE>The Shadowknight Den</TITLE>

</HEAD>

<BODY bgcolor="BLACK" text="white" topmargin=0 leftmargin=0 marginwidth=0 marginheight=0 link="5555FF" vlink="FF55FF" alink="FF55FF" background="http://www.albrandes.com/skden/images/back.jpg">
<table width="100%" border="0" cellspacing="0" topmargin=0 leftmargin=0 marginwidth=0 marginheight=0>
 <TR>
  <TD bgcolor="black">
<img src="http://www.albrandes.com/skden/images/skden_logo.jpg">
</td>
</tr>
</table>
<!--
<div align="center">
 <h1>The Shadowknight Den</h1></div>

<div align="center">
<table width="100%">
 <TR>
  <TD bgcolor="black">
   <img src="/skden/images/skden_logo.jpg" alt="Shadowknight logo">
  </tD>
 </tr>
</table>
</div>
-->
<p></p>

<!---------------------------->
<TABLE background="/skden/images/back.jpg">
 <TR valign="top">
  <TD>
   <TABLE WIDTH=200>
    <TR>
     <TD width="100%">
      <div align="Left" valign="top">
      <b>Home...</b><br>
      &nbsp;&nbsp;&nbsp;<a href="/skden/index.asp">Home</a><br>
      &nbsp;&nbsp;&nbsp;<a href="/skden/archives.asp">Archives</a><br>

      <BR>
      <b>Quests...</b><br>
      &nbsp;&nbsp;&nbsp;<a href="/skden/quests/questlist.asp">General Quests</a><br>
      &nbsp;&nbsp;&nbsp;<a href="http://everquest.allakhazam.com/db/quest.html?quest=766">Epic walkthrough</a><br>
      &nbsp;&nbsp;&nbsp;<a href="/skden/epic/checklist.asp">Epic Checklist</a><br>
      &nbsp;&nbsp;&nbsp;<a href="/skden/epic/Walkthrough.asp">Short epic walkthrough</a><BR>
      <BR>
      <B>Articles...</b><BR>
      &nbsp;&nbsp;&nbsp;<a href="/skden/articles/charisma.asp">Charisma and Fear</a><br>
      &nbsp;&nbsp;&nbsp;<a href="/skden/articles/starting.asp">Starting info</a><br>
      &nbsp;&nbsp;&nbsp;<a href="/skden/articles/FAQ.asp">FAQ</a><br>
      &nbsp;&nbsp;&nbsp;<a href="/skden/articles/armorsets.asp">Armor Sets</a><br>
      &nbsp;&nbsp;&nbsp;<a href="/skden/articles/sleeper.asp">The Sleeper</a><br>
      &nbsp;&nbsp;&nbsp;<a href="/skden/articles/ldon.asp">LDoN Lore</a><br>
     <BR>
      <B>Skills...</b><BR>
      &nbsp;&nbsp;&nbsp;<a href="/skden/articles/HT.asp">Harm Touch Guide</a><br>
      &nbsp;&nbsp;&nbsp;<a href="/skden/articles/skill_caps.asp">Learnable skill caps</a><br>	
      &nbsp;&nbsp;&nbsp;<a href="/skden/articles/disciplines.asp">Disciplines</a><br>
      <br>
      <B>Misc...</b><BR>
      &nbsp;&nbsp;&nbsp;<a href="http://eq.crgaming.com/spells/spells.asp?Class=SK">Spells</a><br>
      <BR>
      <B>Links...</b><BR>
      &nbsp;&nbsp;&nbsp;<a href="http://shadowknight.org">Shadowknight.org</a><br>
      &nbsp;&nbsp;&nbsp;<a href="http://shadow-knight.org/forums">Shadow-knight.org</a><br>
      <BR>
      <B>Contact...</b><BR>
      &nbsp;&nbsp;&nbsp;<a href="mailto:rozzl@albrandes.com">Rozzl</a><br>
      <BR><BR>
      <table border=1 width="100%"><tr>
       <TD align="left">Hits</td><td align="right"><B>129994</b></td></tr></table>
      </div>
     </TD>
    </TR>
   </TABLE>
  </TD>
  <TD width="100%">
   <TABLE width="100%">
    <TR valign="top">
     <TD width="100%">
      <div align="Left">
<!---------------------------->

<div align="center"><h2>Shadow Knight Skill caps</h2></div>

Note:  Most non-trade skills scale with your character throughout it's entire development.  From levels 1-40, you can receive about 5 points per skill per level up to a certain cap (the cap is usually (your level + 1)*5) or the skill cap, whichever is lower, but there are some differences.  Also, from 50 to 60 skill caps increment per level.  You dont't go from a skill of 200 to 250 in one level...<br>
<br>
Anyway, most of these are hopefully accurate guesses and some I have no idea about whatsoever.  If you have any information about skill caps at level 50, 60, or 65 that I don't have, please e-mail me with that information so I can add it to this list.  Places where there is a (?) are places where I guessed at the cap based on reason.  If you can confirm or deny any of my guesses, I'll change them appropriately, thanks for any input you can offer.<br>
<br>

<table border="1">
	<tr>
		<th>Skill</th>
		<th>Cap at 50</th>
		<th>Cap at 60</th>
		<th>Cap at 65</th>
	</tr>
	<Tr>
		<th colspan="4">
		Combat related skills
		</th>
	</tr>
	<tr>
		<td>1 Hand Blunt</td>
		<td>200</td>
		<td>225</td>
		<td>??</td>
	</tr>
	<tr>
		<td>1 Hand Slashing</td>
		<td>200</td>
		<td>225</td>
		<td>??</td>
	</tr>
	<tr>
		<td>2 Hand Blunt</td>
		<td>200</td>
		<td>225</td>
		<td>??</td>
	</tr>
	<tr>
		<td>2 Hand Slashing</td>
		<td>200</td>
		<td>225</td>
		<td>??</td>
	</tr>
	<tr>
		<td>Archery</td>
		<td>75</td>
		<td>75</td>
		<td>??</td>
	</tr>
	<tr>
		<td>Bash (Slam)</td>
		<td>175</td>
		<td>200</td>
		<td>??</td>
	</tr>
	<tr>
		<td>Bind Wound</td>
		<td>150</td>
		<td>200(?)</td>
		<td>??</td>
	</tr>
	<tr>
		<td>Defense</td>
		<td>210</td>
		<td>252(?)</td>
		<td>??</td>
	</tr>
	<tr>
		<td>Disarm</td>
		<td>70</td>
		<td>70</td>
		<td>70(?)</td>
	</tr>
	<tr>
		<td>Dodge</td>
		<td>125</td>
		<td>155(?)</td>
		<td>??</td>
	</tr>
	<tr>
		<td>Double Attack</td>
		<td>200</td>
		<td>235(?)</td>
		<td>??</td>
	</tr>
	<tr>
		<td>Hand to Hand</td>
		<td>100</td>
		<td>100</td>
		<td>100(?)</td>
	</tr>
	<tr>
		<td>Hide</td>
		<td>75</td>
		<td>75</td>
		<td>75(?)</td>
	</tr>
	<tr>
		<td>Offense</td>
		<td>200</td>
		<td>230(?)</td>
		<td>??</td>
	</tr>
	<tr>
		<td>Parry</td>
		<td>175</td>
		<td>205(?)</td>
		<td>??</td>
	</tr>
	<tr>
		<td>Piercing</td>
		<td>200</td>
		<td>225</td>
		<td>??</td>
	</tr>
	<tr>
		<td>Riposte</td>
		<td>175</td>
		<td>200(?)</td>
		<td>??</td>
	</tr>
	<tr>
		<td>Taunt</td>
		<td>180</td>
		<td>??</td>
		<td>??</td>
	</tr>
	<tr>
		<th colspan="4">
		Magic related skills
		</th>
	</tr>
	<tr>
		<td>All casting skills</td>
		<td>235</td>
		<td>235(?)</td>
		<td>235(?)</td>
	</tr>
	<tr>
		<td>Channelling</td>
		<td>200</td>
		<td>200(?)</td>
		<td>200(?)</td>
	</tr>
	<tr>
		<td>Meditate</td>
		<td>185</td>
		<td>235(?)</td>
		<td>235(?)</td>
	</tr>
	<tr>
	<th colspan="4">
	Trade related skills (note, AA points must be used for more than one skill to be above 200)<br>
	Trade skills are not level restrected.  i.e. a level 1 person can get to a skill of 250 in one craft<br>
	Only gnomes may gain skill in tinkering, all other races have a permanent skill of 0.  Gnomes may not exceed 50 skill in tinkering until level 16 or above.
	</th>
	</tr>
	<tr>
		<td>All trade skills</td>
		<td>250</td>
		<td>250</td>
		<td>250</td>
	</tr>
	<tr>
	<th colspan="4">
	All language skills<br>
	Language skills are not level based
	</th>	
	</tr>
		<tr>
		<td>All languages</td>
		<td>100</td>
		<td>100</td>
		<td>100</td>
	</tr>
	<tr>
	<th colspan="4">
	Innate skills
	</th>
	</tr>
	<tr>
		<td>Alcohol Tolerance</td>
		<td>200</td>
		<td>200</td>
		<td>200(?)</td>
	</tr>
	<tr>
		<td>Begging</td>
		<td>200</td>
		<td>200</td>
		<td>200(?)</td>
	</tr>
	<tr>
		<td>Fishing</td>
		<td>200</td>
		<td>200</td>
		<td>200(?)</td>
	</tr>
	<tr>
		<td>Foraging (Iksar)</td>
		<td>50</td>
		<td>50</td>
		<td>50(?)</td>
	</tr>
	<tr>
		<td>Sense Heading</td>
		<td>200</td>
		<td>200</td>
		<td>200(?)</td>
	</tr>
	<tr>
		<td>Swimming</td>
		<td>200</td>
		<td>200</td>
		<td>200(?)</td>
	</tr>
</table>

<!-------------------------------->
      </div>
     </TD>
    </TR>
   </TABLE>
  </TD>
 </TR>
</TABLE>
<!--------------------------------->


</BODY>
</HTML>