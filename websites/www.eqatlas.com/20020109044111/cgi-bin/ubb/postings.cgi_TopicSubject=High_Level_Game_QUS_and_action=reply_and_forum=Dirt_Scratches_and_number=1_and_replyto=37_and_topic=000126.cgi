...
<td>
<TEXTAREA NAME="ReplyMessage" ROWS=10 COLS=45 WRAP="VIRTUAL">[QUOTE]Originally posted by chippers:
[B]It's a shame to see twenty mid level mages all in oot robes. My point, make a lot of items for the players to covet. Enough so that every good spot in the game isn't camped 24/7 by some 50 plus. If there were enough swords,robes,shields and other game rewards the players could go for a 'look' that they want, instead of twenty casters all in oot robes. Is it so hard to build an item? I don't think so. Yet most of the classes all look the same because everyone knows that a ranger has to have 'weapon'. Why not make two three even a dozen swords that are good and serve the same purpose with small diferances, ie lower delay, better stat or stats and diferant graphic. I want to see more than two robes, and this is for all levels. Let level ten paladins want the Ghoul Killer Extrodinare, and later want the Mithral Spectre Wacker. Make twety dieferant kinds of each, connect them to quests for that level, connect them to mobs for that level and let the plaldins look diferant from each other because one prefers the dex stat on his fav where the other wants the str plus on the one he like. The designers are always so busy making the next expansion when the making of rewards requires so little. Let there be rewards in the from of loot for all levels. Get creative and give up options. All players want more options. Not just the high level or the low. Why can't Verant do theirjob which is to provide us with adventure and 
the fun that this would provide.[/B][/QUOTE]
