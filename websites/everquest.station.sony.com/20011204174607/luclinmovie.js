...
/*
 * Pops-up a new window that takes over the whole screen
 */
function fullscreenPopUp(url, name)
{
	window.open(url, name,'fullscreen=yes,scrollbars=no,screenX=0,screenY=0,outerHeight=' + screen.availHeight + ',outerWidth=' + screen.availWidth);
}

/*
 * If the luclinFlash cookie is not set
 *		Set the cookie
 *		Pop Up the new window
 *		redirect this page
 */
function initLuclinFlash()
{
	luclinFlashMovie=getCookie("luclinFlashMovie");
	if (luclinFlashMovie!="set")
	{
		setCookie("luclinFlashMovie", "set", "never");
		fullscreenPopUp("luclinmovie.htm", "Flash");
		document.location="holder.htm"
	}
}