...
LogSplit 1.9 - EQ Log File Splitter
...
7. (Optional) Hit YES to delete the input file(s).
...
It can also extract the character name from the file name, if it's in eitheer
the Eqlog_XX_Charname.txt format, or a file produced by this program.
...
It also looks at the "Soandso saved" messages.  If it was unable to extract a
character name from the file name, and if it finds a "saved" message for a
different character than one encountered previously, it will abort.  

The "saved" check may be bypassed by unchecking the "Sanity Check" option.
