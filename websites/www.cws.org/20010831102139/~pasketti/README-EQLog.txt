...
The "Rerun" button will rerun the report with the files you selected.
It may be faster to reload the data file than it would be to rerun the whole
report.
...
Use Exceptions File 
  ...
  written to a file named LogExceptions.txt, in the same directory as the
  program.
...
Note:  DO NOT DELETE YOUR LOGS JUST BECAUSE YOU HAVE A DATA FILE.  I may add
new features to the program in the future.  This may change the format of the
data file, which means you will have to recreate it.  If the format changes,
I will NOT be expending any effort in making it backward compatible with
previous versions.  You have been warned.
...
Verant has fixed the bug, so this is written to the log file correctly.  Yay Verant!
...
to realistically try to track them.  However, you do get a message when the
...
DPS		Damage Per Second - More on this below
Min		Minimum hit
Max		Maximum hit

The following are repeated for Critical Hits, Crippling Blows,
  Deadly Strikes, and Thunderous Kicks.
XXXX Dmg	Total damage done by that attack type
XXXXs		How many times you hit
Avg XXXX	Average hit
Min XXXX	Minimum hit
Max XXXX	Maximum hit
XXXX Dmg %	Percent of total damage done by this attack
XXXX Hit %	Percent of total hits done by this attack

Misses		How many times you missed
...
Dmg Shield	How many times you were deflected by a damage shield
Dmg Shield %	Mob damage shield percentage
Invulnerable	How many times you tried to hit an invulnerable mob
Invulnerable %	Mob invulnerable percentage

Dodges, Parries, Ripostes, Blocks, Dmg Shield, and Invulnerable are not 
included in the miss percentage.

Non-melee damage is listed, but is not included in the damage total at the
end.

Right-click to select histograms by hits, hit damage, and hit delay.

The Hits histogram shows you the hit damage, and how many times you hit for
that damage, and a handy bar graph.
...
seconds have elapsed since the previous hit.  Zero-delay hits are ignored. 
Delays longer than 30 seconds are ignored.

NOTES ON DAMAGE PER SECOND

Initially, all the damage per second amounts will be blank.  You have to
give the program a hint first.  Why?

There really isn't any clean way to calculate this.  You can't isolate
off-hand, double attack, etc, to any degree of reliability.

I solve this by ignoring it.  Your damage per second is how much damage of
that type you dealt out, total, no matter how you did it.

But then you have a problem of detecting when the battle starts so you'll
know when to start the timer.  "Auto-attack on" may or may not be reliable.

So right-click on the report and select "Hit Delay Histogram".

You'll see a bar graph of the number of hits you had versus how many seconds
have elapsed since the previous hit.  You'll see an early peak, then a
dropoff.  For example, my warrior's delay histogram started like this:
...
We want to pick the delay period that will encompass all the hits in a
battle, while eliminating the delay between battles.  In this example, it's
clearly 5.  To select it, DOUBLE CLICK ON IT.  You'll get a message telling
you the delay for that character and damage type has been set.

Set any other damage types you may want to.

Then rerun the report via the handy Rerun button.
...
Min		Minimum hit
Max		Maximum hit
Misses		How many times it missed
...
Dmg Shield	How many times mob was deflected by your damage shield
Dmg Shield %	Your damage shield percentage
...
Dodges, Parries, Ripostes, Blocks, Dmg Shield, and Invulnerable are not included
in the miss percentage.

Right-click to select histograms by hits, hit damage, and hit delay.

The Hits histogram shows you the hit damage, and how many times it hit for
that damage, and a handy bar graph.
...
Dodges, Parries, Ripostes, Block, Dmg Shield, and Invulnerable are not included
in the miss percentage.
...
Average		Average damage per hit (Total / Hits)
DPS		Damage Per Second - See the "Damage Output" report
Min		Minimum hit
Max		Maximum hit
...
Block %		Mob block percentage
Dmg Shield	How many times pet was deflected by a damage shield
Dmg Shield %	Mob damage shield percentage
Invulnerable	How many times pet tried to hit an invulnerable mob
Invulnerable %	Mob invulnerable percentage

Dodges, Parries, Ripostes, Block, Dmg Shield, and Invulnerable are not 
included in the miss percentage.

Right-click to select histograms by hits, hit damage, and hit delay.

The Hits histogram shows you the hit damage, and how many times the pet hit for
that damage, and a handy bar graph.
...
Dmg Shield	How many times mob was deflected by pet's damage shield
Dmg Shield %	Pet damage shield percentage
Invulnerable	How many times mob tried to hit pet when invulnerable
Invulnerable %	Pet invulnerable percentage

Dodges, Parries, Ripostes, Block, Dmg Shield, and Invilnerable are not included
in the miss percentage.
...
Track the items for sale at or sold to merchants.  This can be helpful locating
...
This is strictly stuff that was bought from merchants, that you either bought,
or clicked on to find out what it was or how much it cost.
