...
/*
function pmLengthChecked() {
	var postform = document.forms[0];
	if (postform.body.value.trim().length > 4900){
		alert("Max. 4900 ("+postform.body.value.trim().length+")");
		return false;
	}
	else {
		return true;
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" onresize="ResizeTextArea()">
  <form method="post" action="pmsend.asp" name="postform" onsubmit="return validateForm(this)">
	<tr> 
	  <td bgcolor="004580"> 
...
if (theForm.__body) theForm.__body.value = ".+;;==|.length<4900"
if (theForm._body) theForm._body.value = "Please enter something into body and subject field!;;Max. 4900"
