...
<P ALIGN="center"><STRONG><BR><STRONG>General Tips for Crashing or Freezing During Gameplay</STRONG></STRONG></P>  <P>&nbsp;</P>  <P><STRONG>Quick Suggestions to Remedy Crashing or Freezing:</STRONG></P>  <P>&nbsp;</P>  <P>First, if you have not already done so, you will want to make sure you
  have the latest drivers for your video and sound card. Go to the Support page
  at &quot;<A HREF="../../../../../support/tech_support.jsp">http://eqlive.station.sony.com/support/tech_support.jsp</A>&quot;
  and view the Supported Video and Sound card sections for a list of supported
  equipment and the latest driver information.</P>  <P>If the problem persists, try opening the case of your computer and
  directing a fan onto the motherboard, full blast. If the problem is heat
  related, this will be a good test. This may sound strange, but due to the
  intense CPU and Video Card usage of the game, the temperature inside your
  computer case will be significantly higher. Many have reported success after
  adding fans to their systems.</P>  <P>Alternatively, there are also known resource conflicts between
  voice-capable modems and sound cards. Aside from causing crashes, this conflict
  can also cause the Voice Modem to &quot;steal&quot; sound from your Sound
  Card, effectively turning it off in the game. Go to the &quot;Sound, Video,
  and Game Controllers&quot; section of the Device Manager (right click on your
  &quot;My Computer&quot; icon and choose &quot;Properties&quot;). You will see
  a listing for &quot;Wave Device for Voice Modem&quot; or something similar.
  Double click on this entry and mark the &quot;Disable in this Hardware
  Profile&quot; option. Most people never use the Voice functions on their
  modems, so disabling this will not change the way you connect to your
  Internet provider.<BR><BR>Another test is to temporarily disable your sound in EverQuest. In order to
  do this, log into&nbsp;EverQuest and click Options on the first menu. On the
  last page of the options you will see Sound. All you need to do is uncheck
  this and it will be disabled till you recheck it.<BR><BR>Also, make sure there are no tasks running in the background while EQ is
  running. After a fresh reboot, hit Ctrl-Alt-Del to bring up a list of running
  tasks. &quot;End Task&quot; on everything except Explorer, Systray, and
  RNAApp (if listed), then connect to the Internet and start the game.</P>  <P>Make sure that you have at least 500 Megs of free hard-drive space and
  that Windows is managing your virtual memory (if you haven't manually changed
  this option then windows is managing your virtual memory and you'll just want
  to make sure you have that much free space).</P>  <P>Finally, try playing on the Test Server. The Test Server is where new
  program code is tested, before it is used on the regular playing servers. In
  order to play on the Test Server, you will need to change the Environment
  option in the Advanced section of the patch program. The patch process will
  download some files, and then you will be able to log on using your
  established Station Name and Password. If the problem does not occur while on
  the test server, then the problem may be a known issue that has been worked
  on, and the new code will eventually migrate to the other servers in a future
  patch.</P>  <P>&nbsp;</P>  <P ALIGN="center"><STRONG>Do you have an Overclocked
  CPU?</STRONG></P>  <P>There are known problems with overclocked systems. Due to the load our
  program puts on the audio and video systems of your machine, DMA transfer
  rates must be maintained precisely at the industry standard, 4.77 MHz. Any
  modifications to the clock speed of the CPU may affect critical internal
  timings on the motherboard, including DMA. If the CPU is a Celeron, it can be
  particularly sensitive. Overclocking your CPU can definitely cause crashes in
  EverQuest.</P>  <P>&nbsp;</P>  <P ALIGN="center"><STRONG>Conflicts with Hardware
  sharing IRQs</STRONG></P>  <P>Due to the major patches in November of 2000, the EverQuest client has
  changed dramatically. EverQuest now demands full time attention from both the
  video and sound cards.</P>  <P>Windows has a built in feature called &quot;IRQ sharing&quot; that allows
  Windows to have multiple devices on the same IRQ without problems. When it
  does this, Windows sets up a queue for each IRQ, so that the devices take
  turns using the IRQ. This works fine for Windows because you never need full
  time attention from each device. However, this causes a problem with the new
  EverQuest client, because the other devices, using the same IRQ, never get to
  do anything. In cases with the video and/or sound card being shared with
  other devices, systems will experience connection problems, crashes, or
  lockups.</P>  <P>Your problem may stem from IRQ sharing on the machine in question. To
  check for this, right click the &quot;My Computer&quot; icon on your desktop
  and choose &quot;Properties&quot; from the menu. Next, choose the
  &quot;Device Manager&quot; tab and double click the &quot;Computer&quot;
  entry at the top of the device list. This should bring up the IRQ Resource
  window. Start looking for IRQs that are shared. Make note of the IRQs being
  used by main devices such as sound, video, and network adapters. If these are
  shared, besides having an IRQ holder, you may experience some problems. You
  will need to consult a computer technician for instructions on how to get
  this resolved.</P>  <P>If you are running Windows XP or Windows 2000 this will not be a problem.<BR>&nbsp;</P>  <P ALIGN="center"><STRONG>Problems Caused by Old
  Sound or Video Card Drivers</STRONG></P>  <P>Some problems can be caused by old drivers still present in your system.
  Usually, these drivers will not display themselves in the normal Windows
  mode, so you have to go into Safe Mode to remove them.</P>  <P>Once you get into Safe Mode, right click on My Computer, left click on
  Properties, select the Device Manager tab, and then left click the plus(+)
  button next to &quot;Display Adapters&quot;. If there are any listings for
  cards that you no longer have, or multiple entries for your current video
  card, highlight and remove them. Do the same for the &quot;Sound, Video and
  Game Controllers&quot; section to remove inactive and duplicate audio card
  entries. Finally, restart your computer and see if the game runs correctly.</P>  <P>If you are still having problems after visiting this support section, you
  can contact our Technical Support Department through <A HREF="mailto:eqtech@soe.sony.com"><STRONG>email</STRONG></A>.</P>
