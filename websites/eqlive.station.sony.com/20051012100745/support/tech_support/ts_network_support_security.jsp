...
<P ALIGN="center"><STRONG><BR><STRONG>Security Alert/Hack Attempt</STRONG></STRONG></P>  <P>The EverQuest servers and their networks are access-controlled, secured,
  ...
  be on any UDP port greater than 1024, and during the course of the game a
  client PC will regularly connect to different servers and different ports.
  When zones are restarted, they will often come up on different ports than
  they were previously on.</P>  <P>The EverQuest game is also very aggressive when attempting to recover from
  ...
  servers will retransmit packets at an increasing packet rate for up to 30 seconds.
  At the 10th or 20th second, this rate can be quite high and appear to be a
  flood or storm of traffic.</P>  <P>Most personal firewall software (e.g. ZoneAlarm, BlackIce, Norton Personal
  ...
  false alarms are:</P>  <UL>   <LI>&quot;Default Block Bla
       ...
       EverQuest are TCP servers running on port 7000. When you start one of
       our games that uses a patcher, your client PC will make a connection to
       one of our many patchers at port 7000, and our patchers will send
       updates back to you from port 7000. In order to optimize the patching
       process, we are using load-balancing network hardware, and have
       distributed the patchers among different geographic locations. As an
       artifact of that optimization, there are times when patching the client
       ...
       for a few seconds. At that time, your firewall software doesn't expect
  ...
  network. It is up to the application to decide if it wants to perform it's
  own data integrity. This allows applications to have more control over their
  networking, and lightens the load on the operating system.</P>
