...
<P ALIGN="center"><STRONG>Firewall/Proxy Information</STRONG></P>  <P>We do not <STRONG>formally</STRONG> support the use of firewalls or
  proxies, including home cable modem/DSL NAT routers and other Internet
  Connection Sharing devices or software. However, the following information
  might be helpful if you need to troubleshoot a connection that utilizes one
  of those technologies:</P>  <P><STRONG>Patch Application:</STRONG><BR>A TCP connection is initiated from the player's PC (using ports numbered
  &gt;1023) to patch.everquest.com, port 7000. There are multiple IP addresses
  for the patch.everquest.com clusters, and they may change as the network
  demands, so check often using a DNS lookup tool if you provide a specific
  firewall rule for patching.</P>  <P><STRONG>EverQuest client:</STRONG><BR>UDP datagrams are sent to/from the player's PC (using ports numbered
  &gt;1023) from/to the EverQuest servers (using ports numbered &gt;1023).
  Additionally, ICMP Unreachables (ICMP type 3) are sent to/from the player's
  PC from/to the EverQuest servers.</P>  <P><STRONG>The EverQuest server IP addresses are on the subnets:</STRONG><BR>64.37.148.* , 64.37.149.* , 64.37.150.* , 64.37.151.* , 64.37.156.* ,
  64.37.157.* , 195.33.132.*, 195.33.108.*, 195.33.133.*, 199.108.1.*,
  199.108.2*, 199.108.3.* 199.108.4*, 199.108.5*, 199.108.13*, 199.108.9*,
  199.108.15*</P>  <P>Please note that EverQuest opens a random UDP data port from the player's
  PC every time EverQuest is run. Thus, the need to have all ports &gt;1023
  available.&nbsp;</P>  <P ALIGN="center"><STRONG>Software Firewall and Proxy
  Information</STRONG></P>  <P>Below is a basic walkthrough on how to set up some of the more popular
  software firewall programs to work with EverQuest. These settings should only
  be applied if you are currently having issues with your software firewall
  program in connecting to your EverQuest server, or you are able to connect to
  your EverQuest server and are experiencing random disconnects while in game.</P>  <P ALIGN="center"><A HREF="../../../../../support/tech_support/ts_network_support_firewall_proxy_info.jsp#ZoneAlarm">Zone
  Alarm</A><BR><A HREF="../../../../../support/tech_support/ts_network_support_firewall_proxy_info.jsp#Internet">Internet
  Connection Firewall</A><BR><A HREF="../../../../../support/tech_support/ts_network_support_firewall_proxy_info.jsp#Sygate">Sygate
  Personal Firewall</A><BR><A HREF="../../../../../support/tech_support/ts_network_support_firewall_proxy_info.jsp#Norton">Norton
  Personal Firewall</A><BR><A HREF="../../../../../support/tech_support/ts_network_support_firewall_proxy_info.jsp#McAfee">McAfee
  Personal Firewall</A><BR><A HREF="../../../../../support/tech_support/ts_network_support_firewall_proxy_info.jsp#Tiny">Tiny
  Personal Firewal</A>l<BR>&nbsp;</P>  <P><A></A>Zone Alarm:</P>  <UL>   <LI>Double click on
       the Zone Alarm system tray icon (the system tray icon is located in the
       lower right hand corner of your screen where your clock is)</LI>   <LI>Click on Program
       Control</LI>   <LI>Slide the Tab to
       the LOW setting</LI>   <LI>Close the window
       out and run EverQuest</LI>  </UL>  <P><A></A>Internet Connection Firewall: </P>  <UL>   <LI>Right click on My
       Network Places and click Properties</LI>   <LI>Right click on LAN
       Connection (or whatever you might have named your broadband connection)
       and click Properties</LI>   <LI>Click the Advanced
       Tab</LI>   <LI>Click the check box
       that states &quot;Protect my computer and network by limiting or
       preventing access to this computer from the internet&quot;</LI>   <LI>Click Apply</LI>   <LI>Close the window and
       run EverQuest</LI>  </UL>  <P><A></A>Sygate Personal Firewall: </P>  <UL>   <LI>Right click on the
       Sygate Personal Firewall icon in the system tray and left click Sygate
       Personal Firewall Pro</LI>   <LI>Click Security and
       then click Allow All</LI>   <LI>Close the Window
       and run EverQuest</LI>  </UL>  <P><A></A>Norton Personal Firewall: </P>  <UL>   <LI>Go through the
       Automated Setup</LI>   <LI>Setup Home
       Networking if you need to do so</LI>   <LI>Run the Automatic
       Program Scan</LI>   <LI>Check boxes next
       to all EverQuest related programs and select Permit All under Internet
       Access</LI>   <LI>Click Finish</LI>   <LI>Double click on
       the system tray icon for Norton and select Personal Firewall</LI>   <LI>Click on Configure</LI>   <LI>Slide the Firewall
       slider to LOW</LI>  </UL>  <P><A></A>McAfee Personal Firewall: </P>  <UL>   <LI>Run through the
       Configuration Assistant</LI>   <LI>Select Allow all
       traffic</LI>   <LI>Follow through the
       Startup Options and the Control Access to shares sections</LI>   <LI>Under Allowed
       Applications, select Search all drives IF EverQuest did not show up
       under the initial scan</LI>   <LI>Place check marks
       next to all EverQuest related applications that show up</LI>   <LI>Click finish and
       allow the Firewall to apply the settings and reboot your computer</LI>   <LI>Enter EverQuest</LI>  </UL>  <P><A></A>Tiny Personal Firewall: </P>  <UL>   <LI>Right click on the
       Tiny Personal Firewall system tray icon and select Administration Tool</LI>   <LI>Click Application
       Groups</LI>   <LI>Click Settings and
       then click Create new Application Group</LI>   <LI>Name the new
       Application Group EverQuest and set the Security Level to Unrestricted</LI>   <LI>Go into the
       Unrestricted Applications group and look for all executable files listed
       under your EverQuest directory and move them to the EverQuest
       Application Group you just created</LI>   <LI>Close the window
       and run EverQuest</LI>  </UL>  <P>If the walkthrough for your respective program does not resolve your
  issue, please contact the SOE Technical Support staff at (858)537-0898, or
  email them at eqtech@soe.sony.com. You can also reach them via Live Chat
  located at <A HREF="http://www2.station.sony.com/en/instantchat.jsp">http://www2.station.sony.com/en/instantchat.jsp</A>.</P>  <P>&nbsp;</P>
