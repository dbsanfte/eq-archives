...
<map name="Map"> 
  <!-- Sign in    --><area shape="rect" coords="88,46,149,62" href="javascript:gnav_call('https://login.station.sony.com/login/station/login.jsp?returnURL=http://eqlive.station.sony.com/support/tech_support/ts_network_support_firewall_proxy_info.jsp?null');">
...
<DIV align=center><STRONG><BR>Firewall and Proxy Information</STRONG> </DIV>
<P align=left>We do not support the use of firewalls or proxies, including home cablemodem/dsl NAT routers and other Internet Connection Sharing devices or software. However, the following information might be helpful if you need to troubleshoot a connection that utilizes one of those technologies:</P>
<P align=left><B>Patch Application:</B><BR>A TCP connection is initiated from the player's PC (using ports numbered &gt;1023) to patch.everquest.com, port 7000. There are multiple IP addresses for the patch.everquest.com clusters, and they may change as the network demands, so check often using a DNS lookup tool if you provide a specific firewall rule for patching.</P>
<P align=left><B>EverQuest client:</B><BR>UDP datagrams are sent to/from the player's PC (using ports numbered &gt;1023) from/to the EverQuest servers (using ports numbered &gt;1023). Additionally, ICMP Unreachables (ICMP type 3) are sent to/from the player's PC from/to the EverQuest servers.</P>
<P align=left><B>The EverQuest server IP addresses are on the subnets:</B><BR>64.37.148.* , 64.37.149.* , 64.37.150.* , 64.37.151.* , 64.37.156.* , 64.37.157.* , 63.241.40.*, 63.241.41.*, 63.241.42.*, 63.241.43.*, 195.33.132.*, and 195.33.108.* These may be summarized as 64.37.148.0/22 , 64.37.156.0/23 , 63.241.40.0/22 , 195.33.132.0/24 , and 195.33.108.0/24</P>
<P align=left>Please note that EverQuest opens a random UDP data port from the player's PC every time EverQuest is run. Thus, the need to have all ports &gt;1023 available.</P>
<P align=left><BR>If you are still having problems after visiting this support section, you can contact our Technical Support Department through an email via our <A href="http://www.station.sony.com/services/everquest-technical.jhtml" target=_blank>online form</A>.<BR></P>
<DIV></DIV><!-- #EndEditable -->
