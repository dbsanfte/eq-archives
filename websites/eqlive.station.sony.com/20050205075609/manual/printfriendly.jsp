...
<body bgcolor="#FFFFFF" text="#000000">
  ...
  After a deity and starting city has been determined, click <STRONG>Enter World</STRONG>  to start the game.
	...
	You begin the game in your starting
  city with these items (press&nbsp;<EM><STRONG>i</STRONG></EM> to view them): <BR><BR>* 20 Bread cakes and 20 skins of milk<BR>* Spells (for some classes)<BR>* Standard weapon (dagger, sword, or club)<BR>* Scroll with instructions (a tattered note: right click on the scroll to
  ...
  by using weapon skills you've learned, but combat is generally easiest if you
  use the auto-attack function instead. Once you've got a target, you can
  simply press <STRONG><EM>q</EM></STRONG> (or whichever key you've remapped this function
  to), and your character will automatically make the attack. Your only task at
  that point is to stay close enough to the target to hit it. <BR><BR><STRONG><EM>q</EM></STRONG> Initiate auto-attack (on current target). <BR><EM><STRONG>Esc</STRONG></EM>ape Remove current target from Target Window. <BR><BR>If you prefer to take your swings or shots manually, you can do so - as long
  ...
  spell book.<BR>* Click on a spell scroll in your inventory to place the scroll on your
  cursor (you can right-click and hold&nbsp;on a scroll first to view its
  contents).<BR>* Click on a free slot in your spell book to scribe (write) the spell.<BR><BR>Scribed spells are permanently recorded in your spell book. But you can't
  ...
  feeling very brave. <BR><BR>FYI: 1 platinum = 10 gold = 100 silver = 1000 copper
	...
	Your character possesses a few
  basic skills at the start of the game. Everyone can Beg, Fish, use Sense
  Heading and Bind Wounds. Some skills (like Elvish or Pick Lock) are only
  originally known to those of a specific race or class, while others that you
  don't originally know can be learned over time (such as Meditation if you are
  a spell caster). <BR><BR>In order to use a skill you'll want to place it on an easily accessed hot key
  ...
  you progress to the next level. <BR><BR>Open your inventory to view your current level and how much experience you need
  to advance to the next level. To gain experience; find your guild, kill
  ...
  game server, the Character Select screen appears. It lets you create, manage
  and delete characters. Each of the eight Create New Character slots can store
  a single character that you create. The rest of this section shows you how to
  create a character, step-by-step. <BR><BR>* Click any <STRONG>Create New Character</STRONG> slot to begin. This displays the Character
  Creation screen. <BR><BR>* Follow the steps in the rest of this section to create a character. When
  ...
  Paladin, Wizard<BR><EM>Racial tensions</EM>: Iksars, Ogres, Trolls,
  Dark Elves<BR><BR><STRONG><EM>Gnome</EM></STRONG>: Distant cousins to Dwarves, Gnomes tend to burrow in the
  ...
  Berserkers are fearsome fighters, wielding two-handed axes and hurling throwing
  weapons. They have the best throwing arms of all Norrath classes. Their
  innate rage causes them to reach a frenzied battle state known as
  ...
  include Pick Lock and Pick Pockets. <BR><EM>Modifiers</EM>: +10 Agility, +10 Dexterity <BR><BR><STRONG><EM>Shadow Knight</EM></STRONG>: The antithesis of a Paladin, a Shadow Knight derives
  power from the evil gods he serves. He's part Warrior, part Necromancer and
  has the innate ability to damage flesh with his Harm Touch. <BR><EM>Modifiers</EM>: +10 Strength, +5 Stamina, +10 Intelligence, +5 Charisma <BR><BR><STRONG><EM>Shaman</EM></STRONG>: Similar to a Cleric, but closer to a tribal witch
  ...
  everyone has a compatible race, class and deity.</EM>
	...
	<P>Choosing a server is one of the first decisions you must make
  when you enter EverQuest. Though it is true that many people who have friends
  in game will choose the server where their friends are, it is generally
  recommended that unconnected people choose the least populated server for
  play. <BR><BR>There are also special servers with different rules from those of the
  ...
  servers. Any quest-related bugs should be submitted via e-mail to <A HREF="mailto:eqtesting@soe.sony.com">eqtesting@soe.sony.com</A>. In addition,
  the Test server has NO official Game Master support, therefore, if an item(s)
  is lost due to a bug, server outage, or any other reason, there will be NO
  reimbursement for the lost goods. It should also be noted that due to the
  nature of testing, the Test server may be brought down at anytime, without
  notice. Finally, it may be necessary from time to time to execute character
  and/or equipment wipes in order to maintain the integrity of the test
  platform.</EM></P>
  ...
  is designed to be easy to use and adjust for every player. With a few clicks
  ...
  some windows with a simple click and drag of the mouse. Other windows can't be
  resized at all. <BR><BR>Look at the borders around the windows. If the border is a simple, solid
  ...
  Simply click and hold the left mouse button, then drag the mouse left and right
  or up and down to change the size of the window. <BR><BR>Notice the directions that the arrows point. You can only drag the window in
  ...
  Selector cannot be closed. <BR><BR><EM>Tool Tip: Pointing your mouse at any of the icons will tell you the name of
  the window that the icon represents.</EM>
  ...
  open by default. You can click on the second icon from the left (the human figure
  ...
  Selector, or by hitting <STRONG><EM>Alt-O</EM></STRONG>. The Options Window has four tabs;
  General, Display, Mouse and Keyboard. Each of these tabs allows you to change
  some of the basic settings of the game. Where an option is set by a button,
  the option is ON if the button is depressed, OFF if the button is out. <BR><BR><EM>Note: There is no 'done' or 'accept' button for the Options Window.
  ...
  cases reducing the clip plane can improve performance. <BR><BR><EM>Particle Density</EM> (Off/Low/Medium/High): Adjust number of particles displayed
  during spell effects. In some cases a lower setting may improve performance. <BR><BR><EM>Window Transparency</EM>: This sets the default transparency (alpha) level
  ...
  of your display is a green bar with four numbers over it. This is your Connection
  Status Bar. It gives you information about your connection to our servers
  through the Internet. This isn't really a good place to try to explain the
  intricacies of the Internet, so we're just going to tell you what the numbers
  are. <BR><BR>Left number - shows the round-trip ping<BR>Second number - shows the packet loss incoming to your machine<BR>Third number - shows the packet loss going to the server<BR>Fourth number - shows the throughput <BR><BR>The main thing to know about the Connection Status Bar is that it changes
  ...
  current INI settings for the new skin, or 0 if you want to load the
  positions/sizes from the XML (and then appropriately modify them for your
  resolution). <BR><BR><STRONG>Labels:</STRONG> <BR><BR>The interface also provides Labels for some of the information that is sent
  ...
  a member of a guild for this chat channel to be available.
	...
	You can appear to bow, be bored,
  point, or convey other visible gestures by using other special commands.
  EverQuest has a lengthy <A HREF="../../../../../manual/manual.jsp%3fhandle=emotelist">list of built-in
  ...
  not accessible from within the game. Instead, click <STRONG>Chat</STRONG> from the Login
  screen (which appears when you activate the EverQuest desktop icon). You can
  use chat to speak to other players about the game and ask questions. Other
  players are often a great source of answers.
	...
	The world of Norrath is an abundant
  world full of items you can pick up, buy, sell, use or transform. When your
  character is &quot;born&quot; he already has several items - a basic weapon,
  some food and water, and a scroll or two (items vary slightly, depending on
  your class and race). <BR><BR>Before long, you'll acquire some valuables. Eventually, you'll want to buy
  ...
  Inventory window. It is often considered polite to ask permission before doing
  such an inspection. <BR><BR>See <A HREF="../../../../../manual/manual.jsp%3fhandle=giving">Giving/Trading
  ...
  hundreds of spells that you can acquire and cast at various levels of experience.
  ...
  before you can cast it):<BR><BR>* Click on the picture of the learned spell in your spell book. A rock-colored
  gem appears on your cursor.<BR>* Click on one of the octagonal spell gems in your Spells Window to the left
  ...
  indicates something about the spell that is memorized in the slot. <BR><BR><EM>Yellow</EM>: You can only cast it on yourself.<BR><EM>Red</EM>: You can cast it on your current target.<BR><EM>Green</EM>: The spell affects anyone close to your target.<BR><EM>Blue</EM>: The spell has no range, and will radiate out from you.<BR><EM>Orange</EM>: The spell only summons items and doesn't affect anyone.<BR><EM>Purple</EM>: You can cast it on everyone in your group.
	...
	Prior to casting, make sure you're
  ready (especially in a combat situation): <BR><BR>* Make sure that you have the spells you plan to use memorized (remember the <STRONG><EM>Alt</EM></STRONG>  key pops up the names of the spells).<BR>* Check your mana supply. If it's low, wait to attack or find a way to boost
  ...
  to magic - Abjuration, Alteration, Channeling, Conjuration, Divination, Evocation,
  Meditate and Specialization (see <A HREF="../../../../../manual/manual.jsp%3fhandle=skillsmagic">Magical Skills</A>,
  ...
  you can check its corpse for useful items. Many things you recover from dead creatures
  are valuable items that you can sell to merchants or other players. <BR><BR><EM>To loot a corpse:</EM> <BR><BR>* Right-click on the corpse.<BR>* Click on an item in the corpse's inventory. It &quot;floats&quot; on your
  cursor.<BR>* Click on a free slot in your inventory.
	...
	If you have the Taunt skill, you
  can use it to draw a creature's attention away from whomever it's attacking.
  When you successfully taunt a monster, it turns and starts attacking you
  instead. You'll find that taunting is an indispensable group skill,
  ...
  you with either a weapon or spell, you take damage and your health is depleted.
  Keep an eye on your red health bar - if it falls below 1/3, you may want to
  consider retreating until you're healed. <BR><BR><EM>Tip: If your party includes a spell caster, that character can spend his
  ...
  unfriendly creatures if they come too close. This can be good for you (if,
  for example, some creature is about to beat you to a pulp), and bad for you
  (if a guard kills a monster you won't get experience for it nor will you be
  able to loot the corpse). So it's a good idea to know where the local guards
  are.
	...
	If you do decide that retreating
  is your best option, look for the safest escape route. Sometimes, you can run
  over to other characters in the area, and someone will help attack the
  monster or taunt him away. But those other characters might not be willing or
  able to help you out. In fact, running over to them might get them killed.
  While it's certainly a good thing to find help when needed, it's not polite
  to bring your problems to other unasked. Communicate with those around you,
  find out if they would be willing to help you before you need it. <BR><BR>There is a term, &quot;training&quot;, for rudely running a bunch of
  ...
  avoid. <BR><BR>Guards (as mentioned in the previous section) can also help save your life.
	...
	Some classes let you develop the
  ability to summon undead pets or elemental forces, or to befriend wild
  ...
  specific goal. Most quests offer some sort of reward. <BR><BR><EM>Ready-Made Quests</EM>. Many non-player characters have small quests. The best
  way to find out what they are is to hail various NPCs and ask. Non-player
  characters have certain keywords marked in square brackets - for example,
  &quot;I need some help with my [problem].&quot; If you respond with a
  question that includes that keyword, you'll receive more information. <BR><BR><EM>Player-Created Quests</EM>. Players can create their own quests and
  ...
  characters, celebrations, and in some cases, even lotteries and contests.
	...
	Adventuring outside the city can
  be dangerous. Joining a group can help you defeat monsters that you normally
  ...
  skill has been replaced by &quot;Tail Rake&quot; for Iksar characters.
	...
	Your EverQuest directory contains
  a file named EQCLIENT.INI. This file contains custom settings for most user
  preferences within EverQuest. The EQCLIENT.INI file is split up into several
  sections.
	...
	<STRONG>[Defaults]</STRONG> <BR><BR>This section contains general information regarding client setup. Items are listed
  here alphabetically, they may not appear in this order in the .ini file. The
  order of their appearance in the file is inconsequential to their function. <BR><BR><STRONG>AttackOnAssist</STRONG> = TRUE or FALSE <BR><BR>Sets whether or not you want &quot;autoattack&quot; to be automatically
  ...
  1600/1200. Players need to have the Velious expansion in order to set this line
  to 3 or 4. <BR><BR><STRONG>VideoModeBitsPerPixel</STRONG> = [16/32] <BR><BR>This line reflects and controls the video mode while in EverQuest. Default is
  ...
  Specify here what NPC /con colors you do not want to see in your tracking display.
  ...
  MAIN, INV, TRACK, or HOTBOX <BR>&lt;red&gt; &lt;green&gt; &lt;blue&gt; -&gt; 0 to 255 <BR>&lt;trans&gt; -&gt; 0 = off, 1 = 25, 2 = 50, 3 = 75, 4 = 100 percent <BR>To reset a window color use /wincolor &lt;window name&gt; reset <BR><BR><STRONG>/yell</STRONG> - will issue a &quot;cry for help&quot; to all players within a 100
  foot radius. Characters hearing the yell will be told what general direction
  ...
  always be preceded by the name of the guild officer that set it up. There is
  a maximum of around 200 characters for this guild motd. Any new guild message
  ...
  by giving a Priest of Discord the <STRONG>Child of Discord</STRONG> book in your
  ...
  command line.
	...
	p.49: The next-to-last paragraph
  on this page reads, &quot;You can have anywhere from 0 to 100 points in any
  skill, and it takes 10 skill points to get each next higher skill
  ranking&quot;. This section is incorrect. This section should read: <BR><BR>Skill values start at 0 and go up from there. The maximum value of any skill
  ...
  more models and graphics features you have turned on, the greater the potential
  increase in loading times you will see using the Texture Cache. <BR><BR><EM>Remember</EM>: This will require a lot of hard drive space, so be certain
  ...
  have the option of choosing which enhancement you wish to gain. Some
  enhancements may have prerequisites. A prerequisite is a skill or skills that
  you will need to obtain before you can get the enhancement. <BR><BR>Experience loss due to death suffered by the character will be taken from the
  ...
  change existing ones at a future date. We will, of course, attempt to keep changes
  to a minimum and will update this document to reflect such changes.
  ...
  on General Abilities. There are three Archetypes&nbsp;- Caster, Melee and Priest.
  <BR><BR>Enchanters, Magicians, Necromancers and Wizards can choose abilities from the
  ...
  restrictions will be listed in the skill description.</EM>
	...
	<STRONG>Caster Archetype Abilities</STRONG>  <BR><BR>Abilities from this Archetype are available to Enchanters, Magicians, Necromancers
  and Wizards, as well as Bards and Shadowknights. The first skill level of
  each Archetype ability costs two skill points. The second skill level of each
  ...
  level prevents failure of spells under 35th level and the third prevents failure
  of spells under 52nd level. <BR><BR><EM>Spell Casting Fury</EM>: (3 skill levels) - Gives the caster the ability to
  ...
  well as Bards, Beastlords, Paladins, Rangers and Shadowknights. The first skill
  ...
  any other class. <BR><BR><EM>Combat Stability</EM>: (3 skill levels) - Each skill level gives the character
  an increased ability to reduce the damage he takes in combat (damage
  mitigation). The first skill level increases the character's damage
  ...
  passive. <BR><BR><BR><EM>Channeling Focus</EM>: (3 skill levels) - Each skill level reduces the
  chances of spell interruption due to damage or movement. The first skill
  level reduces the chance of interruption but 5%, the second skill level increases
  the reduction to 10% and the third brings the reduction to 15%. <BR><BR><EM>Healing Adept</EM>: (3 skill levels) - Each skill level improves the
  ...
  available to characters level 59 and above that have spent twelve skill points
  to purchase Archetype Abilities. Obviously a character can only choose Class
  ...
  expenditure of mana. The Refresh Timer on this ability is 4,320 minutes (60 days
  of game time). <BR><BR><EM>Innate Invisibility to Undead</EM>: (1 skill level - cost 3 skill points) -
  ...
  level - cost 9 skill points) - This ability allows the Druid to charm an NPC without
  fear that the NPC will break the charm (charm will end only when the Druid
  leaves the zone or logs out) without the memorization of a spell or the
  ...
  - cost 9 skill points) - This ability allows the Enchanter to charm an NPC
  without fear that the NPC will break the charm (charm will end only when the
  Enchanter leaves the zone or logs out) without the memorization of a spell or
  the expenditure of mana. All of the usual restriction for charm spells apply
  to this ability as well, except in regards to duration. The Refresh Timer for
  this ability is 72 minutes (one day of game time). <BR><BR><EM>Gather Mana</EM>: (1 skill level - cost 5 skill points - Prerequisite: 3 skill
  levels of the Caster Archetype ability Mental Clarity) - This ability allows
  the Enchanter to instantly regain all of his lost mana. The Refresh Timer for
  this ability is 144 minutes (two days of game time). <BR><BR><EM>Jewel Craft Mastery</EM>: (3 skill levels) - Each skill level reduces the
  chance of failure when using the jewel craft skill. The first skill level
  reduces the failure chance by 10%, the second skill level raises the
  reduction to 25% and the third skill level raises the reduction to 50%. <BR><BR><EM>Mass Group Buff</EM>: (1 skill level - cost 9 skill points) - This ability
  ...
  power to summon a corpse without the use of components or memorization of a spell.
  ...
  critical hits into massive damage to undead.
	...
	<EM>Ambidexterity</EM>: (1 skill
  level - cost 9 skill points) - This ability makes the Ranger more skilled
  with the use of two weapons by increasing his chance to successfully dual
  wield. <BR><BR><EM>Archery Mastery</EM>: (3 skill levels) - This ability makes the Ranger a
  ...
  levels of the Melee Archetype ability Fear Resistance) - This ability makes
  the Shadowknight permanently immune to fear effects. <BR><BR><EM>Leech Touch</EM>: (1 skill level - cost 6 skill points) - This ability gives
  the Shadowknight the power to use a lifetap Harm Touch that does not require
  the memorization of a spell or the expenditure of mana. This ability uses the
  existing Harm Touch Refresh Timer. <BR><BR><EM>Pet Discipline</EM>: (1 skill level - cost 6 skill points) - This ability
  ...
  caster's group. This Ability causes the spell to use up to as much as twice
  as much mana. It will require at least the normal casting cost, and will use
  up to as much as twice the mana if the caster has that much available. The Refresh
  Timer for this ability is 72 minutes (one day of game time). <BR><BR><EM>Pet Discipline</EM>: (1 skill level - cost 6 skill points) - This ability
  ...
  by Spell Casting Reinforcement to 50%.
	...
	<EM>Ambidexterity</EM>: (1 skill
  level - cost 9 skill points) - This ability makes the Warrior more skilled
  with the use of two weapons by increasing his chance to successfully dual
  wield. <BR><BR><EM>Area Taunt</EM>: (1 skill level - cost 5 skill points) - This ability
  ...
  spell Succor instantly and without the memorization of a spell or the expenditure
  ...
  Wizard's ability to do 'critical' damage with direct damage spells. Each skill
  level increases the chance to do critical damage with a direct damage spell. <BR><BR><EM>Strong Root</EM>: (1 skill level - cost 5 skill points) - This ability
