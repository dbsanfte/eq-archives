	...
	<tr>
		<td><table width="780" height="26" cellpadding="0" cellspacing="0" border="0">
			<tr height="26">
				...
				<td width="162" height="26"><a href="https://login.station.sony.com/login/station/login_popup.jsp?returnURL=http://eqlive.station.sony.com/support/tech_support/ts_status_protocol.jsp?null" 
					target="_login" onclick="window.open('https://login.station.sony.com/login/station/login_popup.jsp?returnURL=http://eqlive.station.sony.com/support/tech_support/ts_status_protocol.jsp?null', '_login', 'titlebar=no,status=yes,toolbar=no,width=350,height=380,resizable=yes'); return false;" 
				...
				<td width="81" height="26"><a href="https://www.station.sony.com/secure/en/registration/registration.jsp?returnURL=http://eqlive.station.sony.com/support/tech_support/ts_status_protocol.jsp?null" target="_top" onmouseover="gn2_swap(gn2_img_join,gn2_join_on);" onmouseout="gn2_swap(gn2_img_join,gn2_join_off);"><img src="/common/global_nav/images/gn2/gn2_btn_join_off.gif" width="81" height="26" border="0" alt="Join Free" name="gn2_img_join"></a></td>
...
<P ALIGN="center"><STRONG><BR><STRONG>EverQuest Status Server Protocol</STRONG></STRONG></P>  <P><STRONG>NOTE:</STRONG> The information contained within this document is
  provided as reference only. Sony Online Entertainment cannot and will not
  provide assistance with implementing this code, nor will it provide the
  scripts for you.</P>  <P><STRONG>PURPOSE :</STRONG> The EverQuest Status Server is created and
  maintained by Sony Online Entertainment / Verant Interactive to provide
  support for the EverQuest Status Server Protocol.&nbsp; The EverQuest Status
  Server Protocol is used by any application software that needs to get
  information as to the current status of all everquest chat, world, and zone
  servers.&nbsp; By using the Everquest Status Server protocol, it is possible
  to get the following information :</P>  <UL>   <LI>If the chat server,
       or any of the world or zone servers are up or down.</LI>   <LI>The status of each
       world server, UP or DOWN.</LI>   <LI>Up/Down status of
       each zone for each world.</LI>   <LI>Contents of Chat
       Server Banner.</LI>  </UL>  <P><STRONG>USAGE :</STRONG> Communication with the EQ Status
  Server is accomplished via the UDP protocol using host <STRONG>status.everquest.com</STRONG>  on port <STRONG>24252</STRONG>.&nbsp; The protocol uses a series of very
  basic command and reply sequences, which are explained below : </P>  <P><STRONG>COMMANDS :</STRONG>&nbsp; Currently, the Status Server Protocol
  supports 3 commands.&nbsp; The commands that are sent from the client are
  very basic, and use the same format.&nbsp; Each command consists of 4 bytes
  of data.&nbsp; The first two bytes are always the same (0xFFFF), the second
  two bytes represent the actual command code.<BR><BR>&nbsp;</P>  <TABLE STYLE="width:70.0%;mso-cellspacing:1.5pt;mso-padding-alt:0in 5.4pt 0in 5.4pt"><TR STYLE="mso-yfti-irow:0;mso-yfti-firstrow:yes"><TD STYLE="padding:.75pt .75pt .75pt .75pt"><STRONG>Command</STRONG></TD>    <TD STYLE="padding:.75pt .75pt .75pt .75pt"><STRONG>Sequence</STRONG></TD>    <TD STYLE="padding:.75pt .75pt .75pt .75pt"><STRONG>Notes</STRONG></TD></TR>   <TR STYLE="mso-yfti-irow:1"><TD STYLE="padding:.75pt .75pt .75pt .75pt">Request Server Names&nbsp;</TD>    <TD STYLE="padding:.75pt .75pt .75pt .75pt">0xFFFF0100</TD>    <TD STYLE="padding:.75pt .75pt .75pt .75pt">Request a list of all server names</TD></TR>   <TR STYLE="mso-yfti-irow:2"><TD STYLE="padding:.75pt .75pt .75pt .75pt">Request Abbreviated Status Data</TD>    <TD STYLE="padding:.75pt .75pt .75pt .75pt">0xFFFF0500</TD>    <TD STYLE="padding:.75pt .75pt .75pt .75pt">Request player zone status information for all servers.</TD></TR>   <TR STYLE="mso-yfti-irow:3;mso-yfti-lastrow:yes"><TD STYLE="padding:.75pt .75pt .75pt .75pt">Request Chat Banner</TD>    <TD STYLE="padding:.75pt .75pt .75pt .75pt">0xFFFF0900</TD>    <TD STYLE="padding:.75pt .75pt .75pt .75pt">Request the banner set in the Chat Server.</TD></TR></TABLE>  <P><BR><BR><BR></P>  <P>NOTE 1 : You should only need to send the Request Server
  Names command once, at the start of your application to get the current list
  of server names. </P>  <P>NOTE 2 : When coding your client application you should try to ensure that
  there is a buffer between sending Request Status Data commands, to prevent
  clients from flooding the status server.&nbsp; Please try to not allow
  clients to refresh more than once every 20 to 30 seconds.</P>  <P><STRONG>REPLIES :</STRONG> After you send one of the commands to the Status
  Server, you will get a reply back for that command.&nbsp; For each of the
  commands above, there is an associated reply.&nbsp; The first 4 bytes of each
  reply contain the same 4 bytes that were sent as the command.&nbsp; After
  that, the rest of the reply is based on the command being sent.</P>  <P><STRONG><U>Request Server Names Reply</U></STRONG></P>  <P>This reply contains a listing of current server names, including the chat
  server, and uses the following format :</P>  <BLOCKQUOTE>  <P>BYTE 1 to BYTE 4 - Command Code (0xFFFF0100)<BR>BYTE 5 - Total number of server names<BR>BYTE 6 - Total number of zones in each server<BR>BYTE 7+ - Name Data Stream - The rest of the data is each name of each server
  seperated by a null (0x00) character.&nbsp; The number of names returned is
  equal to the total number of server names count provided in BYTE 5</P>  </BLOCKQUOTE>  <P>Example : </P>  <P>FF FF 01 00 02 46 43 68&nbsp; -&nbsp; 61 74 20 53 65 72 76
  65&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; ......Chat Serve<BR>72 00 54 65 73 74 20 53&nbsp; -&nbsp;&nbsp; 65 72 76 65 72
  00&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
  r.Test Server.</P>  <P>This example shows a server count of 2, a zone count of 70
  (0x46), and lists the two server names as Chat Server, and Test Server.&nbsp;
  You will note that Chat server is returned as one of the total servers. </P>  <P><STRONG><U>Request Abbreviated Status Data Reply</U></STRONG></P>  <P>Here you will get the actual server status, player counts, and indication
  of how many zones are up.&nbsp; The structure of the returned data is as
  follows :</P>  <BLOCKQUOTE>  <P>BYTE 1 to BYTE 4 - Command Code (0xFFFF0500)<BR>BYTE 5 - Total Number of Chat Servers<BR>BYTE 6 to BYTE 7 - Server Status of chat server. -1 = DOWN and 1 = UP<STRONG>*</STRONG><BR>BYTE 8 - Total Number of World Servers<BR>BYTE 9+ - World Server Data (Size = 4 Bytes * Total Number of World Servers) </P>  <P>(World Server Data Block)<BR>BYTE 1 to BYTE 2 - Status of world server. -1 = DOWN and 1 = UP<BR>BYTE 3 - Total number of zones in this world<BR>BYTE 4 - Total number of zones currently up.</P>  <P><STRONG><U>Request Chat Banner</U></STRONG></P>  <P>You will obtain the text string equivalent of the banner currently running
  in the Chat Server. It is null-terminated and will be a null if there is no
  banner. Maximum length of the entire packet returned is 512 bytes.&nbsp; The
  structure of the returned data is as follows :</P>  <BLOCKQUOTE>  <P>BYTE 1 to BYTE 4 - Command Code (0xFFFF0900)<BR>BYTE 5+ - The banner string, null terminated.</P>  </BLOCKQUOTE>  <P><STRONG>ADDITIONAL INFORMATION :</STRONG> Server status clients generally
  may be written in C, C++, PERL, Java and PHP4, or almost any language that
  communicates via UDP. However, please note that Sony Online Entertainment
  cannot provide sample source code nor assist with, or provide technical
  support for, the use of the status server or the protocol outlined in this
  document. Both the status server and the protocol are presented as a free
  public service, on an as-is basis with no guarantee as to their operability
  or availability, nor any guarantee as to the accuracy of the information
  provided. Sony Online Entertainment disclaims responsibility for any and all
  consequences, claims or damages arising from the user's reliance upon the
  status server or the information provided therein. Sony Online Entertainment
  makes no representations or warranties, express or implied, regarding the
  server and the information, including any implied warranty of
  merchantability, fitness for a particular purpose, or noninfringment, nor any
  implied warranties arising from course of dealing or course of performance.
  We reserve the right in our sole discretion to discontinue the server or
  public access to the server at any time.</P>  <P>Copyright &copy; 2001 Sony Online Entertainment Inc.</P>  </BLOCKQUOTE>  <BLOCKQUOTE>  <BLOCKQUOTE>  <P>&nbsp;</P>  </BLOCKQUOTE>  </BLOCKQUOTE>  <P>If you are still having problems after visiting this
  support section, you can contact our Technical Support Department through an
  email via our <A TARGET="_blank" HREF="http://www.station.sony.com/en/services/help/premiumGames.jsp">online form</A>.</P>
