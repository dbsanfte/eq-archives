...
<P ALIGN="center"><STRONG><BR><STRONG>Security Alert/Hack Attempt</STRONG></STRONG></P>  <P>The EverQuest servers and their networks are access-controlled, secured,
  ...
  networking that our games utilize, and packets where the source IP address was
  spoofed.</P>  <P>The EverQuest game uses a high rate of UDP packets to pass game movement
  ...
  false alarms are:</P>  <UL>   <LI>&quot;Default Block Bla
       ...
       will send updates back to you from port 7000. In order to optimize the
       patching proccess, we are using load-balancing network hardware, and
       have distributed the patchers among different geographic locations. As
       an artifact of that optimzation, there are times when you patching
       client will complete the proccess, but a few more packets keep coming
       back from the patching servers.</LI>   <LI>UDP packet storm/flood:
  ...
  each other over the Internet. Some key features are that all IP addresses must
  be unique on the Internet, and programs on that host each use a port number
  ranging from 1 to 65535 to identify themselves to the networking piece of the
  operating system, so that packets coming into that host get routed to the
  right program.</P>  <P><STRONG>ISP : </STRONG><EM><STRONG>Internet Service Providor</STRONG></EM> - a
