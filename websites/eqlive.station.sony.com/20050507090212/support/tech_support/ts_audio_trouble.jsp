...
<P ALIGN="center"><STRONG><BR><STRONG>General Audio Troubleshooting</STRONG></STRONG></P>  <P><STRONG>Is the
  problem specific to EverQuest?</STRONG></P>  <P>The following
  checklist will quickly isolate whether the problem is specific to EverQuest.
  Typically, the wrong drivers are installed or old, inactive drivers were not
  completely replaced. Make sure you have the latest drivers for your sound
  card installed. To get the drivers we've currently tested, please visit the <A HREF="../../../../../support/tech_support/ts_audio.jsp">Supported Sound Cards</A>&nbsp;section.</P>  <OL>   <LI>If you have another video game that uses
       3D audio, try it out and verify that its hardware acceleration is
       working. If you do not have a 3D audio game, there is usually a demo
       that comes with 3D audio cards, that you can try. This will should
       indicate if you have the correct drivers installed - however - there are
       some circumstances where you could have the wrong drivers and the demo
       still works, because it is using DirectX instead of the manufacturer's
       proprietary API calls - such as the Montego Bay demo working on a
       Vortex-based card - but it won't play games, only the demo. (They were
       the wrong drivers.)</LI>   <LI>Make sure there are no conflicts in the
       Device Manager. Due to the major patches in November 2000, the EverQuest
       client now demands full time attention from video, sound, and
       communication devices. Your problem may stem from those devices sharing
       IRQs on the machine in question. Sharing occurs when you look at the
       list of used IRQs and see two devices listed multiply (when you have
       only one) or two devices assigned to a single IRQ (that should each be
       using a unique one). To check for this, right click the &quot;My
       Computer&quot; icon on your desktop and choose &quot;Properties&quot;
       from the menu. Next, choose the &quot;Device Manager&quot; tab, then
       double click the &quot;Computer&quot; entry at the top of the device
       list. This should bring up the IRQ Resource window. Start looking for
       multiple devices assigned to a single IRQ number. Make note of the IRQs
       being used by main devices such as sound, video, and communications
       devices. If these are shared, besides having an IRQ holder, you may
       experience some problems. In most cases, you will have to physically
       remove the device and re-seat it into a different motherboard slot to
       stop sharing. If you are not familiar with doing this, please take your
       machine to a local hardware technician for assistance.</LI>   <LI>Determine if you
       have installed DirectX since the last time the audio was working properly.
       Sometimes installing DirectX can change the audio driver, unbeknownst to
       the user. If this is the case, you can re-install the sound drivers.
       Make sure any previous drivers are removed from both the Device Manager
       and the Add/Remove Programs control panel. If the card in question
       doesn't have an obvious hardware conflict, the problem is likely to be a
       bad driver installation.</LI>  </OL>  <P>&nbsp;</P>  <P><STRONG>No Sound
  in EverQuest</STRONG></P>  <P>If you are not
  getting any sound in the game then while in the game, go to the Options
  screen and choose the General button. Move all of the volume sliders to the
  far right. If they are already there, sit and camp out of the game.</P>  <P>Check the sound
  card manufacturer's website for the latest drivers for your sound card. If
  you are not sure which sound card you have, you can right-click on the
  &quot;My Computer&quot; icon from your desktop and select
  &quot;Properties&quot; from the menu. Next, open up the &quot;Sound, Video
  and Game Controllers&quot; section. Your sound card will be listed in this
  area.</P>  <P>After
  downloading, and remembering where you downloaded the files to, properly
  install your new drivers. If there are no installation instructions packaged
  with the drivers, it is suggested to contact the sound card manufacturer for
  assistance. Many times, your drivers may become corrupt and a simple
  reinstallation may resolve your problem, without having to get the latest
  drivers from the manufacturer.</P>  <P>Another option
  to try updating your DirectX. You can obtain the latest version from &quot;<A HREF="http://www.microsoft.com/directx">http://www.microsoft.com/directx</A>&quot;.</P>  <P>Open your
  Control Panels. Double click the Multimedia control panel and select the
  Audio tab. Look for the option labeled &quot;Preferred Playback Device&quot;.
  Try selecting another device from the drop-down menu and re-run EverQuest. Windows98
  users can also try the following: Under &quot;Preferred Playback&quot;, click
  the &quot;Advance Properties&quot; button and choose the
  &quot;Performance&quot; tab. Lower the &quot;Hardware Acceleration&quot;
  slider, one notch to the left and re-run EverQuest.</P>  <P>Finally, there
  are also known resource conflicts between voice-capable modems and sound
  cards. Aside from causing crashes, this conflict can also cause the Voice
  Modem to &quot;steal&quot; sound from your Sound Card, effectively turning it
  off in the game. Go to the &quot;Sound, Video, and Game Controllers&quot;
  section of the Device Manager (right click on your &quot;My Computer&quot;
  icon and choose &quot;Properties&quot;). You will see a listing for
  &quot;Wave Device for Voice Modem&quot; or something similar. Double click on
  this entry and mark the &quot;Disable in this Hardware Profile&quot; option.
  Most people never use the Voice functions on their modems, and disabling this
  feature will not change the way you connect to your Internet provider.</P>  <P>&nbsp;</P>  <P><STRONG>Multiple
  Sound Cards in Single Machine</STRONG></P>  <P>This is not
  supported. If you have more than one audio device, the sound system will fail
  to recognize the best software for the music system. (It has no problem
  finding the hardware.) You may be able to disable one of the devices in the
  Device Manager and/or create a hardware profile that accomplishes the same
  thing. Some machines have onboard-integrated audio that can be disabled from
  the BIOS.</P>  <P>&nbsp;</P>  <P><STRONG>Emulated
  Sound Drivers</STRONG></P>  <P>Windows will use
  emulated (generic) drivers when installing hardware, if it does not find an
  exact match in it's default drivers. Emulated drivers will cause problems in
  EverQuest. You will need to visit the hardware manufacturer's website to get
  updated drivers for your sound card, or disable that sound card. Until this
  is done, you will continue to experience problems with your system.</P>  <P>&nbsp;</P>  <P><STRONG>Sound
  Blaster Live!&nbsp;</STRONG> Problem - Any driver related issue, check these
  FIRST:</P>  <OL>   <LI>Make sure all
       previous installations of other sound cards are completely removed from
       the Device Manager AND Add/Remove Software control panel. There are
       several entries in each which may need to be removed.</LI>   <LI>Install the drivers
       that came with your SoundBlaster Live card.</LI>   <LI>Get the Live!Ware
       3.0 package from the CD that came with your sound card, or download it
       using one of the following links: </LI>   <OL>    <LI><A HREF="http://www.americas.creative.com/support/files/download.asp?">Win98</A></LI>    <LI><A HREF="http://www.americas.creative.com/support/files/download.asp?">WinME</A></LI>    <LI><A HREF="http://www.americas.creative.com/support/files/download.asp?">Win2K</A></LI>    <LI><A HREF="http://www.americas.creative.com/support/files/download.asp?">WinXP</A></LI>   </OL>   <LI>After downloading
       the package, install it, then download and install any model specific
       driver updates (http://www.creativehelp.com). CRITICAL: you must install
       the Live!Ware 3.0 package first. Otherwise, settings in your control
       panels will not be updated correctly.</LI>   <LI>Once all the drivers
       are installed, reboot and try EverQuest again.</LI>  </OL>  <P>If you do not
  hear any music, then unload any sound fonts in the SoundFont area of the
  AudioHQ control panel. Load the default 2Mb GENERAL MIDI sound font that
  comes with the SBLive (default location is &quot;C:\\\\\\\\Program
  Files\\\\\\\\Creative\\\\\\\\SBLive\\\\\\\\Bank\\\\\\\\gmgsmt.sf2&quot;). Now
  find any .mid file in Windows, double-click it, and it should play using the
  loaded sound font. You will have to listen closely, the instruments should
  sound rich and vibrant. Try several of the MIDI-outs (in the Control Panel,
  Multimedia, MIDI console). Each should sound different if the Windows
  multimedia system is working. If the sound font doesn't play correctly, then
  the problem may lay inside Windows itself. Please contact the vendor of the
  questionable hardware to determine the nature of the problem within the OS.</P>  <P>If you are
  crashing due to 3D audio being activated, make sure you have the latest
  drivers from the creative site (BOTH Live!Ware 3.0 and SBLive driver update).
  Open the Multimedia control panel, and select the Audio section. Click
  Advanced Properties, then click Restore Defaults.</P>  <P>&nbsp;</P>  <P>If you are still
  having problems after visiting this support section, you can contact our
  Technical Support Department via email using our <A HREF="http://www.station.sony.com/en/services/help/eqGM.jsp">online form</A>.</P>    <P>&nbsp;</P>
